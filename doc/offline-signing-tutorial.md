...
> [!NOTE]
> While this tutorial demonstrates the process using the `testnet` network, you should omit the `-testnet` flag in the provided commands when working with `mainnet`.
...
```sh
[offline]$ ./build/bin/bitcoin-cli -testnet -named createwallet \
...
```sh
[offline]$ ./build/bin/bitcoin-cli -testnet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./build/bin/bitcoin-cli -testnet -named createwallet \
...
```sh
[online]$ ./build/bin/bitcoin-cli -testnet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./build/bin/bitcoin-cli -testnet -rpcwallet="watch_only_wallet" getnewaddress

tb1qtu5qgc6ddhmqm5yqjvhg83qgk2t4ewajg0h6yh
```

2. Have a small amount of testnet coins sent to the address from the previous command (mine them, or ask a testnet participant).
...
```sh
[online]$ ./build/bin/bitcoin-cli -testnet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./build/bin/bitcoin-cli -testnet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./build/bin/bitcoin-cli -testnet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./build/bin/bitcoin-cli -testnet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./build/bin/bitcoin-cli -testnet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./build/bin/bitcoin-cli -testnet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./build/bin/bitcoin-cli -testnet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./build/bin/bitcoin-cli -testnet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./build/bin/bitcoin-cli -testnet -rpcwallet="watch_only_wallet" listtransactions
