...
> committed in the genesis coinbase: 100,000,000 XID (decided 2026-09-25), no
> premine. The emission shape is final (2026-09-25), the Annual Tenth: a ramp to
> 50 XID per block, then 10% less every 110,000 blocks, floor 1.5 XID, 0.1 XID tail <!-- [EMISSION-SHAPE] -->
> to block 35,375,353 (~2363). The consensus implementation is `GetBlockSubsidy` in
> `src/validation.cpp` and the emission table in `src/consensus/params.h`,
> generated by `contrib/regenesis/emission.py`.
>
