...
Although there is already a brief explanation about the multisig in the [Descriptors documentation](/doc/descriptors.md#multisig), this tutorial proposes to use the testnet (instead of regtest), bringing the reader closer to a real environment and explaining some functions in more detail.
...
Before starting this tutorial, start the node on the testnet.
...
for ((n=1;n<=3;n++))
do
 ./build/bin/bitcoin rpc -testnet createwallet "participant_${n}"
done
```
...
for ((n=1;n<=3;n++))
do
 xpubs["xpub_${n}"]=$(./build/bin/bitcoin rpc -testnet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))' | sed 's /0/\* /<0;1>/* ')
done
```
...
checksum=$(./build/bin/bitcoin rpc -testnet getdescriptorinfo $desc | jq -r '.checksum')
...
Documentation for these and other parameters can be found by typing `./build/bin/bitcoin rpc -testnet help importdescriptors`.
...
```bash
./build/bin/bitcoin rpc -testnet createwallet "multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/bitcoin rpc -testnet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/bitcoin rpc -testnet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/bitcoin rpc -testnet loadwallet "participant_${n}"; done
./build/bin/bitcoin rpc -testnet loadwallet "multisig_wallet_01"
```

### 1.4 Fund the wallet

The wallet can receive testnet coins by generating a new address and having them sent to it (mine them, or ask a testnet participant).
...
```bash
receiving_address=$(./build/bin/bitcoin rpc -testnet -rpcwallet="multisig_wallet_01" getnewaddress)
```

To copy the receiving address onto the clipboard, use the following command. This can be useful when getting coins sent to the wallet.
...
```bash
./build/bin/bitcoin rpc -testnet -rpcwallet="multisig_wallet_01" getbalances
```
...
```bash
balance=$(./build/bin/bitcoin rpc -testnet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/bitcoin rpc -testnet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/bitcoin rpc -testnet -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./build/bin/bitcoin rpc -testnet decodepsbt $funded_psbt

./build/bin/bitcoin rpc -testnet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/bitcoin rpc -testnet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/bitcoin rpc -testnet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/bitcoin rpc -testnet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/bitcoin rpc -testnet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/bitcoin rpc -testnet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/bitcoin rpc -testnet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/bitcoin rpc -testnet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/bitcoin rpc -testnet finalizepsbt $psbt_2 | jq -r '.hex')
