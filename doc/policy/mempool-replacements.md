...
* Bitcoin Core lowered the incremental relay feerate default to 0.1sat/vB ([PR #33106](https://github.com/bitcoin/bitcoin/pull/33106));
  xCoin keeps it at 1sat/vB, equal to `-minrelaytxfee` (founder decision 2026-09-15; `DEFAULT_INCREMENTAL_RELAY_FEE`
  in `src/policy/policy.h`), because its ML-DSA transactions are about ten times the size of Bitcoin's.
