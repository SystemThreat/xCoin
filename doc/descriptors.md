...
- `pq(KEY)` (top level or inside `pqtr`): xCoin post-quantum single-key output. At the top level the legacy witness v2 output `OP_2 <SHA256(KEY)>` for an ML-DSA-65 key; inside `pqtr` the ML-DSA-65 leaf `<SHA256(KEY)> OP_CHECKSIG` (leaf version 0xc0). See `contrib/regenesis/REGENESIS.md` section 4.
- `slh(KEY)` (only inside `pqtr`): the SLH-DSA-SHA2-128s fallback leaf `<KEY> OP_CHECKSIG` (leaf version 0xc2).
- `pqtr(TREE)` (top level only): xCoin witness v3 output `OP_3 <root>`, the Merkle root of a tree of `pq`/`slh` leaves (same `{A,B}` tree syntax as `tr`, no internal key, at least one leaf). Inside a `pqtr` tree the only other leaf forms are `and_v(v:pq(KEY),after(N))`, `and_v(v:pq(KEY),older(N))` and the same two with `slh(KEY)`.
...
- Followed by the actual key, which is either:
  ...
  - Inside `pq()`: a 1,952-byte ML-DSA-65 public key (3904 hex characters), a 32-byte seed or a 4,032-byte expanded secret key (hex; private forms), or an xpub/xprv key expression as above. Inside `slh()`: a 32-byte SLH-DSA-SHA2-128s public key, a 48-byte seed or a 64-byte secret key (hex), or an xpub/xprv expression. A BIP32 expression derives the post-quantum key from the child *private* key at each position (`SHAKE-256(child_key || "xcoin/hd/ml-dsa-65/seed")`, resp. `"xcoin/hd/slh-dsa-sha2-128s/seed"`), so it only expands with the private key; the wallet caches the derived public keys. A wallet without the private key watches the same scripts through an explicit key list: `pq(keys(<pubkey>,<pubkey>,...))` or `slh(keys(<pubkey>,...))` (public keys only, separated by `,`), where the key at index *i* is the key at position *i*; such a descriptor is ranged with exactly as many positions as keys (one key included). `listdescriptors` (without `private`) writes every BIP32-derived post-quantum key out this way, from the public keys the wallet has derived, so its output can be imported with `importdescriptors` into a wallet with private keys disabled; the `private` output keeps the ranged xprv form.
