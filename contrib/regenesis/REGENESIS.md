...
|---|---|---|
| Ticker | XID (was XCF; before that XAT, retired). The currency symbol Ӿ stays. The forum identity handle stays `xid1…` | Owner decision 2026-09-25 |
| Supply cap | 100,000,000 XID exactly (was 21,000,000) | Owner decision 2026-09-25. The v1 genesis inscribed 21M, but nothing of v1 is carried into this chain (charter section 3), so the v1 figure does not bind it |
| Base unit | 1 XID = 10^8 base units (sat), unchanged; ledgers use a 1/1000 sub-unit off chain | 8 decimals, owner decision 2026-09-25; sub-unit is an L2 convention |
| Mainnet genesis date | November 1, 2026 (postponed from September 30, 2026) | Owner decision 2026-09-25: node-to-node connections get hybrid post-quantum encryption first (ML-KEM-768 layered on BIP324, "HX1", draft XIP-4). The date does not override the checklist in section 8 |
...
| Emission <!-- [EMISSION-SHAPE] --> | FINAL, the Annual Tenth (charter section 4): 6.25, 12.5 and 25 XID for 20,000 blocks each, 50 XID for blocks 60,001–220,000, then 110,000-block steps (~382 days) whose base falls by a tenth, rounded down to 0.01 XID and never under 1.5 XID, each block paying the lesser of its base and 1/1,100,000 of what is not yet scheduled; a 0.1 XID tail from block 34,320,001; 65 rows, the last subsidy block (35,375,353) also minting the 0.0166 XID remainder, 100,000,000 XID exactly. It lives in one file, `contrib/regenesis/emission-policy.json`; the generated rows have SHA-256 `356de0dc…0a230`, stated in the charter | Cap and shape decided 2026-09-25 (owner) |
| Emission after the first eras <!-- [EMISSION-SHAPE] --> | 1.5 XID/block from block 3,850,001 (~2063) until block 31,460,000 (~2326), when about 98.4 % is mined; then a tenth less per step, and 0.1 XID/block from block 34,320,001 (~2353) to the end (~2363). Fee rules under charter section 7 must be in force before block 31,460,001; fees carry security from there — market fees under relay policy; the settlement-levy machinery ships dormant at zero and can be switched on by soft fork (section 6) | Report deliverable 1 |
...
*Interval — 300 s, unchanged.* The emission table is DEFINED against it (a 110,000-block step IS about 382 days at 300 s), ASERT's 2 h half-life is tuned to it (24 blocks), and the rehearsal chain ran under it. 120 s would rewrite every era length and the per-block subsidy, re-tune ASERT, and raise the orphan rate on a chain running at ~2 MH/s, while buying only cosmetic latency: confirmation security is accumulated work, so 120 s blocks need 2.5x the confirmations for the same security. (One reason previously given for 300 s does not hold and should not be repeated: MetalDAG is NOT tuned to the block interval. Its epoch is time-based — `metaldagEpochSeconds`, ~14 d, `src/metaldag/metaldag.cpp` — so it is interval-independent.) <!-- [EMISSION-SHAPE] -->
...
`GetBlockSubsidy(h)`: block 0 mints nothing; from block 1 every block pays the subsidy of its row in `EMISSION_TABLE`; the final subsidy block also mints the closing remainder, so that sum(row subsidies) + remainder == 100,000,000 XID exactly. There is no carry and no distribution.

    sum(subsidies) + dust == 100,000,000 * 10^8 == MAX_MONEY exactly.

There is no premine and nothing is carried in. The cap and the shape are DECIDED (owner decision 2026-09-25): the table is generated by `contrib/regenesis/emission.py` from the policy file `contrib/regenesis/emission-policy.json` (the emission lab's `sim.py` schema) and written into `consensus/params.h` between the `BEGIN/END GENERATED EMISSION TABLE` markers. The final shape was installed that way: the policy file (status FINAL), `python3 contrib/regenesis/emission.py --write-params`, charter section 4 rewritten without its PROVISIONAL comment, `CHARTER_HASH` re-pinned and the unit test `annual_tenth_emission_shape` (which re-derives the table from section 4's words); the testnet A genesis still has to be re-mined. `emission.py --rows <file>` checks a canonical rows file byte for byte (`--rows-out` writes one). `emission.py --check-params` fails whenever the table, `MAX_SUPPLY_SAT` or `MAX_MONEY` disagree with the policy.

The final shape, the Annual Tenth: a ramp of 6.25, 12.5 and 25 XID for 20,000 blocks each, 50 XID to block 220,000, then each 110,000-block step pays the lesser of a base falling by a tenth (in whole hundredths of an XID, floor 1.5 XID) and 1/1,100,000 of the XID not yet scheduled; 0.1 XID per block once a step would pay less; 65 rows; the final subsidy block (35,375,353) also mints the 0.0166 XID remainder. <!-- [EMISSION-SHAPE] -->

Reference (genesis November 1, 2026, 300 s): the ramp mints 875,000 XID (~2027.4); 8.875 % by block 220,000 (~2028.9); half the cap at block ~2,071,500 (~2046.5); 56.95 % when the 1.5 XID floor starts (block 3,850,001, ~2063); 98.37 % by block 31,460,000 (~2326); 99.89 % when the 0.1 XID tail starts (~2353); all of it at block 35,375,353 (~2363). <!-- [EMISSION-SHAPE] -->
...
Consensus checks: `bad-cb-amount` uses the table; `EMISSION_END_HEIGHT` is the last block of the last row; `MAX_SUPPLY_SAT` is 10^16 sat (100,000,000 XID), equal to `MAX_MONEY` by a compile-time assertion, and unit tests prove the table sums to it (`emission_cap_is_100m_xid`, `emission_sums_to_cap_mainnet`).
...
Output-value floor: on mainnet and the rehearsal chain every output that is not `OP_RETURN` carries at least `Consensus::MIN_OUTPUT_VALUE_SAT` = 10,000 sat (`bad-txout-below-min`, `consensus/tx_check.cpp`; regtest sets `minOutputValueSat` to 0 so the inherited fixtures still build sub-floor outputs). A PQ output costs ~5.4 kB of witness to spend, so anything smaller can never be spent for less than it is worth and exists only to bloat the UTXO set. The genesis coinbase is exempt (its marker output is zero-value). Coinbase exemption (founder decision 2026-09-15): a coinbase whose non-`OP_RETURN` outputs number exactly one may pay that output any value from 1 sat, because late in the schedule the subsidy is under the floor and an otherwise empty block could not have claimed it (the final shape never is: its smallest row pays 0.1 XID, so the exemption is a safeguard for any table); every row's subsidy plus the closing remainder is mintable with zero fees. A coinbase with two or more spendable outputs is held to the floor on every one of them, so the reward cannot be sprayed into the UTXO set as sub-floor outputs. The block assembler and the pool (`build_coinbase`) both build one spendable output plus the witness-commitment `OP_RETURN`, so the exemption applies to what they mine. <!-- [EMISSION-SHAPE] -->
...
The planned genesis date is November 1, 2026 (owner decision 2026-09-25). It was September 30, 2026; it moved so that node-to-node connections get hybrid post-quantum encryption first (ML-KEM-768 layered on BIP324, "HX1", draft XIP-4). The date is a plan, not a gate: genesis still waits for every step below.

0. Before step 3, the final emission shape is in the tree (done 2026-09-25: the Annual Tenth). The tree enforces this: `xcoin-genesis` refuses `-chain=main` while charter section 4 carries its PROVISIONAL comment or `Consensus::EMISSION_SHAPE_IS_FINAL` is false, and `GENESIS_IS_FINAL = true` does not compile until the shape is final (except in a local `-DXCOIN_REHEARSAL_BUILD=ON` build, REHEARSAL-2.md). To install it: replace `contrib/regenesis/emission-policy.json` with the chosen policy and set its `"status"` to start with `FINAL`; run `emission.py --write-params` (this sets `EMISSION_SHAPE_IS_FINAL = true`); rewrite charter section 4 for that shape and remove its PROVISIONAL comment; re-pin `CHARTER_HASH` and the charter tests; rewrite the shape unit test (now `annual_tenth_emission_shape`); then work through every entry in `contrib/regenesis/EMISSION-SHAPE-SPOTS.md`, which lists each place that states the shape and the grep tag each carries (a search for the word "provisional" misses some of them). Re-mine testnet A against the final charter. Green: `emission.py --check-params`, `emission.py --check-spots` (both run in ctest) and `test_bitcoin`.
