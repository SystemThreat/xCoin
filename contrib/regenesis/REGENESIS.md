...
- The v2 genesis coinbase scriptSig carries `"Hic experimentum prosperat II - <date> - 21,000,000 XCF - charter " + hex(CHARTER_HASH)[0:16]` (`charter::GenesisMessage`). Only the first 16 hex characters fit: a coinbase scriptSig is capped at 100 bytes by the unchanged `bad-cb-length` rule, which `CheckBlock` applies to the genesis too, and the whole digest is committed in the output below. The genesis coinbase mints nothing; that output is its only output.
