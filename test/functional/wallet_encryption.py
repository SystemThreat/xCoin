...
class WalletEncryptionTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Upstream probes "can the wallet use its keys" by signing a message with a
        # legacy P2PKH address. This chain has no legacy address type (the wallet
        # creates post-quantum descriptors only) and message signing covers nothing
        # else, so the probe is a spend instead: it needs the ML-DSA-65 private key
        # behind a pqtr address, which only an unlocked wallet can decrypt. Mine a
        # balance first; self.generate pays this node's deterministic pqtr coinbase
        # descriptor, which init_wallet imported into the wallet under test.
        node = self.nodes[0]
        self.generate(node, 110)
        address = node.getnewaddress()
        assert_equal(node.getaddressinfo(address)["ismine"], True)
        LOCKED = "Please enter the wallet passphrase with walletpassphrase first"

        def spend():
            """Spend a coin to our own address: a post-quantum signature only an
            unlocked wallet can produce."""
        ...
        assert_raises_rpc_error(-13, LOCKED, spend)
