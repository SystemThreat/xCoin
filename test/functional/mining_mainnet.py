...
class MiningMainnetTest(BitcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        self.skip_on_xcoin(
            "starts a node on mainnet, which refuses to run while GENESIS_IS_FINAL is false (cutover checklist step 3)")
    ...
    def mine(self, height, prev_hash, blocks, node):
        ...
        block = CBlock()
        # xCoin: BIP9 top bits 01 in 31..30, PoW-algorithm id 0 in bits 28..29
        # (REGENESIS.md section 7). Bitcoin's 0x20000000 carries algorithm id 2
        # here and is rejected with "bad-pow-algo".
        block.nVersion = 0x40000000
