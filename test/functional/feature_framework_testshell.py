...
def run_testshell_doc_example(functional_tests_dir):
    ...
    try:
        ...
        # A clean chain starts at height 1: the framework mines block 1, the
        # genesis distribution block (REGENESIS.md section 3).
        assert_equal(test.nodes[0].getblockchaininfo()["blocks"], 1)
        if test.is_wallet_compiled():
            ...
            test.sync_blocks()
            assert_equal(test.nodes[1].getblockchaininfo()["blocks"], 102)
