...
class FeatureFrameworkMiniWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        node = self.nodes[0]
        # Only ADDRESS_OP_TRUE is testable on this chain: the PQ-only output
        # rule (bad-txout-not-pq, REGENESIS.md section 4) admits witness v3,
        # OP_RETURN and (on regtest) witness v2, so the bare OP_TRUE output of
        # RAW_OP_TRUE and the P2PK output of RAW_P2PK cannot be mined or relayed
        # at all. Both modes are kept in MiniWallet for tests that want a
        # rejected transaction; there is nothing here for them to send.
        self.wallets = [
