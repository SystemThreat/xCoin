...
MAX_TIMEWARP = 600
# xCoin (contrib/regenesis/REGENESIS.md section 7): the BIP9 top bits are 01 in
# bits 31..30, not Bitcoin's 001 in bits 31..29, because bits 28..29 are
# reserved as the PoW-algorithm id; TESTDUMMY moved down to bit 27 to make room.
VERSIONBITS_TOP_BITS = 0x40000000
VERSIONBITS_TOP_MASK = 0xC0000000
POW_ALGO_ID_MASK = 0x30000000
VERSIONBITS_DEPLOYMENT_TESTDUMMY_BIT = 27
DEFAULT_BLOCK_MIN_TX_FEE = 1000 # default `-blockmintxfee` setting [sat/kvB], the relay floor (src/policy/policy.h)
...
class MiningTest(BitcoinTestFramework):
    ...
    def mine_chain(self):
        ...
        self.connect_nodes(0, 1)
        template_version = self.nodes[0].getblocktemplate(NORMAL_GBT_REQUEST_PARAMS)['version']
        assert_equal(VERSIONBITS_TOP_BITS + (1 << VERSIONBITS_DEPLOYMENT_TESTDUMMY_BIT), template_version)
        # The template must carry PoW-algorithm id 0 (MetalDAG) in bits 28..29:
        # anything else is "bad-pow-algo" until multi_algo_activation_height.
        assert_equal(template_version & POW_ALGO_ID_MASK, 0)
        assert_equal(template_version & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS)
        self.restart_node(0)
        self.connect_nodes(0, 1)
    ...
    def test_block_max_weight(self):
        ...
        # xCoin: the assembler's default -blockmaxweight is DEFAULT_BLOCK_MAX_WEIGHT
        # (4,000,000 WU, policy.h), not the 64,000,000 WU consensus MAX_BLOCK_WEIGHT,
        # so the template-filling checks below are sized on the default; the two
        # startup refusals at the end still probe the consensus limit.
        LARGE_TXS_COUNT = 10
        LARGE_VSIZE = int(((DEFAULT_BLOCK_MAX_WEIGHT - DEFAULT_BLOCK_RESERVED_WEIGHT) / WITNESS_SCALE_FACTOR) / LARGE_TXS_COUNT)
        ...
        self.verify_block_template(
            ...
            expected_weight=DEFAULT_BLOCK_MAX_WEIGHT - DEFAULT_BLOCK_RESERVED_WEIGHT,
        )

        # Test block template creation with custom -blockmaxweight
        custom_block_weight = DEFAULT_BLOCK_MAX_WEIGHT - 2000
        ...
        self.verify_block_template(
            ...
            expected_weight=DEFAULT_BLOCK_MAX_WEIGHT - DEFAULT_BLOCK_RESERVED_WEIGHT - 2000,
        )

        # Ensure the block weight does not exceed the maximum
        self.log.info(f"Testing that the block weight will never exceed {DEFAULT_BLOCK_MAX_WEIGHT - DEFAULT_BLOCK_RESERVED_WEIGHT}.")
        self.restart_node(0, extra_args=[f"-blockmaxweight={DEFAULT_BLOCK_MAX_WEIGHT}"])
        ...
        self.log.info(f"Testing that the mempool's weight matches the maximum block weight: {DEFAULT_BLOCK_MAX_WEIGHT}.")
        assert_equal(self.nodes[0].getmempoolinfo()['bytes'] * WITNESS_SCALE_FACTOR, DEFAULT_BLOCK_MAX_WEIGHT)
        ...
        self.verify_block_template(
            ...
            expected_weight=DEFAULT_BLOCK_MAX_WEIGHT - 4000,
        )
    ...
    def run_test(self):
        ...
        self.generatetoaddress(node, 1, node.get_deterministic_coinbase_address())
        tmpl = node.getblocktemplate(NORMAL_GBT_REQUEST_PARAMS)
        ...
        self.generatetoaddress(node, 10, node.get_deterministic_coinbase_address())
        assert_raises_rpc_error(-25, 'time-too-old', lambda: node.submitheader(hexdata=CBlockHeader(bad_block_time).serialize().hex()))
        assert_raises_rpc_error(-25, 'bad-prevblk', lambda: node.submitheader(hexdata=CBlockHeader(bad_block2).serialize().hex()))
