...
from test_framework.address import ADDRESS_UNSPENDABLE
...
class NotificationsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        block_count = 10
        blocks = self.generatetoaddress(self.nodes[1], block_count, self.nodes[1].getnewaddress() if self.is_wallet_compiled() else ADDRESS_UNSPENDABLE)
        ...
        if self.is_wallet_compiled():
            ...
            self.generatetoaddress(self.nodes[0], 100, ADDRESS_UNSPENDABLE)

            # Generate transaction on node 0, sync mempools, and check for
            # notification on node 1.
            tx1 = self.nodes[0].sendtoaddress(address=ADDRESS_UNSPENDABLE, amount=1, replaceable=True)
            ...
            blockhash1 = self.generatetoaddress(self.nodes[0], 1, ADDRESS_UNSPENDABLE)[0]
            ...
            tx2 = self.nodes[0].sendtoaddress(address=ADDRESS_UNSPENDABLE, amount=1, replaceable=True)
            ...
            blockhash2 = self.generatetoaddress(self.nodes[0], 1, ADDRESS_UNSPENDABLE, sync_fun=self.no_op)[0]
