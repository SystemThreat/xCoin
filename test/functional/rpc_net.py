...
class NetTest(BitcoinTestFramework):
    ...
    def test_sendmsgtopeer(self):
        ...
        self.log.debug("Test that a large message is delivered and keeps the peer connected")
        # xCoin: the receiving node disconnects a v1 peer only above MAX_SIZE
        # (32 MiB; MAX_PROTOCOL_MESSAGE_LENGTH is 64 MB) and a BIP324 v2 peer
        # cannot frame more than 16,777,215 bytes, so the upstream 4,000,001-byte
        # probe is an ordinary message here. A message past the v1 limit would be
        # 64 MiB of hex, over the RPC server's own MAX_SIZE body cap, so the
        # disconnect cannot be provoked through this RPC at all; check instead
        # that the largest message the RPC body cap admits is still delivered and
        # keeps the peer connected.
        large_message = b'\x00' * (test_framework.messages.MAX_SIZE // 4)
        with self.nodes[1].assert_debug_log(expected_msgs=[f"received: addr ({len(large_message)} bytes)"], timeout=10):
            node.sendmsgtopeer(peer_id=0, msg_type="addr", msg=large_message.hex())
