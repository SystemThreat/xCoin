...
By default, the `TestShell` nodes are initialized with a clean chain, and the
framework then mines block 1 on it, the genesis distribution block: on this
chain block 1 pays the carried balances and no subsidy to its miner
(contrib/regenesis/REGENESIS.md section 3), so it is part of the chain rather
than something a test earns. Each node therefore starts at a block height of 1.
