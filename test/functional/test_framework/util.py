...
def get_binary_paths(config):
    ...
    # xCoin builds name the executables after the project (nexd, nex-cli, ...);
    # the upstream names are tried first so an unrenamed tree keeps working.
    xcoin_names = {
        "bitcoin": "nex",
        "bitcoind": "nexd",
        "bitcoin-cli": "nex-cli",
        "bitcoin-tx": "nex-tx",
        "bitcoin-wallet": "nex-wallet",
    }
    ...
    for binary, env_variable_name in binaries.items():
        ...
        if not os.path.exists(default_filename) and binary in xcoin_names:
            xcoin_filename = os.path.join(
                config["environment"]["BUILDDIR"],
                "bin",
                xcoin_names[binary] + config["environment"]["EXEEXT"],
            )
            if os.path.exists(xcoin_filename):
                default_filename = xcoin_filename
...
# The node reads its configuration from this file in the datadir (BITCOIN_CONF_FILENAME
# in src/common/args.cpp is "nex.conf" on xCoin, not "bitcoin.conf"); every node under
# test must pick it up, or it would start with default (mainnet) parameters and ports.
CONF_FILENAME = "nex.conf"
...
def write_config(config_path, *, n, chain, extra_config="", disable_autoconnect=True):
    # Translate chain subdirectory name to config name (xCoin testnet A lives
    # in <datadir>/testneta, is selected with -testnet and configured in [test]).
    if chain == 'testneta':
    ...
    with open(config_path, 'w') as f:
        ...
        if chain_name_conf_section == 'regtest':
            # xCoin settlement levy (REGENESIS.md section 6): off on every chain
            # at genesis, so nothing to set here. The dormant machinery is
            # covered by xcoin_levy_tests and by xcoin_v3_wallet.py, which
            # enable it with -levybp=5 on the command line.
            # Coinbase maturity is 1,000 on every chain, regtest included. The
            # inherited tests mine COINBASE_MATURITY (100) blocks to a spendable
            # coinbase, so the framework runs regtest at 100; a test that wants
            # the real value passes -coinbasematurity=1000 on the command line.
            f.write("coinbasematurity=100\n")
