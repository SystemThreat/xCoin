...
# xCoin: the PQ-only output rule (bad-txout-not-pq, REGENESIS.md section 4)
# refuses every Bitcoin output type, so the framework's two fixed regtest
# destinations are witness v3. UNSPENDABLE is the all-zero program: no leaf
# hashes to zero, so nothing can ever spend it. ANYONECANSPEND is the single
# 0xc0 leaf over OP_TRUE (test_framework/xcoin.py), spent with a witness stack
# of [leaf script, control block] and no signature.
ADDRESS_UNSPENDABLE = 'nxrt1rqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqn3x836'
ADDRESS_UNSPENDABLE_DESCRIPTOR = 'addr(nxrt1rqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqn3x836)#686ggqy4'
ADDRESS_ANYONECANSPEND = 'nxrt1rydw50gpwa6yfknz6kkqu2jtvtlxkpmf8fltuv67tgk02mw38wysspgwx6w'
...
# xCoin post-quantum addresses (contrib/regenesis/REGENESIS.md section 4): bech32m with the
# chain's own HRP (xpa mainnet, nxrt regtest); witness version 2 = OP_2 <SHA256(ML-DSA-65 pubkey)>
# (xpa1z.../nxrt1z...), witness version 3 = OP_3 <script-tree root> (xpa1r.../nxrt1r...).
XCOIN_HRP_MAIN = "xpa"
XCOIN_HRP_REGTEST = "nxrt"

def xcoin_program_to_witness(version, program, main=False):
    if (type(program) is str):
        program = bytes.fromhex(program)
    assert version in (2, 3)
    assert len(program) == 32
    return encode_segwit_address(XCOIN_HRP_MAIN if main else XCOIN_HRP_REGTEST, version, program)

def xcoin_bech32_to_bytes(address):
    """(witness version, program) of an xCoin bech32m address, or (None, None)."""
    hrp = address.split('1')[0]
    if hrp not in [XCOIN_HRP_MAIN, XCOIN_HRP_REGTEST, 'txa']:
        return (None, None)
...
def address_to_scriptpubkey(address):
    ...
    version, payload = xcoin_bech32_to_bytes(address)
    if version is not None:
        return program_to_witness_script(version, payload)  # xCoin witness v2/v3
