...
# Regtest BIP32 extended private key prefix (base58Prefixes[EXT_SECRET_KEY] in
# src/kernel/chainparams.cpp), i.e. the usual "tprv".
EXT_SECRET_KEY_PREFIX_REGTEST = bytes.fromhex("04358394")


def byte_string_to_base58check(payload):
    """base58check of an already-versioned byte string (BIP32 keys carry a
    four-byte version, which byte_to_base58 in address.py cannot express)."""
    b = payload + hash256(payload)[:4]
...
def deterministic_coinbase_xprv(index):
    """A BIP32 master private key that depends only on the node index.

    Built here rather than derived, so it needs no BIP32 arithmetic in Python:
    depth 0, no parent, chain code and key both a SHA-256 of the index."""
    key = hashlib.sha256(f"xcoin/test/node-coinbase/{index}/key".encode()).digest()
    chaincode = hashlib.sha256(f"xcoin/test/node-coinbase/{index}/chaincode".encode()).digest()
    # secp256k1 group order; a SHA-256 above it would not be a valid key.
    assert 0 < int.from_bytes(key, "big") < 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141
    body = EXT_SECRET_KEY_PREFIX_REGTEST + b"\x00" + b"\x00" * 4 + b"\x00" * 4 + chaincode + b"\x00" + key
    assert len(body) == 78
    return byte_string_to_base58check(body)


def deterministic_coinbase_descriptor(index):
    """The coinbase destination of the test node with this index, as a private
    descriptor.

    The xCoin counterpart of TestNode.PRIV_KEYS: one key per node index, mined
    to by TestNode.generate and imported into that node's wallet by
    BitcoinTestFramework.init_wallet, so a generate() funds the wallet the way
    it does upstream. The PQ-only output rule (bad-txout-not-pq, REGENESIS.md
    section 4) refuses a P2PKH output, so this is the single-leaf witness v3
    tree over an ML-DSA-65 key.

    The key is written as a BIP32 expression, not as a literal ML-DSA seed, and
    that matters: the wallet's descriptor manager only stores secp256k1 private
    keys, so a literal pq(<seed>) descriptor imports and then cannot sign (the
    import RPC refuses it for that reason). Under pq(xprv/0/0) the ML-DSA key is
    derived from the child secp256k1 key (src/pqhd.h), which the wallet does
    store, so the imported coins are spendable.
    """
    return descsum_create(f"pqtr(pq({deterministic_coinbase_xprv(index)}/0/0))")
...
class TestNode():
    ...
    def __init__(
        ...
        *,
        ...
        self.bitcoinconf = self.datadir_path / CONF_FILENAME
    ...
    def get_deterministic_coinbase_descriptor(self):
        """This node's deterministic coinbase destination, as a descriptor.

        The Bitcoin analogue is get_deterministic_priv_key(): a key that depends
        only on the node's index, which the framework both mines to and imports
        into the node's wallet, so `generate` funds that wallet. It cannot be a
        P2PKH key here, because the PQ-only output rule (bad-txout-not-pq,
        REGENESIS.md section 4) refuses every Bitcoin output type, so it is the
        single-leaf witness v3 tree over an ML-DSA-65 key."""
        return deterministic_coinbase_descriptor(self.index)

    def get_deterministic_coinbase_address(self):
        """The address of the descriptor above. Needs a running node, because
        deriving an ML-DSA-65 public key from its seed is the node's job."""
        return self.deriveaddresses(self.get_deterministic_coinbase_descriptor())[0]

    def generate(self, nblocks, maxtries=1000000, **kwargs):
        self.log.debug("TestNode.generate() dispatches `generate` call to `generatetodescriptor`")
        return self.generatetodescriptor(nblocks, self.get_deterministic_coinbase_descriptor(), maxtries, **kwargs)
