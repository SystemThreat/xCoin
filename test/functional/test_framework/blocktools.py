...
from .xcoin import xcoin_v3_op_true
...
# xCoin (REGENESIS.md sections 2, 3 and 4). Two chain rules the inherited
# helpers below have to obey, both enforced on regtest too:
#
# 1. PQ-only outputs (bad-txout-not-pq): an output may only be witness v3,
#    OP_RETURN, or - where the chain permits it, which regtest does - witness
#    v2. Every Bitcoin output type is consensus-invalid, so the framework's
...
def get_block_subsidy(height, halving_period=REGTEST_RETARGET_PERIOD, base=50):
    ...
    regtest table: three halvings on 150-block eras, then the fourth rate until
    the emission end height, after which the subsidy is zero)."""
    if height < 1 or height > REGTEST_EMISSION_END_HEIGHT:
        return 0
    era = min((height - 1) // halving_period, REGTEST_EMISSION_ERAS - 1)
    return (base * COIN) >> era
...
def create_coinbase(height, pubkey=None, *, script_pubkey=None, extra_output_script=None, fees=0, nValue=50, halving_period=REGTEST_RETARGET_PERIOD):
    ...
    otherwise the anyone-can-spend witness v3 output. Note that a P2PK output
    is consensus-invalid on every chain of this fork (bad-txout-not-pq), so
    `pubkey` is only useful for a test that wants a rejected block.
    ...
    if nValue == 50:
        coinbaseoutput.nValue = get_block_subsidy(height, halving_period)
    ...
    if pubkey is not None:
        ...
        coinbaseoutput.scriptPubKey = ANYONECANSPEND_V3_SCRIPT
...
def create_tx_with_script(prevtx, n, script_sig=b"", *, amount, output_script=None):
    ...
    if output_script is None:
        output_script = ANYONECANSPEND_V3_SCRIPT
    tx = CTransaction()
