...
MAX_LOCATOR_SZ = 101
MAX_BLOCK_WEIGHT = 64000000  # xCoin: follows MAX_BLOCK_WEIGHT in src/consensus/consensus.h (REGENESIS.md section 0)
MAX_BLOCK_SERIALIZED_SIZE = 64000000  # follows MAX_BLOCK_SERIALIZED_SIZE in src/consensus/consensus.h
MAX_SIZE = 0x02000000  # the serialization hard cap (src/serialize.h): what a v1 transport message can actually carry
DEFAULT_BLOCK_MAX_WEIGHT = 4000000  # xCoin: the assembler's default -blockmaxweight (src/policy/policy.h), deliberately below MAX_BLOCK_WEIGHT
DEFAULT_BLOCK_RESERVED_WEIGHT = 8000
MINIMUM_BLOCK_RESERVED_WEIGHT = 2000
MAX_BLOOM_FILTER_SIZE = 36000
MAX_BLOOM_HASH_FUNCS = 50

COIN = 100000000  # 1 XID in satoshis
MAX_MONEY = 100000000 * COIN  # 100,000,000 XID (consensus/amount.h)

MAX_BIP125_RBF_SEQUENCE = 0xfffffffd  # Sequence number that is rbf-opt-in (BIP 125) and csv-opt-out (BIP 68)
MAX_SEQUENCE_NONFINAL = 0xfffffffe  # Sequence number that is csv-opt-out (BIP 68)
SEQUENCE_FINAL = 0xffffffff  # Sequence number that disables nLockTime if set for every input of a tx

MAX_PROTOCOL_MESSAGE_LENGTH = 64000000  # Maximum length of incoming protocol messages (follows src/net.h; the v1 transport refuses anything over MAX_SIZE first)
...
# Default setting for -datacarriersize (xCoin v2: 80 bytes, REGENESIS.md section 5).
MAX_OP_RETURN_RELAY = 80
...
# Per-network P2P magic, as set in src/kernel/chainparams.cpp. These are this
# chain's, not Bitcoin's: mainnet v2 is 'X','P','A',0x03 (REGENESIS.md section 8),
# the private rehearsal chain is 'X','T','A',0x02 (TESTNET-A.md), and regtest
# keeps the first chain's NEX prefix.
MAGIC_BYTES = {
    "mainnet": b"\x58\x50\x41\x03",   # 'X','P','A',0x03
    "testnet": b"\x58\x54\x41\x02",   # 'X','T','A',0x02 (the rehearsal chain)
    "regtest": b"\x4e\x45\x58\x03",   # 'N','E','X',0x03
}
...
class CBlockHeader:
    ...
    def pow_int(self):
        """The value the node compares against nBits: the MetalDAG hash through the
        registered oracle, else the SHA-256d identity hash."""
        if POW_ORACLE is None:
            return self.hash_int
        return int(POW_ORACLE(self._serialize_header().hex()), 16)

    def pow_valid(self):
        return self.pow_int() <= uint256_from_compact(self.nBits)

    def solve(self):
        """Find a nonce whose proof of work meets nBits (on regtest about one nonce in two)."""
        while not self.pow_valid():
            self.nNonce += 1

    def unsolve(self):
        """Find a nonce whose proof of work fails nBits, for tests that need an invalid header."""
        while self.pow_valid():
            self.nNonce += 1
...
# xCoin: the proof of work is MetalDAG, which only the node computes cheaply. The test
# framework registers the node's getmetaldagpowhash RPC here (set_pow_oracle) so that
# CBlock.solve() / is_valid() grind the real proof of work instead of the SHA-256d
# identity hash (audit finding 15: every Python-built block was invalid with probability
# one half). Without an oracle the SHA-256d hash is used, which is right only for the
# framework's own unit tests.
POW_ORACLE = None


def set_pow_oracle(fn):
    """fn(header_hex) -> big-endian hex of the MetalDAG proof-of-work value."""
    global POW_ORACLE
    POW_ORACLE = fn
