...
from .test_node import (
    TestNode,
    deterministic_coinbase_descriptor,
)
from .util import (
    Binaries,
    MAX_NODES,
...
from .xcoin import create_deterministic_address_xcoin_v3_op_true
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def init_wallet(self, *, node):
        ...
        if wallet_name is not False:
            ...
            # The node's deterministic coinbase key, so `self.generate(node, n)`
            # funds this wallet as it does upstream. It is an ML-DSA-65 key
            # under a witness v3 tree, not a P2PKH privkey (test_node.py,
            # deterministic_coinbase_descriptor).
            import_res = n.get_wallet_rpc(wallet_name).importdescriptors([{
                "desc": n.get_deterministic_coinbase_descriptor(),
                "timestamp": 0,
                "label": "coinbase",
            }])
    ...
    def _register_pow_oracle(self):
        """xCoin: let CBlock.solve()/is_valid() ask a running node for the MetalDAG
        proof-of-work value (audit finding 15). Tries every running node, so a test
        that stops node 0 can still build blocks."""
        def oracle(header_hex):
            last_error = None
            for node in self.nodes:
                if not node.running:
                    continue
                try:
                    return node.getmetaldagpowhash(header_hex)
                except Exception as e:  # noqa: BLE001 - try the next node
                    last_error = e
            raise RuntimeError(f"no running node can compute the MetalDAG proof of work: {last_error}")
        from test_framework import messages
        messages.set_pow_oracle(oracle)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            # The addresses are witness v3, because the PQ-only output rule
            # (bad-txout-not-pq, REGENESIS.md section 4) refuses every Bitcoin
            # output type, so the P2PKH addresses of TestNode.PRIV_KEYS cannot be
            # mined to at all. The first three are tagged, i.e. nobody's; the
            # fourth is MiniWallet's own anyone-can-spend address, as upstream.
            # Mined to DESCRIPTORS, not addresses: the first three are the
            # deterministic coinbase keys of nodes 0-2 (which init_wallet
            # imports, so those wallets start with 25 mature coinbases as they
            # do upstream) and the fourth is MiniWallet's own anyone-can-spend
            # witness v3 script. A P2PKH address cannot be mined to at all here
            # (bad-txout-not-pq, REGENESIS.md section 4), and an ML-DSA-65
            # address cannot be derived without the node, so the descriptor is
            # the thing the framework can name offline.
            gen_descriptors = [deterministic_coinbase_descriptor(i) for i in range(3)]
            gen_descriptors.append(descsum_create(
                f'raw({create_deterministic_address_xcoin_v3_op_true()[1].scriptPubKey.hex()})'))
            assert_equal(len(gen_descriptors), 4)
            for i in range(8):
                self.generatetodescriptor(
                    cache_node,
                    25 if i != 7 else 24,
                    gen_descriptors[i % len(gen_descriptors)],
                )
    ...
    def skip_on_xcoin(self, reason):
        """Skip a test inherited from Bitcoin Core whose premise this chain does
        not have: ECDSA/Schnorr signatures, P2PKH / P2SH / P2WPKH / P2TR outputs
        (the PQ-only output rule refuses all of them, REGENESIS.md section 4),
        Bitcoin's emission, or a value pinned to Bitcoin's genesis. `reason` is
        one line saying which. The test is not deleted and not weakened: it is
        recorded as not applicable, and REGENESIS.md section 9 lists them."""
        raise SkipTest(f"xCoin: {reason}")
