...
from test_framework.xcoin import create_deterministic_address_xcoin_v3_op_true
...
class MiniWalletMode(Enum):
    ...
    it uses a fixed bech32m witness v3 address (REGENESIS.md section 4) whose
    single 0xc0 leaf is OP_TRUE, so its coins are spent with a witness stack of
    [leaf script, control block] and no signature, i.e. following an
    anyone-can-spend policy.
    However, if the transactions need to be modified by the user (e.g. prepending
    scriptSig for testing opcodes that are activated by a soft-fork), or the txs
    should contain an actual signature, the raw modes RAW_OP_TRUE and RAW_P2PK
    can be useful. BOTH RAW MODES PAY AN OUTPUT THIS CHAIN REFUSES: the PQ-only
    output rule (bad-txout-not-pq) admits only witness v3, OP_RETURN and (on
    regtest) witness v2, so a bare OP_TRUE or a P2PK output is consensus-invalid
    and a test that needs one has nothing to test here. They are kept only so
    such a test can still construct the rejected transaction.
    In order to avoid mixing of UTXOs between different MiniWallet
...
class MiniWallet:
    def __init__(self, test_node, *, mode=MiniWalletMode.ADDRESS_OP_TRUE, tag_name=None):
        ...
        if mode == MiniWalletMode.RAW_OP_TRUE:
            ...
            self._address, self._xcoin_v3_info = create_deterministic_address_xcoin_v3_op_true(tag_name)
    ...
    def sign_tx(self, tx, fixed_length=True):
        if self._mode == MiniWalletMode.RAW_P2PK:
            ...
            for i in tx.wit.vtxinwit:
                assert_equal(len(self._xcoin_v3_info.leaves), 1)
                i.scriptWitness.stack = self._xcoin_v3_info.witness_stack()
        else:
            assert False
    ...
    def _spend_vsize(self):
        """The virtual size of a 1-in-1-out self transfer, measured rather than
        pinned: a tagged wallet's witness v3 leaf script is longer than the
        untagged OP_TRUE one, so the two do not have the same size. The untagged
        wallet measures 104, the same as the p2tr-OP_TRUE spend upstream."""
        if self._measured_vsize is None:
            dummy = CTransaction()
            dummy.vin = [CTxIn(COutPoint(0, 0))]
            dummy.vout = [CTxOut(0, bytearray(self._scriptPubKey))]
            self.sign_tx(dummy)
            self._measured_vsize = Decimal(dummy.get_vsize())
        return self._measured_vsize
    ...
    def create_self_transfer(
        ...
        if self._mode == MiniWalletMode.ADDRESS_OP_TRUE:
            vsize = self._spend_vsize()  # anyone-can-spend witness v3, 104 for the untagged leaf
        elif self._mode == MiniWalletMode.RAW_OP_TRUE:
