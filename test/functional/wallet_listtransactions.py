...
class ListTransactionsTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        # -permitv2outputs=1: this test hands out and pays an xcoin-v2 address (below),
        # which the wallet does only where the node relays new v2 outputs (audit finding T9).
        self.extra_args = [["-walletrbf=0", "-permitv2outputs=1"]] * self.num_nodes
    ...
    def run_externally_generated_address_test(self):
        ...
        # xCoin: 50, not upstream's 1,000. Every witness v3 address costs an ML-DSA-65 and an
        # SLH-DSA key generation (~60 ms each for the external and the internal chain), so
        # 1,000 would run past the 30 s RPC timeout; the test hands out three addresses.
        self.nodes[0].keypoolrefill(50)
        self.stop_nodes()
        ...
        # xCoin: the wallet has no legacy, p2sh-segwit or bech32 addresses (the chain
        # refuses those outputs); the two post-quantum types stand in for them.
        addr1 = self.nodes[0].getnewaddress("pizza1", 'xcoin-v3')
        addr2 = self.nodes[0].getnewaddress("pizza2", 'xcoin-v2')
        addr3 = self.nodes[0].getnewaddress("pizza3", 'xcoin-v3')
