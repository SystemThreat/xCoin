...
class EstimateFeeTest(BitcoinTestFramework):
    ...
    def test_sub_1s_per_vb_estimates(self):
        # xCoin's default relay floor is 1 sat/vB (DEFAULT_MIN_RELAY_TX_FEE), so a 0.5 sat/vB
        # transaction is neither relayed nor mined by a node on the defaults; the estimator's
        # lowest bucket is still 0.1 sat/vB, so run this case on nodes told to relay and mine
        # down to that bucket.
        sub_floor = [f"-{opt}={MIN_BUCKET_FEERATE:.8f}" for opt in ("minrelaytxfee", "incrementalrelayfee", "blockmintxfee")]
        for i, args in enumerate(self.extra_args):
            self.restart_node(i, extra_args=args + sub_floor)
