...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def test_option_feerate(self):
        ...
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (sat/vB) and feeRate (XID/kvB)",
    ...
    def test_transaction_too_large(self):
        ...
        outputs = {}
        rawtx = recipient.createrawtransaction([], {wallet.getnewaddress(): 9.79899260})

        # Make 100 0.1 XID outputs. The amount that we target for funding is in
        # the BnB range when these outputs are used.  However if these outputs
        # are selected, the transaction will end up being too large, so it
        # shouldn't use BnB and instead fall back to Knapsack but that behavior
        # is not implemented yet. For now we just check that we get an error.
        # xCoin: 100 outputs, not upstream's 1,500. A post-quantum input carries
        # ~5.4 kB of witness, so some 70 of them already exceed the 400,000 WU
        # standard transaction weight, and every witness v3 address costs an
        # ML-DSA-65 and an SLH-DSA key generation (~60 ms per chain): a 1,500-key
        # refill would run past the 30 s RPC timeout.
        # First, force the wallet to bulk-generate the addresses we'll need.
        recipient.keypoolrefill(100)
        for _ in range(100):
