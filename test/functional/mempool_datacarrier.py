...
class DataCarrierTest(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 4
        self.extra_args = [
            [], # default: MAX_OP_RETURN_RELAY (80 bytes on xCoin)
    ...
    def run_test(self):
        ...
        # By default (xCoin v2, REGENESIS.md section 5) the OP_RETURN scriptPubKey may be
        # 80 bytes: 77 bytes of data (+1 for OP_RETURN, +2 for the pushdata opcodes).
        default_size_data = randbytes(MAX_OP_RETURN_RELAY - 3)
        too_long_default_data = randbytes(MAX_OP_RETURN_RELAY - 2)
        ...
        self.log.info("Testing a null data transaction with a size equal to the default -datacarriersize (80 bytes).")
        self.test_null_data_transaction(node=self.nodes[0], data=default_size_data, success=True)

        self.log.info("Testing a null data transaction one byte over the default -datacarriersize (81 bytes).")
        self.test_null_data_transaction(node=self.nodes[0], data=too_long_default_data, success=False)
