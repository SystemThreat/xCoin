...
# xCoin: MAX_PROTOCOL_MESSAGE_LENGTH is 64 MB, but the v1 transport refuses any
# message over MAX_SIZE (32 MiB) first (net.cpp: nMessageSize > MAX_SIZE ||
# nMessageSize > MAX_PROTOCOL_MESSAGE_LENGTH), so the largest deliverable v1
# message is MAX_SIZE bytes. The BIP324 v2 transport cannot even frame more
# than 16,777,215 bytes, so it never reaches either limit (see test_size).
MAX_V1_MESSAGE_SIZE = min(MAX_PROTOCOL_MESSAGE_LENGTH, MAX_SIZE)
VALID_DATA_LIMIT = MAX_V1_MESSAGE_SIZE - 5  # Account for the 5-byte length prefix
...
class InvalidMessagesTest(BitcoinTestFramework):
    ...
    def test_size(self):
        ...
        if self.options.v2transport:
            # xCoin: the node's v2 "packet too large" bound is min(MAX_SIZE,
            # MAX_PROTOCOL_MESSAGE_LENGTH) + 13 = 33,554,445 bytes, more than a
            # BIP324 packet's 3-byte length field can express (16,777,215), so no
            # v2 peer can send an oversized packet and the framework refuses to
            # build one (v2_p2p.py asserts len(contents) <= 2**24 - 1).
            self.log.info("Skipping on v2 transport: a BIP324 packet cannot exceed the node's size limit")
            return
        conn = self.nodes[0].add_p2p_connection(P2PDataStore())
        error_msg = [f'Header error: Size too large (badmsg, {MAX_V1_MESSAGE_SIZE + 1} bytes)']
    ...
    def test_invalid_pow_headers_msg(self):
        ...
        # xCoin: proof of work is MetalDAG, not the header hash; solve through the oracle
        blockheader.solve()
    ...
    def test_resource_exhaustion(self):
        ...
        assert len(msg_at_size.serialize()) == MAX_V1_MESSAGE_SIZE

        self.log.info(f"(a) Send 80 messages, each of maximum valid data size ({MAX_V1_MESSAGE_SIZE} bytes)")
        for _ in range(80):
