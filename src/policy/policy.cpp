...
bool IsStandardTx(const CTransaction& tx, const std::optional<unsigned>& max_datacarrier_bytes, bool permit_bare_multisig, bool permit_v2_outputs, const CFeeRate& dust_relay_fee, std::string& reason)
{
    ...
    for (const CTxOut& txout : tx.vout) {
        ...
        if (whichType == TxoutType::NULL_DATA) {
            ...
            return false;
        } else if ((whichType == TxoutType::WITNESS_V2_PQ) && (!permit_v2_outputs)) {
            // Spending a legacy witness v2 output stays valid forever, but creating
            // new ones is non-standard after the re-genesis: new outputs go to witness v3.
            reason = "legacy-v2-output";
            return false;
        }
    }
...
bool IsWitnessStandard(const CTransaction& tx, const CCoinsViewCache& mapInputs)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        // Check policy limits for xCoin witness v3 spends:
        // - MAX_STANDARD_XCOIN_V3_STACK_ITEM_SIZE limit for stack item size under the PQ and SLH leaves
        // - No annexes
        if (witnessversion == 3 && witnessprogram.size() == WITNESS_V3_SIZE && !p2sh) {
            std::span stack{tx.vin[i].scriptWitness.stack};
            if (stack.size() >= 2 && !stack.back().empty() && stack.back()[0] == ANNEX_TAG) {
                // Annexes are nonstandard as long as no semantics are defined for them.
                return false;
            ...
            if (stack.size() < 2) {
                // No key path: fewer than [script, control] is already invalid by consensus rules
                return false;
            }
            const auto& control_block = SpanPopBack(stack);
            SpanPopBack(stack); // Ignore script
            if (control_block.empty()) return false; // Empty control block is invalid
            const uint8_t leaf_version{static_cast<uint8_t>(control_block[0] & TAPROOT_LEAF_MASK)};
            if (leaf_version == XCOIN_LEAF_PQ || leaf_version == XCOIN_LEAF_SLH) {
                for (const auto& item : stack) {
                    if (item.size() > MAX_STANDARD_XCOIN_V3_STACK_ITEM_SIZE) return false;
                }
            }
        }
    }
