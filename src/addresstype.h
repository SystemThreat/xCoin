...
/**
 * Witness v2 PQ destination: OP_2 <32-byte SHA256(pq_pubkey)>
 *
 * Encoded as nex1pq... using Bech32m (witness version 2).
 * The witness program is the 32-byte SHA-256 hash of the ML-DSA-65 public key.
 * To spend, the witness must contain the full 1,952-byte public key and a
...
struct WitnessV2PQ : public BaseHash<uint256>
{
    ...
    explicit WitnessV2PQ(const CPQPubKey& pubkey) : BaseHash(pubkey.GetID()) {}
};

/**
 * Witness v3 PQ destination: OP_3 <32-byte Merkle root of algorithm-tagged leaves>
 *
 * Encoded with Bech32m (witness version 3), so a mainnet address reads xpa1r...
 * The program is the root of the xCoin v3 script tree (REGENESIS.md section 4);
 * a single-leaf tree's root is the leaf hash itself. There is no key path.
 */
struct WitnessV3PQ : public BaseHash<uint256>
{
    WitnessV3PQ() : BaseHash() {}
    explicit WitnessV3PQ(const uint256& merkle_root) : BaseHash(merkle_root) {}
};
 ...
 */
using CTxDestination = std::variant<CNoDestination, PubKeyDestination, PKHash, ScriptHash, WitnessV0ScriptHash, WitnessV0KeyHash, WitnessV1Taproot, WitnessV2PQ, WitnessV3PQ, PayToAnchor, WitnessUnknown>;
