...
public:
    ...
    bool unlock(const SecureString& wallet_passphrase) override
    {
        ...
        // xCoin: post-quantum addresses are derived from the private key only, so the look-ahead
        // cannot be refilled while the wallet is locked; refill it on unlock, as the
        // walletpassphrase RPC does (audit finding Q4).
