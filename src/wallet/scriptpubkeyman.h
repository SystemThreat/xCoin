...
//! Default for -keypool
//! xCoin: 100, not upstream's 1000. Every witness v3 address costs an ML-DSA-65 and an
//! SLH-DSA key generation (~50 ms together), so a 1,000-address look-ahead per chain made
//! createwallet take ~100 s. 100 keeps it near 10 s; -keypool raises it, and a restore with
//! a larger gap is keypoolrefill <n> + rescanblockchain (audit finding 9).
...
public:
    ...
    /** The public descriptor string a wallet without private keys can import to watch the same
     *  scripts (Descriptor::ToExportString): the normalized string, with BIP32-derived
     *  post-quantum keys written out as explicit lists of the cached public keys. */
    [[nodiscard]] bool GetExportDescriptorString(std::string& out) const;
