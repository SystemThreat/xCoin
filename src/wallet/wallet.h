...
//! -fallbackfee default: on, at the relay floor (1 sat/vB, DEFAULT_MIN_RELAY_TX_FEE in policy/policy.h;
//! founder decision 2026-09-15). A fresh node has no fee-estimate history, so without a fallback every
//! send would fail with "Fee estimation failed" until enough blocks had been seen; -fallbackfee=0 disables it.
...
//! xCoin: the post-quantum script tree (pqtr, xpa1r... addresses) is the default; the legacy
//! witness v2 type stays receivable (getnewaddress "" xcoin-v2) but is never the default.
constexpr OutputType DEFAULT_ADDRESS_TYPE{OutputType::XCOIN_V3};
...
public:
    ...
    /** xCoin: whether this wallet may create a NEW legacy witness v2 output: the chain
     *  permits them (regtest only) and the node was started with -permitv2outputs=1, so
     *  that it would relay and mine the transaction (audit finding T9). Existing v2
     *  coins are recognised and spent regardless. */
    bool PermitsV2Outputs() const;
    /** The error every refusal above reports; it names the flag. */
    static bilingual_str V2OutputsNotPermittedError();
    ...
    //! Create new DescriptorScriptPubKeyMans and add them to the wallet. By default only the
    //! post-quantum types (XCOIN_OUTPUT_TYPES, outputtype.h); the inherited coin-selection
    //! tests pass OUTPUT_TYPES because they exercise ECDSA outputs the chain never sees.
    void SetupDescriptorScriptPubKeyMans(WalletBatch& batch, const CExtKey& master_key, std::span<const OutputType> types = XCOIN_OUTPUT_TYPES) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    void SetupDescriptorScriptPubKeyMans(std::span<const OutputType> types = XCOIN_OUTPUT_TYPES) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

    //! Create new seed and default DescriptorScriptPubKeyMans for this wallet
    void SetupOwnDescriptorScriptPubKeyMans(WalletBatch& batch, std::span<const OutputType> types = XCOIN_OUTPUT_TYPES) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
