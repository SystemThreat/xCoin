        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    // The chain's output-value floor (consensus, bad-txout-below-min): the wallet must never
    // build an output the node will refuse (audit finding 10).
    const CAmount output_floor{Params().GetConsensus().minOutputValueSat};
    // A new legacy witness v2 output only where the node relays and mines it (audit finding T9).
    const bool permit_v2{wallet.PermitsV2Outputs()};
    for (const auto& recipient : vecSend) {
        if (IsDust(recipient, wallet.chain().relayDustFee())) {
            return util::Error{_("Transaction amount too small")};
        }
        if (!permit_v2 && std::holds_alternative<WitnessV2PQ>(recipient.dest)) {
            return util::Error{CWallet::V2OutputsNotPermittedError()};
        }
        if (recipient.nAmount < output_floor && !recipient.fSubtractFeeFromAmount) {
            return util::Error{strprintf(_("Transaction amount %s is below this chain's minimum output value of %s"), FormatMoney(recipient.nAmount), FormatMoney(output_floor))};
        }
    ...
    coin_selection_params.min_viable_change = std::max({change_spend_fee + 1, dust, output_floor}); // never change below the chain's floor
    ...
    // Settlement levy (consensus/levy.h, REGENESIS.md section 6). The chain ships
    // without one (the genesis rule is zero rate, zero cap, so everything below is
    // inert and the fee is the byte-rate fee alone); if a later soft fork switches
    // it on, the fee must reach levy_bp of everything the transaction moves, capped
    // per transaction. It is
    // charged ONCE: the levy on the recipients plus the expected change joins the
    // selection target, and the final fee below is floored at the levy the finished
    // outputs actually owe. (Charging every input its own levy as well, as this code
    // once did, multiplied a per-transaction cap by the input count and refused
    // affordable payments: audit finding 11.) With subtract-fee-from-amount the
    // recipients absorb it like any other fee.
    const Consensus::SettlementLevyRule levy{Params().GetConsensus().SettlementLevyAt(wallet.GetLastBlockHeight() + 1)};
    if (!coin_selection_params.m_subtract_fee_outputs) selection_target += Consensus::SettlementLevy(recipients_sum + coin_selection_params.m_min_change_target, levy);
    ...
    // Settlement levy floor. When the outputs still absorb the fee (a change
    // output that is adjusted below, or subtract-fee-from-amount) the floor is
    // the fixed point over the selected value; with fixed outputs it is the
    // levy on exactly those outputs.
    if (levy.bp > 0) {
        const CAmount levy_floor{(change_pos || coin_selection_params.m_subtract_fee_outputs)
                                     ? Consensus::SettlementLevyFromInputs(result.GetSelectedValue(), levy)
                                     : Consensus::SettlementLevy(output_value, levy)};
        fee_needed = std::max(fee_needed, levy_floor);
    }
    ...
    if (coin_selection_params.m_subtract_fee_outputs) {
        ...
        for (const auto& recipient : vecSend)
        {
            ...
            if (recipient.fSubtractFeeFromAmount)
            {
                ...
                // Error if this output is reduced to be below dust or the chain's output floor
                if (IsDust(txout, wallet.chain().relayDustFee()) || txout.nValue < output_floor) {
                    if (txout.nValue < 0) {
    ...
    // Limit size
    // Never hand back a transaction the chain refuses outright (audit finding 10): the
    // output rule, the value floor and the rest of CheckTransaction, with this chain's
    // parameters. A failure here is a wallet bug, reported instead of committed.
    {
        TxValidationState tx_state;
        if (!CheckTransaction(*tx, tx_state, Params().GetConsensus().permitV2Outputs, Params().GetConsensus().minOutputValueSat)) {
            return util::Error{strprintf(_("Transaction violates a consensus rule: %s"), tx_state.ToString())};
        }
    }
