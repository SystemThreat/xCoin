...
{
    // If -changetype is specified, always use that change type ... unless it is the legacy
    // witness v2 form on a chain, or a node, that refuses new v2 outputs: consensus or the
    // node's own policy would reject the transaction, so fall back to the post-quantum
    // script tree (audit findings 12 and T9).
    if (change_type) {
        if (*change_type == OutputType::XCOIN_V2 && !PermitsV2Outputs()) return OutputType::XCOIN_V3;
        return *change_type;
    }
    ...
    // xCoin: change always goes to the post-quantum script tree when the wallet has one. It is
    // the only output type that is both consensus-valid and standard to create on this chain
    // (witness v2 is non-standard to create; v0/v1 are invalid), so the recipient's type does
    // not matter.
    const bool has_xcoin_v3_spkman(GetScriptPubKeyMan(OutputType::XCOIN_V3, /*internal=*/true));
    if (has_xcoin_v3_spkman) {
        return OutputType::XCOIN_V3;
    }
...
bool CWallet::PermitsV2Outputs() const
{
    // Consensus first (mainnet and testnet A never create v2 outputs, audit finding 12),
    // then the node's relay policy (-permitv2outputs, regtest only, audit finding T9).
    if (!Params().GetConsensus().permitV2Outputs) return false;
    return HaveChain() && chain().permitV2Outputs();
}

bilingual_str CWallet::V2OutputsNotPermittedError()
{
    if (!Params().GetConsensus().permitV2Outputs) {
        return _("Legacy witness v2 addresses cannot be paid on this chain (audit finding 12); use the default post-quantum type");
    }
    return _("Legacy witness v2 outputs are not relayed or mined by this node: creating a new witness v2 output requires a regtest node started with -permitv2outputs=1; use the default post-quantum type");
}

util::Result<CTxDestination> CWallet::GetNewDestination(const OutputType type, const std::string& label)
{
    LOCK(cs_wallet);
    if (type == OutputType::XCOIN_V2 && !PermitsV2Outputs()) {
        return util::Error{V2OutputsNotPermittedError()};
    }
    ...
    if (!op_dest) return op_dest;
    CTxDestination dest = *op_dest;
    if (std::holds_alternative<WitnessV3PQ>(dest)) {
        // xCoin address-reuse refusal (REGENESIS.md section 4): a witness v3 address is handed
        // out at most once. The descriptor index only ever moves forward, so a repeat can only
        // come from an address that was already given out (it is in the address book) or that
        // already received and spent coins (e.g. a look-ahead address found by a rescan). Skip
        // such addresses rather than hand them out again.
        int skipped{0};
        while (m_address_book.contains(dest) || IsAddressPreviouslySpent(dest)) {
            if (++skipped > 1000) {
                return util::Error{_("Error: Unable to find an unused xcoin-v3 address; call keypoolrefill")};
            }
            auto next = spk_man->GetNewDestination(type);
            if (!next) return next;
            dest = *next;
        }
        if (skipped) {
            WalletLogPrintf("%s: skipped %d already used xcoin-v3 address(es)\n", __func__, skipped);
        }
    }
...
bool CWallet::LoadWalletArgs(std::shared_ptr<CWallet> wallet, const WalletContext& context, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (!args.GetArg("-changetype", "").empty()) {
        ...
        if (parsed.value() == OutputType::XCOIN_V2 && !wallet->PermitsV2Outputs()) {
            error = Untranslated("-changetype=xcoin-v2: ") + CWallet::V2OutputsNotPermittedError();
            return false;
        }
...
void CWallet::SetupDescriptorScriptPubKeyMans(WalletBatch& batch, const CExtKey& master_key, std::span<const OutputType> types)
{
    AssertLockHeld(cs_wallet);
    // No v2 descriptor on a chain that refuses new v2 outputs (mainnet, testnet A): it
    // could never receive and would only hand out unpayable addresses (audit finding 12).
    const bool permit_v2{Params().GetConsensus().permitV2Outputs};
    for (bool internal : {false, true}) {
        for (OutputType t : types) {
            if (t == OutputType::XCOIN_V2 && !permit_v2) continue;
            SetupDescriptorScriptPubKeyMan(batch, master_key, t, internal);
        }
    }
}

void CWallet::SetupOwnDescriptorScriptPubKeyMans(WalletBatch& batch, std::span<const OutputType> types)
{
    AssertLockHeld(cs_wallet);
    ...
    SetupDescriptorScriptPubKeyMans(batch, master_key, types);
}

void CWallet::SetupDescriptorScriptPubKeyMans(std::span<const OutputType> types)
{
    AssertLockHeld(cs_wallet);

    if (!IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER)) {
        if (!RunWithinTxn(GetDatabase(), /*process_desc=*/"setup descriptors", [&](WalletBatch& batch) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet){
            SetupOwnDescriptorScriptPubKeyMans(batch, types);
            return true;
