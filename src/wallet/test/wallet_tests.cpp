...
static_assert(DEFAULT_FALLBACK_FEE == DEFAULT_MIN_RELAY_TX_FEE, "the wallet fallback fee is the relay floor (founder decision 2026-09-15)");

BOOST_FIXTURE_TEST_SUITE(wallet_tests, WalletTestingSetup)

/** Spend output `index` of `from`, one of the fixture's coinbases (witness v2 to the
 *  shared test ML-DSA-65 key), to `pubkey`, paying DEFAULT_TRANSACTION_MAXFEE. */
static CMutableTransaction TestSimpleSpend(const CTransaction& from, uint32_t index, const CScript& pubkey)
{
    BOOST_REQUIRE(IsTestPQScript(from.vout[index].scriptPubKey));
    CMutableTransaction mtx;
    mtx.vout.emplace_back(from.vout[index].nValue - DEFAULT_TRANSACTION_MAXFEE, pubkey);
    mtx.vin.push_back({CTxIn{from.GetHash(), index}});
    SignTestPQInput(mtx, 0, from.vout[index]);
    return mtx;
}
...
// The wallet owns the coinbases through the post-quantum coinbase descriptor
// (wallet/test/util.h): two blocks pay it, the second in a new block file.
BOOST_FIXTURE_TEST_CASE(scan_for_wallet_transactions, TestChain100Setup)
{
    ...
    BOOST_REQUIRE_EQUAL(newTip->nHeight, 102);
    BOOST_REQUIRE(WITH_LOCK(::cs_main, return oldTip->GetBlockPos().nFile != newTip->GetBlockPos().nFile));
...
public:
    ...
    {
        // Two coinbases to the wallet's post-quantum coinbase key, then enough
        // blocks (to the unrelated shared test key) for both to mature.
        for (int i = 0; i < 2; ++i) CreateAndProcessBlock({}, TestPQCoinbaseScript());
        mineBlocks(COINBASE_MATURITY);
        wallet = CreateSyncedPQWallet(*m_node.chain, WITH_LOCK(Assert(m_node.chainman)->GetMutex(), return m_node.chainman->ActiveChain()));
    }
    ...
    CWalletTx& AddTx(CRecipient recipient)
    {
        ...
        const CBlock block{CreateAndProcessBlock({CMutableTransaction(blocktx)}, GetTestPQScript())};

        LOCK(wallet->cs_wallet);
        LOCK(Assert(m_node.chainman)->GetMutex());
        BOOST_REQUIRE_EQUAL(m_node.chainman->ActiveChain().Tip()->GetBlockHash(), block.GetHash()); // connected, not silently refused
...
// ListCoins groups a coin under its non-change ancestor's address: the change of a
// spend (witness v3, a different script) is listed under the witness v2 coinbase
// address it came from.
BOOST_FIXTURE_TEST_CASE(ListCoinsTest, ListCoinsTestingSetup)
{
    CTxDestination coinbase_address;
    BOOST_REQUIRE(ExtractDestination(TestPQCoinbaseScript(), coinbase_address));
    BOOST_REQUIRE(std::holds_alternative<WitnessV2PQ>(coinbase_address));

    // Confirm ListCoins initially returns the two mature coinbases, grouped under
    ...
    BOOST_CHECK(list.begin()->first == coinbase_address);
    BOOST_CHECK_EQUAL(list.begin()->second.size(), 2U);

    // Check initial balance from the two mature coinbase transactions.
    ...
    // coinbase address, even though the change script is a different one (the
    // post-quantum script tree, witness v3).
    const CWalletTx& wtx{AddTx(CRecipient{WitnessV3PQ{uint256::ONE}, 1 * COIN, /*subtract_fee=*/false})};
    BOOST_REQUIRE_EQUAL(wtx.tx->vin.size(), 1U); // one coinbase spent, the other still listed
    {
        LOCK(wallet->cs_wallet);
        list = ListCoins(*wallet);
    }
    ...
    BOOST_CHECK_EQUAL(list.begin()->second.size(), 2U);
    {
        const auto& coins{list.begin()->second};
        const auto change{std::find_if(coins.begin(), coins.end(), [&](const COutput& c) { return c.outpoint.hash == wtx.GetHash(); })};
        BOOST_REQUIRE(change != coins.end());
        BOOST_CHECK(change->txout.scriptPubKey.IsPayToXcoinV3());
        BOOST_CHECK(change->txout.scriptPubKey != TestPQCoinbaseScript());
    }
...
// AvailableCoins buckets coins by output type. Only the two post-quantum types
// can hold coins on this chain (every other output is bad-txout-not-pq at
// consensus), and change always goes to the witness v3 script tree.
BOOST_FIXTURE_TEST_CASE(BasicOutputTypesTest, ListCoinsTest)
{
    ...
    // Verify our wallet has two usable coinbase UTXOs before starting. They pay
    // the pq() coinbase descriptor, so they sit in the witness v2 bucket.
    expected_coins_sizes[OutputType::XCOIN_V2] = 2U;
    CoinsResult available_coins = WITH_LOCK(wallet->cs_wallet, return AvailableCoins(*wallet));
    BOOST_CHECK_EQUAL(available_coins.Size(), 2U);
    for (const auto& [type, size] : expected_coins_sizes) BOOST_CHECK_EQUAL(size, available_coins.coins[type].size());

    // A self transfer to a witness v2 address spends one coinbase and leaves the
    // recipient output in the v2 bucket and the change in the v3 bucket.
    expected_coins_sizes[OutputType::XCOIN_V2] = 2U; // the other coinbase and the recipient
    expected_coins_sizes[OutputType::XCOIN_V3] = 1U; // the change
    TestCoinsResult(*this, OutputType::XCOIN_V2, 1 * COIN, expected_coins_sizes);

    // A self transfer to a witness v3 address spends the remaining coinbase (the
    // first transfer's outputs are locked): recipient and change both land in v3.
    expected_coins_sizes[OutputType::XCOIN_V2] = 1U;
    expected_coins_sizes[OutputType::XCOIN_V3] = 3U;
    TestCoinsResult(*this, OutputType::XCOIN_V3, 1 * COIN, expected_coins_sizes);
}
...
// The wallet is paid at one of its own witness v3 addresses; the spends come
// from the fixture's coinbases (witness v2 to the shared test key).
BOOST_FIXTURE_TEST_CASE(CreateWallet, TestChain100Setup)
{
    m_args.ForceSetArg("-unsafesqlitesync", "1");
    m_args.ForceSetArg("-keypool", "5"); // a witness v3 address costs an SLH-DSA key generation (~50 ms)
    // Create new wallet, take an address of its own, and unload it.
    WalletContext context;
    context.args = &m_args;
    context.chain = m_node.chain.get();
    auto wallet = TestCreateWallet(context);
    const CScript wallet_script{GetScriptForDestination(*Assert(wallet->GetNewDestination(OutputType::XCOIN_V3, "")))};
    ...
    std::string error;
    m_coinbase_txns.push_back(CreateAndProcessBlock({}, GetTestPQScript()).vtx[0]);
    auto block_tx = TestSimpleSpend(*m_coinbase_txns[0], 0, wallet_script);
    m_coinbase_txns.push_back(CreateAndProcessBlock({block_tx}, GetTestPQScript()).vtx[0]);
    auto mempool_tx = TestSimpleSpend(*m_coinbase_txns[1], 0, wallet_script);
    ...
    auto handler = HandleLoadWallet(context, [&](std::unique_ptr<interfaces::Wallet> wallet) {
            BOOST_CHECK(rescan_completed);
            m_coinbase_txns.push_back(CreateAndProcessBlock({}, GetTestPQScript()).vtx[0]);
            block_tx = TestSimpleSpend(*m_coinbase_txns[2], 0, wallet_script);
            m_coinbase_txns.push_back(CreateAndProcessBlock({block_tx}, GetTestPQScript()).vtx[0]);
            mempool_tx = TestSimpleSpend(*m_coinbase_txns[3], 0, wallet_script);
            BOOST_CHECK(m_node.chain->broadcastTransaction(MakeTransactionRef(mempool_tx), DEFAULT_TRANSACTION_MAXFEE, node::TxBroadcast::MEMPOOL_NO_BROADCAST, error));
            m_node.validation_signals->SyncWithValidationInterfaceQueue();
        });
    wallet = TestLoadWallet(context);
...
// The wallet is paid at one of its own witness v3 addresses from a fixture coinbase.
BOOST_FIXTURE_TEST_CASE(RemoveTxs, TestChain100Setup)
{
    ...
    m_coinbase_txns.push_back(CreateAndProcessBlock({}, GetTestPQScript()).vtx[0]);
    auto block_tx = TestSimpleSpend(*m_coinbase_txns[0], 0, wallet_script);
    CreateAndProcessBlock({block_tx}, GetTestPQScript());
