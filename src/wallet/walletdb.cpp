...
const std::string WALLETDESCRIPTORXCOINCACHE{"walletdescriptorxcoincache"};
...
bool WalletBatch::WriteDescriptorXcoinCache(const std::vector<unsigned char>& pubkey, const uint256& desc_id, uint32_t key_exp_index, uint32_t der_index)
{
    return WriteIC(std::make_pair(std::make_pair(DBKeys::WALLETDESCRIPTORXCOINCACHE, desc_id), std::make_pair(key_exp_index, der_index)), pubkey);
}

bool WalletBatch::WriteDescriptorCacheItems(const uint256& desc_id, const DescriptorCache& cache)
{
    for (const auto& [key_exp_index, keys] : cache.GetCachedXcoinPubKeys()) {
        for (const auto& [der_index, pubkey] : keys) {
            if (!WriteDescriptorXcoinCache(pubkey, desc_id, key_exp_index, der_index)) {
                return false;
            }
        }
    }
...
static DBErrors LoadDescriptorWalletRecords(CWallet* pwallet, DatabaseBatch& batch, int last_client) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    LoadResult desc_res = LoadRecords(pwallet, batch, DBKeys::WALLETDESCRIPTOR,
        ...
        // Get the xCoin post-quantum key cache for this descriptor
        prefix = PrefixStream(DBKeys::WALLETDESCRIPTORXCOINCACHE, id);
        LoadResult xcoin_cache_res = LoadRecords(pwallet, batch, DBKeys::WALLETDESCRIPTORXCOINCACHE, prefix,
            [&id, &cache] (CWallet* pwallet, DataStream& key, DataStream& value, std::string& err) {
            uint256 desc_id;
            uint32_t key_exp_index;
            uint32_t der_index;
            ...
            value >> pubkey;
            cache.CacheXcoinPubKey(key_exp_index, der_index, pubkey);
            return DBErrors::LOAD_OK;
        });
        result = std::max(result, xcoin_cache_res.m_result);
