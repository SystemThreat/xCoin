    ...
    return RPCHelpMan{
        ...
        HELP_REQUIRING_PASSPHRASE,
                ...
                RPCExamples{
                    ...
                    "\nSend 0.1 " + CURRENCY_UNIT + " with a confirmation target of 6 blocks in economical fee estimate mode using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"donation\" \"sean's outpost\" false true 6 economical") +
                    "\nSend 0.1 " + CURRENCY_UNIT + " with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB, subtract fee from amount, BIP125-replaceable, using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"drinks\" \"room77\" true true null \"unset\" null 1.1") +
                    "\nSend 0.2 " + CURRENCY_UNIT + " with a confirmation target of 6 blocks in economical fee estimate mode using named arguments\n"
                    + HelpExampleCli("-named sendtoaddress", "address=\"" + EXAMPLE_ADDRESS[0] + "\" amount=0.2 conf_target=6 estimate_mode=\"economical\"") +
                    "\nSend 0.5 " + CURRENCY_UNIT + " with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
...
RPCHelpMan send()
{
    return RPCHelpMan{
        ...
        RPCExamples{""
        "\nSend 0.1 " + CURRENCY_UNIT + " with a confirmation target of 6 blocks in economical fee estimate mode\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.1}' 6 economical\n") +
        "Send 0.2 " + CURRENCY_UNIT + " with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB using positional arguments\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" 1.1\n") +
        "Send 0.2 " + CURRENCY_UNIT + " with a fee rate of 1 " + CURRENCY_ATOM + "/vB using the options argument\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" null '{\"fee_rate\": 1}'\n") +
        "Send 0.3 " + CURRENCY_UNIT + " with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
...
RPCHelpMan sendall()
{
    return RPCHelpMan{"sendall",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            // sendall builds its outputs itself, so it applies the rule CreateTransaction applies:
            // a new legacy witness v2 output only where the node relays and mines it (audit finding T9).
            if (!pwallet->PermitsV2Outputs()) {
                for (const CTxOut& out : rawTx.vout) {
                    std::vector<std::vector<unsigned char>> solutions;
                    if (Solver(out.scriptPubKey, solutions) == TxoutType::WITNESS_V2_PQ) {
                        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, CWallet::V2OutputsNotPermittedError().original);
                    }
                }
            }
            ...
            // The settlement levy (consensus/levy.h): everything not paid as fee becomes output value.
            const CAmount fee_from_size{std::max(fee_rate.GetFee(tx_size.vsize),
                                                 Consensus::SettlementLevyFromInputs(total_input_value, Params().GetConsensus().SettlementLevyAt(pwallet->GetLastBlockHeight() + 1)))};
            ...
            for (CTxOut& out : rawTx.vout) {
                ...
                if (addresses_without_amount.contains(addr)) {
                    ...
                    if (IsDust(out, pwallet->chain().relayDustFee()) || out.nValue < Params().GetConsensus().minOutputValueSat) {
                        // Dynamically generated output amount is dust or below the chain's output floor
                        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, "Dynamically assigned remainder results in an output below the dust or minimum-value threshold.");
                    }
                } else {
                    if (IsDust(out, pwallet->chain().relayDustFee()) || out.nValue < Params().GetConsensus().minOutputValueSat) {
                        // Specified output amount is dust or below the chain's output floor
                        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Specified output amount to %s is below the dust or minimum-value threshold.", addr));
                    }
                }
            }
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::NUM, "changepos", "The position of the added change output, or -1"},
                                ...
                                RPCExamples{
                            "\nCreate a PSBT with automatically picked inputs that sends 0.5 " + CURRENCY_UNIT + " to an address and has a fee rate of 2 sat/vB:\n"
