...
public:
    ...
    UniValue operator()(const WitnessV2PQ& id) const
    {
        UniValue obj(UniValue::VOBJ);
        CPQPubKey pubkey;
        if (provider && provider->GetPQPubKey(CPQKeyID(uint256(id)), pubkey)) {
            obj.pushKV("pubkey", HexStr(pubkey));
            obj.pushKV("pubkey_algorithm", "ml-dsa-65");
        }
        return obj;
    }

    UniValue operator()(const WitnessV3PQ& id) const
    {
        UniValue obj(UniValue::VOBJ);
        XcoinV3SpendData spenddata;
        if (!provider || !provider->GetXcoinV3SpendData(uint256(id), spenddata)) return obj;
        const auto tree = InferXcoinV3Tree(spenddata, uint256(id));
        if (!tree) return obj;
        UniValue leaves(UniValue::VARR);
        for (const auto& [depth, script_bytes, leaf_ver] : *tree) {
            ...
            leaf.pushKV("asm", ScriptToAsmStr(script));
            if (const auto tmpl = ParseXcoinV3LeafTemplate(leaf_ver, script)) {
                leaf.pushKV("algorithm", tmpl->algorithm == XcoinV3LeafTemplate::Algorithm::MLDSA ? "ml-dsa-65" : "slh-dsa-sha2-128s");
                if (tmpl->algorithm == XcoinV3LeafTemplate::Algorithm::MLDSA) {
                    CPQPubKey pubkey;
                    leaf.pushKV("pubkey_sha256", HexStr(tmpl->key));
                    if (provider->GetPQPubKey(CPQKeyID(uint256(tmpl->key)), pubkey)) leaf.pushKV("pubkey", HexStr(pubkey));
                } else {
                    leaf.pushKV("pubkey", HexStr(tmpl->key));
                }
                if (tmpl->timelock == XcoinV3LeafTemplate::Timelock::AFTER) leaf.pushKV("after", static_cast<int64_t>(tmpl->timelock_value));
                if (tmpl->timelock == XcoinV3LeafTemplate::Timelock::OLDER) leaf.pushKV("older", static_cast<int64_t>(tmpl->timelock_value));
            }
            // Anyone-can-spend leaves (OP_SUCCESSx, or a key push that is not
            // 32 bytes): never produced by the wallet's own descriptors, but a
            // tree imported from elsewhere can carry one, and a holder has to
            // be told before he funds the address.
            if (const std::vector<std::string> leaf_warnings{XcoinV3LeafWarnings(script)}; !leaf_warnings.empty()) {
                UniValue arr(UniValue::VARR);
                for (const std::string& w : leaf_warnings) arr.push_back(w);
                leaf.pushKV("warnings", std::move(arr));
            }
            leaves.push_back(std::move(leaf));
        }
        obj.pushKV("xcoin_v3_leaves", std::move(leaves));
        return obj;
    }
        ...
        "Return information about the given bitcoin address.\n"
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::BOOL, "ispq", /*optional=*/true, "True for the xCoin post-quantum witness versions (2 = legacy ML-DSA-65 key hash, 3 = script tree)."},
                        ...
                        {RPCResult::Type::STR, "pubkey_algorithm", /*optional=*/true, "xCoin witness v2: the algorithm of the post-quantum key behind the address (\"ml-dsa-65\")."},
                        {RPCResult::Type::ARR, "xcoin_v3_leaves", /*optional=*/true, "xCoin witness v3: the leaves of the script tree behind the address, in tree order (only if the tree is known).",
                        {
                            {RPCResult::Type::OBJ, "", "",
                            {
                                {RPCResult::Type::NUM, "depth", "Depth of the leaf in the tree"},
                                {RPCResult::Type::NUM, "leaf_version", "Leaf version (192 = ML-DSA-65 leaf, 194 = SLH-DSA-SHA2-128s leaf)"},
                                {RPCResult::Type::STR_HEX, "script", "The leaf script"},
                                {RPCResult::Type::STR, "asm", "The leaf script, disassembled"},
                                {RPCResult::Type::STR, "algorithm", /*optional=*/true, "\"ml-dsa-65\" or \"slh-dsa-sha2-128s\""},
                                {RPCResult::Type::STR_HEX, "pubkey_sha256", /*optional=*/true, "ML-DSA-65 leaf: SHA-256 of the public key (what the script commits to)"},
                                {RPCResult::Type::STR_HEX, "pubkey", /*optional=*/true, "The post-quantum public key (ML-DSA-65: only if known to the wallet)"},
                                {RPCResult::Type::NUM, "after", /*optional=*/true, "OP_CHECKLOCKTIMEVERIFY value of a time-locked leaf"},
                                {RPCResult::Type::NUM, "older", /*optional=*/true, "OP_CHECKSEQUENCEVERIFY value of a relative-time-locked leaf"},
                                {RPCResult::Type::ARR, "warnings", /*optional=*/true, "Present only when this leaf is ANYONE-CAN-SPEND: an OP_SUCCESSx byte, or a key push that is not 32 bytes.",
                                {
