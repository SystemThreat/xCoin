...
static UniValue ProcessDescriptorImport(CWallet& wallet, const UniValue& data, const int64_t timestamp) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    try {
        ...
        if (!parsed_descs.at(0)->IsRange() && data.exists("range")) {
            ...
            // xCoin: a descriptor carrying explicit post-quantum key lists (the watch-only
            // export of a pq()/pqtr() descriptor, audit finding W3) has one position per key.
            const std::optional<int64_t> range_size{parsed_descs.at(0)->RangeSize()};
            if (data.exists("range")) {
                auto range = ParseDescriptorRange(data["range"]);
                range_start = range.first;
                range_end = range.second + 1; // Specified range end is inclusive, but we need range end as exclusive
                if (range_size && range_end > *range_size) {
                    throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Range end %d is beyond the %d positions of the descriptor's post-quantum key list (the last position is %d)", range_end - 1, *range_size, *range_size - 1));
                }
            } else {
                warnings.push_back("Range not given, using default keypool range");
                range_start = 0;
                range_end = range_size ? std::min<int64_t>(wallet.m_keypool_size, *range_size) : wallet.m_keypool_size;
            }
        ...
        // xCoin: a descriptor's private material may be secp256k1, ML-DSA-65 or
        // SLH-DSA-SHA2-128s. A pqtr()/pq()/slh() descriptor carries no CKey at
        // all, so asking only `keys.keys` here refused every post-quantum
        // descriptor that HAD its private key, and accepted one into a
        // private-keys-disabled wallet.
        const bool import_has_private_keys{!keys.keys.empty() || !keys.pq_keys.empty() || !keys.slh_keys.empty()};

        // If the wallet disabled private keys, abort if private keys exist
        if (wallet.IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS) && import_has_private_keys) {
        ...
        for (size_t j = 0; j < parsed_descs.size(); ++j) {
            ...
            bool have_all_privkeys = !expand_keys.keys.empty() || !expand_keys.pq_keys.empty() || !expand_keys.slh_keys.empty();
            ...
            for (const auto& entry : expand_keys.pq_pubkeys) {
                if (!expand_keys.pq_keys.count(entry.first)) {
                    have_all_privkeys = false;
                    break;
                }
            }
            ...
            if (!wallet.IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
               if (!import_has_private_keys) {
                    throw JSONRPCError(RPC_WALLET_ERROR, "Cannot import descriptor without private keys to a wallet with private keys enabled");
               }
               if (!keys.pq_keys.empty() || !keys.slh_keys.empty()) {
                    // xCoin: DescriptorScriptPubKeyMan only persists secp256k1 private
                    // keys (m_map_keys / m_map_crypted_keys), and HavePrivateKeys() is
                    // what gates ExpandPrivate at signing time. A LITERAL ML-DSA-65 or
                    // SLH-DSA private key is therefore dropped by the import: alone, it
                    // would import, show ismine and solvable, and then fail to sign; next
                    // to a BIP32 key (e.g. pqtr({pq(<seed>),slh(xprv/...)})) the import
                    // would succeed and every spend would silently go through whichever
                    // leaf the surviving key satisfies, such as the 7,856-byte SLH-DSA
                    // fallback (audit finding W2). Refuse both out loud. The BIP32 forms
                    // carry a secp256k1 key and work: the post-quantum keys are derived
                    // from the root key and path (REGENESIS.md section 4, src/pqhd.h).
                    throw JSONRPCError(RPC_WALLET_ERROR, "Cannot import a descriptor that carries a literal post-quantum private key (a pq()/slh() seed or secret key): this wallet cannot store one yet, so the key would be dropped and the coins would be unspendable or spendable only through another leaf. Use a BIP32 form for every key instead, e.g. pqtr({pq(xprv.../0h/0/*),slh(xprv.../0h/0/*)}).");
               }
...
RPCHelpMan listdescriptors()
{
    return RPCHelpMan{
        ...
        RPCResult{RPCResult::Type::OBJ, "", "", {
            ...
            {
                {RPCResult::Type::OBJ, "", "", {
                    {RPCResult::Type::STR, "desc", "Descriptor string representation. Without \"private\", a BIP32-derived post-quantum key (pq()/slh(), which has no public derivation) is written out as the explicit list of the public keys this wallet has derived, one per position, so the output can be imported into a wallet with private keys disabled to watch the same scripts"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (const auto& spk_man : wallet->GetAllScriptPubKeyMans()) {
        ...
        std::string descriptor;
        // The public form is the one a wallet without private keys can import to watch the same
        // scripts: BIP32-derived post-quantum keys are written out as explicit key lists (audit
        // finding W3); the private form is the ranged xprv descriptor, unchanged.
        CHECK_NONFATAL(priv ? desc_spk_man->GetDescriptorString(descriptor, /*priv=*/true) : desc_spk_man->GetExportDescriptorString(descriptor));
