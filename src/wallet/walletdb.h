...
public:
    ...
    //! xCoin: a derived post-quantum public key (pq()/slh() keys have no public derivation, so every one is cached)
    bool WriteDescriptorXcoinCache(const std::vector<unsigned char>& pubkey, const uint256& desc_id, uint32_t key_exp_index, uint32_t der_index);
