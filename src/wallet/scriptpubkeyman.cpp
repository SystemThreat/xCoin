...
IsMineResult LegacyWalletIsMineInnerDONOTUSE(const LegacyDataSPKM& keystore, const CScript& scriptPubKey, IsMineSigVersion sigversion, bool recurse_scripthash=true)
{
    ...
    case TxoutType::WITNESS_V2_PQ: // xCoin post-quantum outputs: never a legacy-wallet key
...
util::Result<CTxDestination> DescriptorScriptPubKeyMan::GetNewDestination(const OutputType type)
{
    ...
    }
    {
        ...
        if (!m_wallet_descriptor.descriptor->ExpandFromCache(m_wallet_descriptor.next_index, m_wallet_descriptor.cache, scripts_temp, out_keys)) {
            // We can't generate anymore keys
            if (m_storage.IsLocked()) {
                // xCoin: a post-quantum key is derived from the private key only (there is no
                // public derivation), so the look-ahead cannot be refilled while the wallet is
                // locked and keypoolrefill would refuse too (audit finding Q4). Say so: the
                // pool is topped up again on unlock (walletpassphrase).
                return util::Error{_("Error: Keypool ran out and the wallet is locked: unlock it with walletpassphrase to derive more addresses (the keypool is refilled on unlock)")};
            }
            return util::Error{_("Error: Keypool ran out, please call keypoolrefill first")};
        }
...
{
    LOCK(cs_desc_man);
    // xCoin: a witness v3 address is never handed out twice, so a returned one is simply burned.
    if (std::holds_alternative<WitnessV3PQ>(addr)) return;
...
bool DescriptorScriptPubKeyMan::TopUpWithDB(WalletBatch& batch, unsigned int size)
{
    ...
    if (size > 0) {
        ...
        target_size = m_keypool_size; // the configured look-ahead for every type (audit finding 9: a 20-address cap hid restored funds)
    }

    // Calculate the new range_end
    int32_t new_range_end = std::max(m_wallet_descriptor.next_index + (int32_t)target_size, m_wallet_descriptor.range_end);
    // xCoin: a descriptor carrying explicit post-quantum key lists (the watch-only export of a
    // pq()/pqtr() descriptor, audit finding W3) has one position per key; importdescriptors
    // keeps range_end within them, so the look-ahead is what stops at the end of the list.
    if (const auto range_size = m_wallet_descriptor.descriptor->RangeSize()) {
        const int32_t limit{static_cast<int32_t>(std::min<int64_t>(*range_size, std::numeric_limits<int32_t>::max()))};
        new_range_end = std::max(m_wallet_descriptor.range_end, std::min(new_range_end, limit));
    }
...
bool DescriptorScriptPubKeyMan::GetExportDescriptorString(std::string& out) const
{
    LOCK(cs_desc_man);

    FlatSigningProvider provider;
    provider.keys = GetKeys();
    return m_wallet_descriptor.descriptor->ToExportString(provider, out, &m_wallet_descriptor.cache);
}
