...
/** Map key derivation index -> xCoin post-quantum public key bytes (1,952 for ML-DSA-65, 32 for SLH-DSA-SHA2-128s) */
using XcoinPubKeyMap = std::unordered_map<uint32_t, std::vector<unsigned char>>;
...
private:
    ...
    /** Map key expression index -> map of (key derivation index -> post-quantum public key).
     *  pq()/slh() keys have no public derivation, so every derived key is cached. */
    std::unordered_map<uint32_t, XcoinPubKeyMap> m_xcoin_pubkeys;

public:
    /** Cache an xCoin post-quantum public key derived at an index */
    void CacheXcoinPubKey(uint32_t key_exp_pos, uint32_t der_index, const std::vector<unsigned char>& pubkey);
    /** Retrieve a cached xCoin post-quantum public key derived at an index */
    bool GetCachedXcoinPubKey(uint32_t key_exp_pos, uint32_t der_index, std::vector<unsigned char>& pubkey) const;
    /** Retrieve all cached xCoin post-quantum public keys */
    std::unordered_map<uint32_t, XcoinPubKeyMap> GetCachedXcoinPubKeys() const;
...
struct Descriptor {
    ...
    /** Convert the descriptor to the string a wallet without private keys can import to watch the
     *  same scripts (listdescriptors). The same as ToNormalizedString, except that a BIP32-derived
     *  xCoin post-quantum key (pq()/slh(), which has no public derivation) is written out as the
     *  explicit list of the public keys in `cache`, one per position: pq(keys(<pub>,<pub>,...)). The
     *  result is then ranged over exactly those positions (RangeSize()); the ranged private form
     *  is unchanged (audit finding W3). */
    virtual bool ToExportString(const SigningProvider& provider, std::string& out, const DescriptorCache* cache = nullptr) const = 0;

    /** The number of positions a ranged descriptor can be expanded at when that is finite (it
     *  carries an explicit post-quantum key list); std::nullopt when unbounded. */
    virtual std::optional<int64_t> RangeSize() const = 0;
