...
std::string ScriptErrorString(const ScriptError serror)
{
    switch (serror)
    {
        ...
        case SCRIPT_ERR_XCOIN_V3_CONTROL:
            return "Invalid witness v3 control block (size, parity bit or no-key slot)";
        case SCRIPT_ERR_XCOIN_V3_SIG_SIZE:
            return "Invalid witness v3 signature size (ML-DSA 3,309 or SLH-DSA 7,856 bytes plus an optional hash-type byte)";
        case SCRIPT_ERR_XCOIN_V3_SIG_HASHTYPE:
            return "Invalid witness v3 signature hash type";
        case SCRIPT_ERR_XCOIN_V3_SIG:
            return "Invalid witness v3 ML-DSA or SLH-DSA signature";
        case SCRIPT_ERR_XCOIN_V3_PUBKEY:
            return "Witness v3 public key does not match the leaf's key hash, has the wrong size, or is not empty next to an empty signature";
        case SCRIPT_ERR_XCOIN_V3_HASH_WEIGHT:
            return "Witness v3 leaf hashes more bytes than its validation-weight budget (witness size + 50, shared with the signature checks)";
        case SCRIPT_ERR_DISCOURAGE_UPGRADABLE_XCOIN_LEAF_VERSION:
            return "Witness v3 leaf version reserved for soft-fork upgrades";
