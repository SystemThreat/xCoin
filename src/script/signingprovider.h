...
/** Spending data for an xCoin witness v3 output (contrib/regenesis/REGENESIS.md section 4):
 *  the Merkle root (= the witness program) and, for every leaf, its control block(s). There is
 *  no key path and no internal key: the control blocks carry the fixed XCOIN_V3_NOKEY marker. */
struct XcoinV3SpendData
{
    /** The Merkle root of the leaf tree; this IS the 32-byte witness program. */
    uint256 merkle_root;
    /** Map from (script, leaf_version) to (sets of) control blocks, smallest first. */
    std::map<std::pair<std::vector<unsigned char>, int>, std::set<std::vector<unsigned char>, ShortestVectorFirstComparator>> scripts;
    /** Merge other XcoinV3SpendData (for the same scriptPubKey) into this. */
    void Merge(XcoinV3SpendData other);
};

/** Utility class to construct xCoin witness v3 outputs from a leaf tree. Same DFS
 *  Add(depth, ...) protocol as TaprootBuilder, but the xCoin leaf/branch tags are used, there
 *  is no internal key, and a tree must have at least one leaf (there is no key path to fall
 *  back on). */
class XcoinV3Builder
{
private:
    struct LeafInfo
    {
...
public:
    /** Add a leaf (leaf_version must be even: XCOIN_LEAF_PQ, XCOIN_LEAF_SLH, ...). Add() calls
     *  must follow a depth-first traversal of the binary tree. */
    XcoinV3Builder& Add(int depth, std::span<const unsigned char> script, int leaf_version, bool track = true);
    /** Like Add(), but for a Merkle node with a given hash. */
    XcoinV3Builder& AddOmitted(int depth, const uint256& hash);

    bool IsValid() const { return m_valid; }
    /** Whether the leaves added so far form exactly one complete tree with at least one leaf. */
    bool IsComplete() const { return m_valid && m_branch.size() == 1 && m_branch[0].has_value(); }
    /** The Merkle root (after IsComplete()). */
    uint256 GetRoot() const;
    /** The output (after IsComplete()): OP_3 <root>. */
    WitnessV3PQ GetOutput() const;
    /** Check if a list of depths is legal (will lead to IsComplete()). */
    static bool ValidDepths(const std::vector<int>& depths) { return !depths.empty() && TaprootBuilder::ValidDepths(depths); }
    /** Compute spending data (after IsComplete()). */
    XcoinV3SpendData GetSpendData() const;
    /** (depth, leaf_version, script) for every tracked leaf, in DFS order. */
    std::vector<std::tuple<uint8_t, uint8_t, std::vector<unsigned char>>> GetTreeTuples() const;

    bool operator==(const XcoinV3Builder& other) const { return GetTreeTuples() == other.GetTreeTuples(); }
};

/** Given an XcoinV3SpendData and the witness program (root), reconstruct the leaf tree as
 *  (depth, script, leaf_ver) tuples in DFS order, or std::nullopt if the data is inconsistent
 *  or incomplete. */
...
public:
    ...
    // xCoin post-quantum keys and witness v3 trees (contrib/regenesis/REGENESIS.md section 4).
    //! ML-DSA-65 public key by SHA-256(pubkey) (the witness v2 program / the v3 leaf key hash).
    virtual bool GetPQPubKey(const CPQKeyID& keyid, CPQPubKey& pubkey) const { return false; }
    virtual bool GetPQKey(const CPQKeyID& keyid, CPQKey& key) const { return false; }
    virtual bool HavePQKey(const CPQKeyID& keyid) const { return false; }
    //! SLH-DSA-SHA2-128s secret key by its 32-byte public key (the key pushed by an SLH leaf).
    virtual bool GetSLHKey(const CSLHPubKey& pubkey, CSLHKey& key) const { return false; }
    virtual bool HaveSLHKey(const CSLHPubKey& pubkey) const { return false; }
    //! Leaf tree of a witness v3 output by its Merkle root (= witness program).
    virtual bool GetXcoinV3SpendData(const uint256& root, XcoinV3SpendData& spenddata) const { return false; }
    virtual bool GetXcoinV3Builder(const uint256& root, XcoinV3Builder& builder) const { return false; }
...
public:
    ...
    bool GetPQPubKey(const CPQKeyID& keyid, CPQPubKey& pubkey) const override;
    bool GetPQKey(const CPQKeyID& keyid, CPQKey& key) const override;
    bool HavePQKey(const CPQKeyID& keyid) const override;
    bool GetSLHKey(const CSLHPubKey& pubkey, CSLHKey& key) const override;
    bool HaveSLHKey(const CSLHPubKey& pubkey) const override;
    bool GetXcoinV3SpendData(const uint256& root, XcoinV3SpendData& spenddata) const override;
    bool GetXcoinV3Builder(const uint256& root, XcoinV3Builder& builder) const override;
...
struct FlatSigningProvider final : public SigningProvider
{
    ...
    // xCoin post-quantum material
    std::map<CPQKeyID, CPQPubKey> pq_pubkeys; /** ML-DSA-65 public keys by SHA-256(pubkey) */
    std::map<CPQKeyID, CPQKey> pq_keys;       /** ML-DSA-65 secret keys by SHA-256(pubkey) */
    std::map<CSLHPubKey, CSLHKey> slh_keys;   /** SLH-DSA-SHA2-128s secret keys by public key */
    std::map<uint256, XcoinV3Builder> xcoin_v3_trees; /** Map from witness v3 program (root) to its leaf tree */
