...
std::optional<uint256> MutableTransactionSignatureCreator::ComputeXcoinV3SignatureHash(const uint256& leaf_hash, SigVersion sigversion) const
{
    assert(IsXcoinV3Tapscript(sigversion));
    // Like BIP 341/342 signing, the v3 sighash needs every spent output.
    if (!m_txdata || !m_txdata->m_bip341_taproot_ready || !m_txdata->m_spent_outputs_ready) return std::nullopt;
    // Only the hash types consensus accepts (REGENESIS.md section 4).
    if (!(nHashType >= 0 && (nHashType <= 0x03 || (nHashType >= 0x81 && nHashType <= 0x83)))) return std::nullopt;

    ScriptExecutionData execdata;
    execdata.m_annex_init = true;
    execdata.m_annex_present = false; // Annex-less signing only (annexes are non-standard).
    execdata.m_codeseparator_pos_init = true;
    execdata.m_codeseparator_pos = 0xFFFFFFFF; // No OP_CODESEPARATOR in the supported leaves.
    execdata.m_tapleaf_hash_init = true;
    execdata.m_tapleaf_hash = leaf_hash;
    uint256 hash;
    if (!SignatureHashXcoinV3(hash, execdata, m_txto, nIn, static_cast<uint8_t>(nHashType), sigversion, *m_txdata, MissingDataBehavior::FAIL)) return std::nullopt;
    return hash;
}

bool MutableTransactionSignatureCreator::CreatePQSig(const SigningProvider& provider, std::vector<unsigned char>& sig, const CPQKeyID& keyid, const CScript& scriptCode) const
{
    CPQKey key;
    if (!provider.GetPQKey(keyid, key)) return false;
    // Signing without known amount does not work in witness scripts.
    if (!MoneyRange(amount)) return false;
    // The witness v2 check (CheckPQSig) reads the hash type from the last byte and
    // computes the BIP143-style digest; SIGHASH_DEFAULT is not a v0 concept.
    const int hashtype = nHashType == SIGHASH_DEFAULT ? SIGHASH_ALL : nHashType;
    const uint256 hash = SignatureHash(scriptCode, m_txto, nIn, hashtype, amount, SigVersion::WITNESS_V0, m_txdata);
    if (!key.Sign(hash, sig)) return false;
    sig.push_back(static_cast<unsigned char>(hashtype));
    return true;
}

bool MutableTransactionSignatureCreator::CreateXcoinV3PQSig(const SigningProvider& provider, std::vector<unsigned char>& sig, const CPQKeyID& keyid, const uint256& leaf_hash) const
{
    CPQKey key;
    if (!provider.GetPQKey(keyid, key)) return false;
    std::optional<uint256> hash = ComputeXcoinV3SignatureHash(leaf_hash, SigVersion::XCOIN_PQ_TAPSCRIPT);
    if (!hash) return false;
    // The tagged 32-byte digest IS the ML-DSA message (never pre-hashed again).
    if (!key.SignMessage(std::span<const unsigned char>{hash->begin(), hash->size()}, sig)) return false;
    // Strict encoding: exactly PQ_SIGNATURE_SIZE bytes, then one optional hash-type byte.
    if (sig.size() != PQ_SIGNATURE_SIZE) return false;
    if (nHashType != SIGHASH_DEFAULT) sig.push_back(static_cast<unsigned char>(nHashType));
    return true;
}

bool MutableTransactionSignatureCreator::CreateXcoinV3SLHSig(const SigningProvider& provider, std::vector<unsigned char>& sig, const CSLHPubKey& pubkey, const uint256& leaf_hash) const
{
    CSLHKey key;
    if (!provider.GetSLHKey(pubkey, key)) return false;
    std::optional<uint256> hash = ComputeXcoinV3SignatureHash(leaf_hash, SigVersion::XCOIN_SLH_TAPSCRIPT);
    if (!hash) return false;
    // FIPS 205 pure mode with an empty context over the tagged digest (CSLHKey::Sign).
    if (!key.Sign(*hash, sig)) return false;
...
// ── xCoin post-quantum outputs (contrib/regenesis/REGENESIS.md section 4) ────

std::optional<XcoinV3LeafTemplate> ParseXcoinV3LeafTemplate(int leaf_version, const CScript& script)
{
    XcoinV3LeafTemplate tmpl;
    if (leaf_version == XCOIN_LEAF_PQ) {
        tmpl.algorithm = XcoinV3LeafTemplate::Algorithm::MLDSA;
    } else if (leaf_version == XCOIN_LEAF_SLH) {
        tmpl.algorithm = XcoinV3LeafTemplate::Algorithm::SLH;
    } else {
        return std::nullopt;
    }
    // Decode the script into (opcode, push data) pairs.
    std::vector<std::pair<opcodetype, valtype>> ops;
    CScript::const_iterator it = script.begin();
    while (it < script.end()) {
        opcodetype opcode;
        valtype data;
        if (!script.GetOp(it, opcode, data)) return std::nullopt;
        ops.emplace_back(opcode, std::move(data));
    }
    // <key32> is a canonical 32-byte push (0x20 <32 bytes>) in every template.
    if (ops.size() != 2 && ops.size() != 4) return std::nullopt;
    if (ops[0].first != 0x20 || ops[0].second.size() != 32) return std::nullopt;
    tmpl.key = ops[0].second;
    if (ops.size() == 2) {
        if (ops[1].first != OP_CHECKSIG) return std::nullopt;
        return tmpl;
    }
    if (ops[1].first != OP_CHECKSIGVERIFY) return std::nullopt;
    // <n> as the script engine (and miniscript) encode it: OP_1..OP_16 or a minimal CScriptNum push.
    int64_t n;
    if (ops[2].first >= OP_1 && ops[2].first <= OP_16) {
        n = CScript::DecodeOP_N(ops[2].first);
    } else if (ops[2].first <= OP_PUSHDATA4 && !ops[2].second.empty() && ops[2].second.size() <= 5) {
        try {
            n = CScriptNum(ops[2].second, /*fRequireMinimal=*/true, 5).GetInt64();
    ...
    if (n < 1 || n > 0x7fffffff) return std::nullopt;
    tmpl.timelock_value = static_cast<uint32_t>(n);
    if (ops[3].first == OP_CHECKLOCKTIMEVERIFY) {
        tmpl.timelock = XcoinV3LeafTemplate::Timelock::AFTER;
    } else if (ops[3].first == OP_CHECKSEQUENCEVERIFY) {
        tmpl.timelock = XcoinV3LeafTemplate::Timelock::OLDER;
    } else {
        return std::nullopt;
    }
    return tmpl;
...
{
    // Two ways a leaf script is spendable by anyone (REGENESIS.md section 4):
    //
    //  1. It contains an OP_SUCCESSx byte. BIP-342's rule, which the v3 leaves
    //     inherit unchanged, is that the whole script succeeds before a single
    //     opcode runs. Note that 0xbb / 0xbc, which are OP_CHECKSIG_PQ and
    //     OP_CHECKSIGVERIFY_PQ everywhere else on this chain, are
    //     OP_SUCCESS187 / OP_SUCCESS188 inside a leaf: putting the legacy PQ
    //     checksig in a leaf hands the coins away.
    //  2. Its key push is not 32 bytes. Both leaf versions read the push before
    //     the signature opcode as the key (ML-DSA: the SHA-256 of the key;
    //     SLH-DSA: the key itself); any other size is an UPGRADABLE key type,
    //     which the interpreter passes without verifying anything, exactly as
    //     BIP-342 does for a non-32-byte x-only key.
    //
    // Neither is an error: both are how the soft-fork room is left open. They
    // are only ever a mistake in a script someone means to pay to, so the
    // wallet-facing RPCs (decodescript, getaddressinfo) report them and let a
    // wallet warn. Empty result means nothing to say.
    ...
    while (it < script.end()) {
        ...
        if (!script.GetOp(it, opcode, data)) {
            // A leaf that does not parse fails with BAD_OPCODE unless an
            // OP_SUCCESSx was already found, and those are reported below.
            parses = false;
            break;
        }
        ops.emplace_back(opcode, std::move(data));
    }
    for (const auto& [opcode, data] : ops) {
        if (IsOpSuccess(opcode)) {
            warnings.push_back(strprintf("as an xCoin witness v3 leaf script this is ANYONE-CAN-SPEND: it contains OP_SUCCESS%d (0x%02x), which makes the whole leaf succeed without running", static_cast<int>(opcode), static_cast<int>(opcode)));
            break;
        }
    }
    if (!parses) return warnings;

    // The key push is the one immediately before the signature opcode.
    for (size_t i = 1; i < ops.size(); ++i) {
        const opcodetype op{ops[i].first};
        if (op != OP_CHECKSIG && op != OP_CHECKSIGVERIFY && op != OP_CHECKSIGADD) continue;
        const auto& [push_op, push] = ops[i - 1];
        if (push_op > OP_PUSHDATA4) continue; // not a push at all: not a key slot
        if (push.size() == 32) continue;
        warnings.push_back(strprintf("as an xCoin witness v3 leaf script this is ANYONE-CAN-SPEND: the key push before %s is %d bytes, not 32, which is an upgradable key type and passes the check without verifying a signature", GetOpName(op), static_cast<int>(push.size())));
        break;
    }
    return warnings;
}

static bool GetPQPubKey(const SigningProvider& provider, const SignatureData& sigdata, const CPQKeyID& keyid, CPQPubKey& pubkey)
{
    const auto it = sigdata.pq_pubkeys.find(keyid);
    if (it != sigdata.pq_pubkeys.end()) {
        pubkey = it->second;
        return true;
    }
    return provider.GetPQPubKey(keyid, pubkey);
}

/** Witness v2: [sig, pubkey] spending OP_2 <SHA256(pubkey)>. */
static bool SignXcoinV2(const SigningProvider& provider, const BaseSignatureCreator& creator, const uint256& program, const CScript& scriptPubKey, SignatureData& sigdata, std::vector<valtype>& result)
{
    const CPQKeyID keyid{program};
    CPQPubKey pubkey;
    if (!GetPQPubKey(provider, sigdata, keyid, pubkey)) return false;
    sigdata.pq_pubkeys.emplace(keyid, pubkey);
    // The v2 sighash scriptCode is OP_2 <program>, i.e. the scriptPubKey itself
    // (VerifyWitnessProgram in interpreter.cpp).
    std::vector<unsigned char> sig;
    if (!creator.CreatePQSig(provider, sig, keyid, scriptPubKey)) return false;
    result.clear();
    result.push_back(std::move(sig));
    result.emplace_back(pubkey.begin(), pubkey.end());
    return true;
}

/** Try to satisfy one v3 leaf; on success `result` holds the leaf's initial stack items
 *  (everything below the leaf script and control block). */
static bool SatisfyXcoinV3Leaf(const SigningProvider& provider, const BaseSignatureCreator& creator, SignatureData& sigdata, int leaf_version, const CScript& script, const uint256& leaf_hash, std::vector<valtype>& result)
{
    const auto tmpl = ParseXcoinV3LeafTemplate(leaf_version, script);
    if (!tmpl) return false;
    // Time locks: only satisfiable if the transaction already meets them.
    switch (tmpl->timelock) {
    case XcoinV3LeafTemplate::Timelock::NONE: break;
    case XcoinV3LeafTemplate::Timelock::AFTER:
        if (!creator.Checker().CheckLockTime(CScriptNum(tmpl->timelock_value))) return false;
        break;
    case XcoinV3LeafTemplate::Timelock::OLDER:
        if (!creator.Checker().CheckSequence(CScriptNum(tmpl->timelock_value))) return false;
        break;
    }
    result.clear();
    if (tmpl->algorithm == XcoinV3LeafTemplate::Algorithm::MLDSA) {
        const CPQKeyID keyid{uint256(tmpl->key)};
        CPQPubKey pubkey;
        if (!GetPQPubKey(provider, sigdata, keyid, pubkey)) return false;
        sigdata.pq_pubkeys.emplace(keyid, pubkey);
        std::vector<unsigned char> sig;
        if (!creator.CreateXcoinV3PQSig(provider, sig, keyid, leaf_hash)) return false;
        // Stack shape (pubkey sig hash32 -- bool): the 1,952-byte key sits below the signature.
        result.emplace_back(pubkey.begin(), pubkey.end());
        result.push_back(std::move(sig));
        return true;
    }
    const CSLHPubKey pubkey{tmpl->key};
    std::vector<unsigned char> sig;
    if (!creator.CreateXcoinV3SLHSig(provider, sig, pubkey, leaf_hash)) return false;
    // Stack shape (sig pubkey -- bool): the key is pushed by the leaf script itself.
    result.push_back(std::move(sig));
    return true;
}

/** Witness v3: pick the cheapest leaf we can satisfy and assemble [items..., script, control]. */
static bool SignXcoinV3(const SigningProvider& provider, const BaseSignatureCreator& creator, const uint256& root, SignatureData& sigdata, std::vector<valtype>& result)
{
    if (sigdata.xcoin_v3_spenddata.scripts.empty()) {
        XcoinV3SpendData spenddata;
        if (provider.GetXcoinV3SpendData(root, spenddata)) {
            sigdata.xcoin_v3_spenddata.Merge(std::move(spenddata));
        }
    }
    if (sigdata.xcoin_v3_spenddata.merkle_root != root) return false;

    std::vector<valtype> smallest_result_stack;
    for (const auto& [key, control_blocks] : sigdata.xcoin_v3_spenddata.scripts) {
        const auto& [script_bytes, leaf_ver] = key;
        if (control_blocks.empty()) continue;
        const CScript script(script_bytes.begin(), script_bytes.end());
        const uint256 leaf_hash = ComputeXcoinLeafHash(static_cast<uint8_t>(leaf_ver), script_bytes);
        std::vector<valtype> result_stack;
        if (SatisfyXcoinV3Leaf(provider, creator, sigdata, leaf_ver, script, leaf_hash, result_stack)) {
            result_stack.push_back(script_bytes);
            result_stack.push_back(*control_blocks.begin()); // the smallest control block
            if (smallest_result_stack.empty() || GetSerializeSize(result_stack) < GetSerializeSize(smallest_result_stack)) {
                smallest_result_stack = std::move(result_stack);
            }
        }
    }
    if (smallest_result_stack.empty()) return false;
    result = std::move(smallest_result_stack);
    return true;
}
                     ...
                     std::vector<valtype>& ret, TxoutType& whichTypeRet, SigVersion sigversion, SignatureData& sigdata)
{
    ...
    case TxoutType::WITNESS_V2_PQ:
        // xCoin legacy witness v2: OP_2 <SHA256(ML-DSA-65 pubkey)>, witness [sig, pubkey].
        return SignXcoinV2(provider, creator, uint256(vSolutions[0]), scriptPubKey, sigdata, ret);
    case TxoutType::WITNESS_V3_PQ:
        // xCoin witness v3 script tree: [leaf items..., leaf script, control block].
        return SignXcoinV3(provider, creator, uint256(vSolutions[0]), sigdata, ret);
...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    else if (solved && whichType == TxoutType::WITNESS_V0_SCRIPTHASH)
    {
        ...
        result.clear();
    } else if ((whichType == TxoutType::WITNESS_V2_PQ || whichType == TxoutType::WITNESS_V3_PQ) && !P2SH) {
        sigdata.witness = true;
        if (solved) {
            sigdata.scriptWitness.stack = std::move(result);
        }
...
public:
    ...
    bool CheckPQSig(const std::vector<unsigned char>& vchSig, const std::vector<unsigned char>& vchPubKey, const CScript& scriptCode, SigVersion sigversion) const override { return vchSig.size() != 0; }
    bool CheckPQSigV3(std::span<const unsigned char> sig, std::span<const unsigned char> pubkey, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError* serror) const override { return sig.size() != 0; }
    bool CheckSLHSigV3(std::span<const unsigned char> sig, std::span<const unsigned char> pubkey, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError* serror) const override { return sig.size() != 0; }
...
public:
    ...
    // xCoin signatures are fixed size; the dummies carry the sizes the wallet must budget for.
    ...
    {
        sig.assign(PQ_SIGNATURE_SIZE + 1, '\000'); // plus the hash-type byte
    ...
    {
        sig.assign(PQ_SIGNATURE_SIZE, '\000'); // SIGHASH_DEFAULT: no hash-type byte
        return true;
    }
    ...
    {
        sig.assign(SLH_SIGNATURE_SIZE, '\000');
        return true;
    }
};
