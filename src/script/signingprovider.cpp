...
bool HidingSigningProvider::GetPQPubKey(const CPQKeyID& keyid, CPQPubKey& pubkey) const
{
    return m_provider->GetPQPubKey(keyid, pubkey);
}
bool HidingSigningProvider::GetPQKey(const CPQKeyID& keyid, CPQKey& key) const
{
    if (m_hide_secret) return false;
    return m_provider->GetPQKey(keyid, key);
}
bool HidingSigningProvider::HavePQKey(const CPQKeyID& keyid) const
{
    if (m_hide_secret) return false;
    return m_provider->HavePQKey(keyid);
}
bool HidingSigningProvider::GetSLHKey(const CSLHPubKey& pubkey, CSLHKey& key) const
{
    if (m_hide_secret) return false;
    return m_provider->GetSLHKey(pubkey, key);
}
bool HidingSigningProvider::HaveSLHKey(const CSLHPubKey& pubkey) const
{
    if (m_hide_secret) return false;
    return m_provider->HaveSLHKey(pubkey);
}
bool HidingSigningProvider::GetXcoinV3SpendData(const uint256& root, XcoinV3SpendData& spenddata) const
{
    return m_provider->GetXcoinV3SpendData(root, spenddata);
}
bool HidingSigningProvider::GetXcoinV3Builder(const uint256& root, XcoinV3Builder& builder) const
{
    return m_provider->GetXcoinV3Builder(root, builder);
}
...
bool FlatSigningProvider::GetPQPubKey(const CPQKeyID& keyid, CPQPubKey& pubkey) const { return LookupHelper(pq_pubkeys, keyid, pubkey); }
bool FlatSigningProvider::GetPQKey(const CPQKeyID& keyid, CPQKey& key) const { return LookupHelper(pq_keys, keyid, key); }
bool FlatSigningProvider::HavePQKey(const CPQKeyID& keyid) const { return pq_keys.contains(keyid); }
bool FlatSigningProvider::GetSLHKey(const CSLHPubKey& pubkey, CSLHKey& key) const { return LookupHelper(slh_keys, pubkey, key); }
bool FlatSigningProvider::HaveSLHKey(const CSLHPubKey& pubkey) const { return slh_keys.contains(pubkey); }
bool FlatSigningProvider::GetXcoinV3SpendData(const uint256& root, XcoinV3SpendData& spenddata) const
{
    XcoinV3Builder builder;
    if (LookupHelper(xcoin_v3_trees, root, builder)) {
        spenddata = builder.GetSpendData();
        return true;
    }
    return false;
}
bool FlatSigningProvider::GetXcoinV3Builder(const uint256& root, XcoinV3Builder& builder) const
{
    return LookupHelper(xcoin_v3_trees, root, builder);
}
...
FlatSigningProvider& FlatSigningProvider::Merge(FlatSigningProvider&& b)
{
    ...
    pq_pubkeys.merge(b.pq_pubkeys);
    pq_keys.merge(b.pq_keys);
    slh_keys.merge(b.slh_keys);
    xcoin_v3_trees.merge(b.xcoin_v3_trees);
...
bool MultiSigningProvider::GetPQPubKey(const CPQKeyID& keyid, CPQPubKey& pubkey) const
{
    for (const auto& provider: m_providers) {
        if (provider->GetPQPubKey(keyid, pubkey)) return true;
    }
    return false;
}

bool MultiSigningProvider::GetPQKey(const CPQKeyID& keyid, CPQKey& key) const
{
    for (const auto& provider: m_providers) {
        if (provider->GetPQKey(keyid, key)) return true;
    }
    return false;
}

bool MultiSigningProvider::HavePQKey(const CPQKeyID& keyid) const
{
    for (const auto& provider: m_providers) {
        if (provider->HavePQKey(keyid)) return true;
    }
    return false;
}

bool MultiSigningProvider::GetSLHKey(const CSLHPubKey& pubkey, CSLHKey& key) const
{
    for (const auto& provider: m_providers) {
        if (provider->GetSLHKey(pubkey, key)) return true;
    }
    return false;
}

bool MultiSigningProvider::HaveSLHKey(const CSLHPubKey& pubkey) const
{
    for (const auto& provider: m_providers) {
        if (provider->HaveSLHKey(pubkey)) return true;
    }
    return false;
}

bool MultiSigningProvider::GetXcoinV3SpendData(const uint256& root, XcoinV3SpendData& spenddata) const
{
    for (const auto& provider: m_providers) {
        if (provider->GetXcoinV3SpendData(root, spenddata)) return true;
    }
    return false;
}

bool MultiSigningProvider::GetXcoinV3Builder(const uint256& root, XcoinV3Builder& builder) const
{
    for (const auto& provider: m_providers) {
        if (provider->GetXcoinV3Builder(root, builder)) return true;
    }
    return false;
}
...
// ── xCoin witness v3 trees ───────────────────────────────────────────────────

void XcoinV3SpendData::Merge(XcoinV3SpendData other)
{
    if (merkle_root.IsNull() && !other.merkle_root.IsNull()) {
        merkle_root = other.merkle_root;
    }
...
/*static*/ XcoinV3Builder::NodeInfo XcoinV3Builder::Combine(NodeInfo&& a, NodeInfo&& b)
{
    NodeInfo ret;
    for (auto& leaf : a.leaves) {
        leaf.merkle_branch.push_back(b.hash);
    ...
    ret.hash = ComputeXcoinBranchHash(a.hash, b.hash);
    return ret;
}

void XcoinV3Builder::Insert(XcoinV3Builder::NodeInfo&& node, int depth)
{
    assert(depth >= 0 && (size_t)depth <= XCOIN_V3_CONTROL_MAX_NODE_COUNT);
    // Same DFS invariant as TaprootBuilder::Insert.
    if ((size_t)depth + 1 < m_branch.size()) {
        m_valid = false;
        return;
    }
...
XcoinV3Builder& XcoinV3Builder::Add(int depth, std::span<const unsigned char> script, int leaf_version, bool track)
{
    assert((leaf_version & ~TAPROOT_LEAF_MASK) == 0);
    if (!IsValid()) return *this;
    NodeInfo node;
    node.hash = ComputeXcoinLeafHash(static_cast<uint8_t>(leaf_version), script);
    if (track) node.leaves.emplace_back(LeafInfo{std::vector<unsigned char>(script.begin(), script.end()), leaf_version, {}});
    Insert(std::move(node), depth);
    return *this;
}

XcoinV3Builder& XcoinV3Builder::AddOmitted(int depth, const uint256& hash)
{
    if (!IsValid()) return *this;
    NodeInfo node;
    node.hash = hash;
...
uint256 XcoinV3Builder::GetRoot() const
{
    assert(IsComplete());
    return m_branch[0]->hash;
}

WitnessV3PQ XcoinV3Builder::GetOutput() const { return WitnessV3PQ{GetRoot()}; }

XcoinV3SpendData XcoinV3Builder::GetSpendData() const
{
    assert(IsComplete());
    XcoinV3SpendData spd;
    spd.merkle_root = m_branch[0]->hash;
    for (const auto& leaf : m_branch[0]->leaves) {
        std::vector<unsigned char> control_block;
        control_block.resize(XCOIN_V3_CONTROL_BASE_SIZE + XCOIN_V3_CONTROL_NODE_SIZE * leaf.merkle_branch.size());
        // No key: the parity bit is 0 and the internal-key slot holds the fixed marker.
        control_block[0] = static_cast<unsigned char>(leaf.leaf_version);
        std::copy(XCOIN_V3_NOKEY.begin(), XCOIN_V3_NOKEY.end(), control_block.begin() + 1);
        if (leaf.merkle_branch.size()) {
            std::copy(leaf.merkle_branch[0].begin(),
                      leaf.merkle_branch[0].begin() + XCOIN_V3_CONTROL_NODE_SIZE * leaf.merkle_branch.size(),
                      control_block.begin() + XCOIN_V3_CONTROL_BASE_SIZE);
        }
        spd.scripts[{leaf.script, leaf.leaf_version}].insert(std::move(control_block));
    }
    return spd;
...
std::vector<std::tuple<uint8_t, uint8_t, std::vector<unsigned char>>> XcoinV3Builder::GetTreeTuples() const
{
    assert(IsComplete());
    std::vector<std::tuple<uint8_t, uint8_t, std::vector<unsigned char>>> tuples;
    for (const auto& leaf : m_branch[0]->leaves) {
        assert(leaf.merkle_branch.size() <= XCOIN_V3_CONTROL_MAX_NODE_COUNT);
        tuples.emplace_back((uint8_t)leaf.merkle_branch.size(), (uint8_t)leaf.leaf_version, leaf.script);
    }
    return tuples;
}

std::optional<std::vector<std::tuple<int, std::vector<unsigned char>, int>>> InferXcoinV3Tree(const XcoinV3SpendData& spenddata, const uint256& root)
{
    if (spenddata.merkle_root.IsNull() || spenddata.merkle_root != root) return std::nullopt;
    std::vector<std::tuple<int, std::vector<unsigned char>, int>> ret;

    // The same reconstruction as InferTaprootTree, with the xCoin tags and the no-key control block.
    struct TreeNode {
        uint256 hash;
        std::unique_ptr<TreeNode> sub[2];
        const std::pair<std::vector<unsigned char>, int>* leaf = nullptr;
    ...
    TreeNode tree_root;
    tree_root.hash = spenddata.merkle_root;
    for (const auto& [key, control_blocks] : spenddata.scripts) {
        const auto& [script, leaf_ver] = key;
        for (const auto& control : control_blocks) {
            if (leaf_ver < 0 || leaf_ver >= 0x100 || leaf_ver & 1) continue;
            if (control.size() < XCOIN_V3_CONTROL_BASE_SIZE || control.size() > XCOIN_V3_CONTROL_MAX_SIZE ||
                ((control.size() - XCOIN_V3_CONTROL_BASE_SIZE) % XCOIN_V3_CONTROL_NODE_SIZE) != 0) continue;
            if (control[0] != leaf_ver) continue;
            if (!std::equal(XCOIN_V3_NOKEY.begin(), XCOIN_V3_NOKEY.end(), control.begin() + 1)) continue;
            const uint256 leaf_hash = ComputeXcoinLeafHash(static_cast<uint8_t>(leaf_ver), script);
            const uint256 merkle_root = ComputeXcoinV3MerkleRoot(control, leaf_hash);
            if (merkle_root != spenddata.merkle_root) continue;

            TreeNode* node = &tree_root;
            size_t levels = (control.size() - XCOIN_V3_CONTROL_BASE_SIZE) / XCOIN_V3_CONTROL_NODE_SIZE;
            for (size_t depth = 0; depth < levels; ++depth) {
                if (node->explored && !node->inner) return std::nullopt;
                uint256 hash;
                std::copy(control.begin() + XCOIN_V3_CONTROL_BASE_SIZE + (levels - 1 - depth) * XCOIN_V3_CONTROL_NODE_SIZE,
                          control.begin() + XCOIN_V3_CONTROL_BASE_SIZE + (levels - depth) * XCOIN_V3_CONTROL_NODE_SIZE,
                          hash.begin());
                if (node->sub[0]) {
                    bool desc = false;
                    for (int i = 0; i < 2; ++i) {
    ...
    std::vector<TreeNode*> stack{&tree_root};
    while (!stack.empty()) {
        TreeNode& node = *stack.back();
        if (!node.explored) {
            return std::nullopt;
            ...
            node.done = true;
            stack.pop_back();
        } else if (!node.sub[0]->done && !node.sub[1]->done && node.sub[0]->explored && node.sub[1]->explored &&
                   node.sub[1]->hash < node.sub[0]->hash) {
            // A control block cannot tell left from right (the branch hash sorts its inputs), so
            // the first record seen landed on the left. Emit siblings in a canonical order, by
            // hash, so the inferred pqtr() string is stable; the root is unchanged either way.
            std::swap(node.sub[0], node.sub[1]);
        } else if (node.sub[0]->done && !node.sub[1]->done && !node.sub[1]->explored && !node.sub[1]->hash.IsNull() &&
                   ComputeXcoinBranchHash(node.sub[1]->hash, node.sub[1]->hash) == node.hash) {
            // Two identical subtrees under one node (see InferTaprootTree).
            node.sub[0]->done = false;
            node.sub[1]->done = true;
        } else if (node.sub[0]->done && node.sub[1]->done) {
            node.sub[0]->done = false;
