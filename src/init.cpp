...
using node::DEFAULT_ALLOW_SOLO_MINING;
...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-bind=<addr>[:<port>][=onion]", strprintf("Bind to given address and always listen on it (default: 0.0.0.0). Use [host]:port notation for IPv6. Append =onion to tag any incoming connections to that address and port as incoming Tor connections (default: 127.0.0.1:%u=onion, testnet: 127.0.0.1:%u=onion, regtest: 127.0.0.1:%u=onion)", defaultChainParams->GetDefaultPort() + 1, testnetChainParams->GetDefaultPort() + 1, regtestChainParams->GetDefaultPort() + 1), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::CONNECTION);
    ...
    argsman.AddArg("-port=<port>", strprintf("Listen for connections on <port> (default: %u, testnet: %u, regtest: %u). Not relevant for I2P (see doc/i2p.md). If set to a value x, the default onion listening port will be set to x+1.", defaultChainParams->GetDefaultPort(), testnetChainParams->GetDefaultPort(), regtestChainParams->GetDefaultPort()), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::CONNECTION);
    ...
    // Bitcoin's hard-coded checkpoints were removed. We keep `-checkpoints` as a hidden arg to display a more user friendly error when set.
    ...
    argsman.AddArg("-datacarriersize",
                   strprintf("Relay and mine transactions whose data-carrying raw scriptPubKeys in aggregate "
                             ...
                             "Independently of this policy, consensus limits the OP_RETURN scriptPubKeys of a "
                             "block to %u bytes in total",
                             MAX_OP_RETURN_RELAY, MAX_BLOCK_DATACARRIER_BYTES),
                   ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-permitbaremultisig", strprintf("Relay transactions creating non-P2SH multisig outputs (default: %u)", DEFAULT_PERMIT_BAREMULTISIG), ArgsManager::ALLOW_ANY,
                   OptionsCategory::NODE_RELAY);
    argsman.AddArg("-permitv2outputs", strprintf("Regtest only: relay and mine transactions creating new legacy witness v2 outputs (default: %u). "
                                                 "On mainnet and the rehearsal chain a new witness v2 output is consensus-invalid (bad-txout-not-pq) "
                                                 "and this option is refused; existing v2 outputs stay spendable everywhere",
                                                 DEFAULT_PERMIT_V2_OUTPUTS),
                   ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    ...
    argsman.AddArg("-allowsolomining", strprintf("Serve block templates with no peers and during initial block download: for a single-node launch or an isolated rehearsal only (default: %u)", DEFAULT_ALLOW_SOLO_MINING), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    if (args.GetBoolArg("-allowsolomining", DEFAULT_ALLOW_SOLO_MINING) &&
        ...
        InitWarning(_("-allowsolomining is set: getblocktemplate serves templates with no peers and during initial block download. Single-node launch or isolated rehearsal only; a peered node must not run with it."));
    }
    ...
    if (chain == ChainType::TESTNET) {
        LogInfo("xCoin testnet A: the v2 consensus rules on a separate dress-rehearsal chain (contrib/regenesis/TESTNET-A.md); coins here have no value\n");
    }
...

    {
        ...
        if (max_block_weight > MAX_RELAYABLE_BLOCK_WEIGHT) {
            return InitError(strprintf(_("Specified -blockmaxweight (%d) exceeds the largest block the peer-to-peer transport can carry (%d); such a block would be valid and undeliverable"), max_block_weight, MAX_RELAYABLE_BLOCK_WEIGHT));
        }
    }
