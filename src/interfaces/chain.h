...
public:
    ...
    //! xCoin: whether this node relays and mines transactions that create NEW legacy
    //! witness v2 outputs: consensus permits them (regtest only) AND the node was started
    //! with -permitv2outputs=1 (node/mempool_args.cpp). The wallet hands out and pays a
    //! witness v2 address only when this is true (audit finding T9).
    virtual bool permitV2Outputs() = 0;
