...
/** Whether an output is a data carrier: its scriptPubKey starts with OP_RETURN
 *  (xCoin consensus admits only WITNESS_V2_PQ and NULL_DATA outputs, so this is
 *  exactly the NULL_DATA set). The witness commitment output counts too. */
static inline bool IsDataCarrierOutput(const CTxOut& out)
{
    return !out.scriptPubKey.empty() && out.scriptPubKey[0] == OP_RETURN;
}
/** Total scriptPubKey bytes of a transaction's data-carrier outputs. */
static inline uint64_t GetTransactionDataCarrierBytes(const CTransaction& tx)
{
    uint64_t bytes{0};
    for (const CTxOut& out : tx.vout) {
        if (IsDataCarrierOutput(out)) bytes += out.scriptPubKey.size();
    }
    return bytes;
}
/** Total scriptPubKey bytes of the data-carrier outputs of every transaction in
 *  a block; consensus caps it at MAX_BLOCK_DATACARRIER_BYTES at every height,
 *  see ContextualCheckBlock. */
static inline uint64_t GetBlockDataCarrierBytes(const CBlock& block)
{
    uint64_t bytes{0};
    for (const CTransactionRef& tx : block.vtx) {
        bytes += GetTransactionDataCarrierBytes(*tx);
    }
    return bytes;
}
