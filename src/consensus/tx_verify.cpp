...
bool Consensus::CheckTxInputs(const CTransaction& tx, TxValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, int coinbase_maturity, CAmount& txfee)
{
...
bool Consensus::CheckSettlementLevy(const CTransaction& tx, CAmount txfee, const SettlementLevyRule& rule, TxValidationState& state)
{
    if (tx.IsCoinBase()) return true;
    // GetValueOut() cannot throw here: CheckTransaction ran before (see CheckTxInputs).
    const CAmount levy{SettlementLevy(tx.GetValueOut(), rule)};
    if (txfee < levy) {
        return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-txns-levy",
                             strprintf("fee %s below the settlement levy %s (%d bp of %s moved, capped at %s)", FormatMoney(txfee), FormatMoney(levy), rule.bp, FormatMoney(tx.GetValueOut()), FormatMoney(rule.capSat)));
    }
    return true;
}
