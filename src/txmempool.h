...
public:
    ...
    // An alias, not a derived struct: since Boost 1.92 indexed_by<...> is only
    // forward-declared (multi_index matches it as a type list), so it cannot be
    // a base class.
...
public:
     ...
     * check does nothing.
     */
    void check(const CCoinsViewCache& active_coins_tip, int64_t spendheight, int coinbase_maturity) const EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
