...
public:
    ...
    bool permitV2Outputs() override
    {
        // The policy the mempool actually runs; without a mempool, the same rule it was
        // built from (node/mempool_args.cpp).
        if (m_node.mempool) return m_node.mempool->m_opts.permit_v2_outputs;
        return chainman().GetConsensus().permitV2Outputs && args().GetBoolArg("-permitv2outputs", DEFAULT_PERMIT_V2_OUTPUTS);
    }
...
public:
    ...
    std::unique_ptr<BlockTemplate> createNewBlock(const BlockCreateOptions& options, bool cooldown) override
    {
        ...
        if (cooldown) {
            ...
            // because on regtest and single-miner chains this would wait
