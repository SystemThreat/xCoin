...
bool BlockTreeDB::LoadBlockIndexGuts(const Consensus::Params& consensusParams, std::function<CBlockIndex*(const uint256&)> insertBlockIndex, const util::SignalInterrupt& interrupt)
{
    ...
    // Upstream re-checks each entry's proof of work here against the block
    // hash it already holds: a SHA-256d compare, microseconds, a guard against
    // a corrupt index. The MetalDAG proof of work is not a hash the index
    // holds, so the same line here re-derived hashimoto_light for every entry
    // plus a verification-cache build per epoch: minutes per year of chain at
    // every start (audit finding C1). The index is trusted the way upstream
    // trusts it instead. A header is not re-hashed when this node already
    // validated it (BLOCK_VALID_TREE is raised only after CheckProofOfWork
    // passed on arrival, and is persisted with the entry) or when it lies at
    // or below the highest release checkpoint this build carries; anything
    // else is verified in full as before. Every entry keeps the cheap
    // structural check: its nBits must encode a target within powLimit.
    // Nothing changes about what is verified when a header first arrives
    // from the network or from a file.
    const int checkpoint_height{consensusParams.release_checkpoints.empty() ? -1 : consensusParams.release_checkpoints.rbegin()->first};
    size_t n_loaded{0};
    size_t n_hashed{0};
    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                ++n_loaded;
                const bool header_validated{(pindexNew->nStatus & BLOCK_VALID_MASK) >= BLOCK_VALID_TREE};
                const bool under_checkpoint{pindexNew->nHeight <= checkpoint_height};
                if (header_validated || under_checkpoint) {
                    // The genesis block is exempt from the target check as it is
                    // from CheckProofOfWork: it is the hardcoded root of trust.
                    if (!EnableFuzzDeterminism() && pindexNew->GetBlockHash() != consensusParams.hashGenesisBlock &&
                        !DeriveTarget(pindexNew->nBits, consensusParams.powLimit)) {
    ...
    LogInfo("Block index: %u entries loaded, %u headers re-hashed (MetalDAG)", n_loaded, n_hashed);
    return true;
}
