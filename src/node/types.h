...
struct BlockCreateOptions {
    ...
    /**
     * Script to put in the coinbase transaction. The default is the
     * unspendable witness v3 program of 32 zero bytes: no xCoin leaf hashes to
     * zero, so nothing can spend it, and it is a WITNESS_V3_PQ output, which is
     * the only spendable output type this chain's PQ-only rule admits
     * (consensus/tx_check.cpp, "bad-txout-not-pq"; REGENESIS.md sections 3 and
     * 4). It has to be a valid output type on EVERY chain: getblocktemplate
     * builds its template with this placeholder and runs TestBlockValidity over
     * it, so a placeholder the chain refuses makes getblocktemplate fail at
     * every height and no pool can mine. It was `OP_2 <32 zero bytes>` (witness
     * v2) until stage T2, which was refused on mainnet and on the rehearsal
     * chain once stage B5 made a new v2 output invalid there.
     *
     ...
     */
    CScript coinbase_output_script{CScript() << OP_3 << std::vector<unsigned char>(32, 0)};
    /**
