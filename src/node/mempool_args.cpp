...
util::Result<void> ApplyArgsManOptions(const ArgsManager& argsman, const CChainParams& chainparams, MemPoolOptions& mempool_opts)
{
    ...
    // -permitv2outputs is a regtest-only knob (REGENESIS.md section 4): on mainnet and the
    // rehearsal chain a new witness v2 output is consensus-invalid (bad-txout-not-pq), and on the
    // inherited test chains there is no reason to relay one. Only regtest, whose fixtures still pay
    // legacy v2 scripts, may turn it on.
    if (argsman.IsArgSet("-permitv2outputs") && chainparams.GetChainType() != ChainType::REGTEST) {
        return util::Error{Untranslated("-permitv2outputs is a regtest-only option (creating witness v2 outputs is invalid on this chain; carried balances are witness v3)")};
    }
    mempool_opts.permit_v2_outputs = chainparams.GetConsensus().permitV2Outputs &&
                                     argsman.GetBoolArg("-permitv2outputs", DEFAULT_PERMIT_V2_OUTPUTS);
