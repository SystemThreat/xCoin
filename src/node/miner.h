...
/** -allowsolomining: serve block templates on mainnet with no peers and during
 *  initial block download (a single-node launch or an isolated rehearsal only). */
static const bool DEFAULT_ALLOW_SOLO_MINING = false;
...
private:
    ...
    // OP_RETURN scriptPubKey bytes so far, incl. the coinbase reserve
    // (consensus cap MAX_BLOCK_DATACARRIER_BYTES, enforced at every height)
    uint64_t nBlockDataCarrierBytes;
    CAmount nFees;
...
private:
    ...
    */
    ...
    bool TestChunkBlockLimits(FeePerWeight chunk_feerate, int64_t chunk_sigops_cost, uint64_t chunk_datacarrier_bytes) const;
    /** Perform locktime and settlement-levy checks on each transaction in a
      * chunk. The locktime check should always succeed and is here only as an
      * extra check in case of a bug; the levy check is against the schedule row
      * for the template height, which can differ from the row the mempool
      * admitted the transaction under (audit finding C7). */
