...
static BlockAssembler::Options ClampOptions(BlockAssembler::Options options)
{
    ...
    // Limit weight to between block_reserved_weight and MAX_RELAYABLE_BLOCK_WEIGHT for sanity
    // (audit finding 8: a block above what the transport carries is valid and undeliverable):
    // block_reserved_weight can safely exceed -blockmaxweight, but the rest of the block template will be empty.
    options.nBlockMaxWeight = std::clamp<size_t>(options.nBlockMaxWeight, *options.block_reserved_weight, MAX_RELAYABLE_BLOCK_WEIGHT);
    return options;
}
...
void BlockAssembler::resetBlock()
{
    ...
    nBlockDataCarrierBytes = DEFAULT_COINBASE_DATACARRIER_RESERVE;
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock()
{
    ...
    LogInfo("CreateNewBlock(): block weight: %u txs: %u fees: %ld sigops %d datacarrier bytes %u\n", GetBlockWeight(*pblock), nBlockTx, nFees, nBlockSigOpsCost, GetBlockDataCarrierBytes(*pblock));
...
bool BlockAssembler::TestChunkBlockLimits(FeePerWeight chunk_feerate, int64_t chunk_sigops_cost, uint64_t chunk_datacarrier_bytes) const
{
    ...
    // Consensus rejects a block over MAX_BLOCK_DATACARRIER_BYTES of OP_RETURN
    // outputs ("bad-blk-datacarrier", ContextualCheckBlock) at every height.
    if (nBlockDataCarrierBytes + chunk_datacarrier_bytes > MAX_BLOCK_DATACARRIER_BYTES) {
        return false;
    }
    return true;
}
...
// - the settlement levy row in force at the template height (audit finding C7)
bool BlockAssembler::TestChunkTransactions(const std::vector<CTxMemPoolEntryRef>& txs) const
{
    // The mempool admits a transaction against the levy row for Height() + 1
    // at admission time only. A later row of the schedule (a soft fork that
    // raises the rate or the cap) would leave transactions admitted under the
    // old row in the mempool; ConnectBlock enforces the row for nHeight, so
    // including one would make CreateNewBlock's own TestBlockValidity throw
    // ("bad-txns-levy") until it expired. Re-check every transaction against
    // the row this block is subject to and skip the chunk instead.
    const Consensus::SettlementLevyRule levy_rule{chainparams.GetConsensus().SettlementLevyAt(nHeight)};
    for (const auto tx : txs) {
        if (!IsFinalTx(tx.get().GetTx(), nHeight, m_lock_time_cutoff)) {
            return false;
        }
        TxValidationState levy_state;
        if (!Consensus::CheckSettlementLevy(tx.get().GetTx(), tx.get().GetFee(), levy_rule, levy_state)) {
            return false;
        }
    }
    return true;
...
void BlockAssembler::AddToBlock(const CTxMemPoolEntry& entry)
{
    ...
    nBlockDataCarrierBytes += GetTransactionDataCarrierBytes(entry.GetTx());
    nFees += entry.GetFee();
...
void BlockAssembler::addChunks()
{
    ...
    while (selected_transactions.size() > 0) {
        ...
        uint64_t chunk_datacarrier_bytes = 0;
        for (const auto& tx : selected_transactions) {
            chunk_sig_ops += tx.get().GetSigOpCost();
            chunk_datacarrier_bytes += GetTransactionDataCarrierBytes(tx.get().GetTx());
        }

        // Check to see if this chunk will fit.
        if (!TestChunkBlockLimits(chunk_feerate, chunk_sig_ops, chunk_datacarrier_bytes) || !TestChunkTransactions(selected_transactions)) {
