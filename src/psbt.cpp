...
PSBTError SignPSBTInput(const SigningProvider& provider, PartiallySignedTransaction& psbt, int index, const PrecomputedTransactionData* txdata, std::optional<int> sighash,  SignatureData* out_sigdata, bool finalize)
{
    ...
    // xCoin witness v3 signatures encode SIGHASH_DEFAULT like Taproot (no hash-type byte).
    const bool default_sighash_zero = utxo.scriptPubKey.IsPayToTaproot() || utxo.scriptPubKey.IsPayToXcoinV3();
    if (!sighash) sighash = default_sighash_zero ? SIGHASH_DEFAULT : SIGHASH_ALL;
    ...
    if (default_sighash_zero ? sighash != SIGHASH_DEFAULT :
