...
/** Audit finding 2: at most one MetalDAG cache build for a cold epoch per this interval, across all peers. */
static constexpr std::chrono::seconds COLD_EPOCH_BUILD_INTERVAL{30};
...
/** Window, in blocks, for connecting to NODE_NETWORK_LIMITED peers: they are
 *  desirable while our tip is under 24 hours old (GetDesirableServiceFlags, via
 *  ApproximateBestBlockDepth, which divides the tip age by nPowTargetSpacing).
 *  Bitcoin's 144 was one day of 600 s blocks; at this chain's 300 s spacing a
 *  day is 288 blocks (audit finding T4). P2P policy, not consensus. */
static const unsigned int NODE_NETWORK_LIMITED_ALLOW_CONN_BLOCKS = 288;
...
private:
    ...
    */
    ...
    /** Whether verifying these headers' MetalDAG proof of work on behalf of a peer is
     *  affordable now (audit finding 2). A header whose epoch cache is resident, or whose
     *  epoch is next to the best header or the wall clock, always is; any other epoch draws
     *  on m_cold_cache_budget, one build per interval across all peers. False means "drop
     *  the message, retry later", never misbehaviour: an honest deep fork is rare and waits. */
    bool MayVerifyHeadersNow(std::span<const CBlockHeader> headers) EXCLUSIVE_LOCKS_REQUIRED(!m_cold_cache_mutex);
    Mutex m_cold_cache_mutex;
    metaldag::CacheBuildBudget m_cold_cache_budget GUARDED_BY(m_cold_cache_mutex){COLD_EPOCH_BUILD_INTERVAL};
...
bool PeerManagerImpl::MayVerifyHeadersNow(std::span<const CBlockHeader> headers)
{
    uint32_t best_header_time{0};
    {
        LOCK(cs_main);
        if (m_chainman.m_best_header) best_header_time = m_chainman.m_best_header->nTime;
    }
    const int64_t now{GetTime()};
    LOCK(m_cold_cache_mutex);
    for (const CBlockHeader& header : headers) {
        if (!metaldag::MayVerifyFromNetwork(header, m_chainparams.GetConsensus(), best_header_time, now, m_cold_cache_budget)) return false;
    }
    return true;
}
                                            ...
                                            bool via_compact_block)
{
    ...
    // Cheap check first. MetalDAG verification costs ~2.5 ms per header plus a
    // cache build at an epoch boundary — three orders of magnitude more than the
    // SHA256d check upstream could afford to run before anything else. So before
    // spending that, ask the one question that needs no hashing: do these headers
    // connect to a block we know? If not, and this peer is not mid headers-sync
    // (where headers chain off the sync's own state rather than our index), treat
    // them as a possibly-benign announcement without verifying proof of work at
    // all. HandleUnconnectingHeaders only sends a rate-limited getheaders and
    // records availability; it does not rely on PoW-valid headers. The cost of an
    // unconnected junk header drops from milliseconds to a map lookup. Headers
    // that DO connect, and any header entering HeadersSyncState, are still
    // PoW-checked immediately below.
    {
        const bool in_headers_sync{WITH_LOCK(peer.m_headers_sync_mutex, return !!peer.m_headers_sync)};
        if (!in_headers_sync) {
            const bool connects{WITH_LOCK(::cs_main, return m_chainman.m_blockman.LookupBlockIndex(headers[0].hashPrevBlock) != nullptr)};
            if (!connects) {
    ...
    // MetalDAG verification is memory-hard: a header naming an epoch whose cache is
    // not resident costs a cache build (tens of MiB, seconds). Headers next to the
    // best header or the clock are always verified; any other epoch is rationed
    // (audit finding 2). Dropped, not punished: a real deep fork is retried later.
    if (!MayVerifyHeadersNow(headers)) {
        LogDebug(BCLog::NET, "dropping %u headers from peer=%d: cold MetalDAG epoch, build budget exhausted\n", headers.size(), pfrom.GetId());
        return;
    }
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        // An unrequested block runs the memory-hard PoW check in CheckBlock before any
        // anti-DoS gate (audit finding 7). One whose parent we do not know can never be
        // accepted (prev-blk-not-found) and is not hashed at all; one naming a cold epoch
        // draws on the same build budget as headers. Requested blocks are never held back.
        if (!forceProcessing) {
            if (!prev_block) {
                // Upstream reaches the same verdict (BLOCK_MISSING_PREV, punished) only after
                // hashing the block; here the verdict comes first and the hash never happens.
                LogDebug(BCLog::NET, "unrequested block %s from peer=%d has an unknown parent: not hashed\n", hash.ToString(), pfrom.GetId());
                Misbehaving(peer, "unrequested block with unknown parent");
                return;
            }
            const CBlockHeader& unrequested_header{*pblock};
            if (!MayVerifyHeadersNow(std::span<const CBlockHeader>{&unrequested_header, 1})) {
                LogDebug(BCLog::NET, "ignoring unrequested block %s from peer=%d: cold MetalDAG epoch, build budget exhausted\n", hash.ToString(), pfrom.GetId());
                return;
            }
        }
        {
