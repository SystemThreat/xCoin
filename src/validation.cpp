    ...
    const auto filter_final_and_mature = [&](CTxMemPool::txiter it)
        ...
        if (it->GetSpendsCoinbase()) {
            for (const CTxIn& txin : tx.vin) {
                ...
                if (coin.IsCoinBase() && mempool_spend_height - coin.nHeight < m_chainman.GetConsensus().coinbaseMaturity) {
                    return true;
                }
            }
        }
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckTransaction(tx, state, m_active_chainstate.m_chainman.GetConsensus().permitV2Outputs,
                          m_active_chainstate.m_chainman.GetConsensus().minOutputValueSat)) {
    ...
    std::string reason;
    if (m_pool.m_opts.require_standard && !IsStandardTx(tx, m_pool.m_opts.max_datacarrier_bytes, m_pool.m_opts.permit_bare_multisig, m_pool.m_opts.permit_v2_outputs, m_pool.m_opts.dust_relay_feerate, reason)) {
    ...
    if (!Consensus::CheckTxInputs(tx, state, m_view, m_active_chainstate.m_chain.Height() + 1, m_active_chainstate.m_chainman.GetConsensus().coinbaseMaturity, ws.m_base_fees)) {
        return false; // state filled in by CheckTxInputs
    }

    // Settlement levy (REGENESIS.md section 6): consensus, so a transaction
    // that cannot be mined is never admitted (the mempool's own per-byte floor
    // is checked separately below).
    if (!Consensus::CheckSettlementLevy(tx, ws.m_base_fees, m_active_chainstate.m_chainman.GetConsensus().SettlementLevyAt(m_active_chainstate.m_chain.Height() + 1), state)) {
        return false; // state filled in by CheckSettlementLevy
    }
...
{
    // xCoin (XID) emission (contrib/regenesis/REGENESIS.md section 2, consensus/params.h):
    // block 0 mints nothing; from block 1 every block pays the era subsidy from this
    // chain's table (mainnet: EMISSION_TABLE, generated by contrib/regenesis/emission.py;
    // FINAL shape the Annual Tenth, charter section 4: 65 rows to block 35,375,353) [EMISSION-SHAPE]
    // to EmissionEndHeight(). That block alone additionally mints the closing
    // remainder (exactly once) that makes sum(era subsidies) + dust == MAX_SUPPLY_SAT
    // exact. After it the subsidy is zero: fees only. There is no premine and nothing
    // is carried in. O(eras): 65 on mainnet.
    if (nHeight < Consensus::EMISSION_START_HEIGHT) return 0;
    for (const Consensus::EmissionEra& era : consensusParams.emissionTable) {
        if (nHeight > era.endHeight) continue;
        if (nHeight < era.startHeight) return 0; // a gap in a malformed table mints nothing
        CAmount subsidy{era.baseSubsidy};
        if (nHeight == consensusParams.EmissionEndHeight()) subsidy += consensusParams.emissionClosingDustSat;
        return subsidy;
    }
...
script_verify_flags GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    // violating blocks. xCoin witness v3 is part of the genesis rule set on
    // every chain, so it is always on as well.
    script_verify_flags flags{SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_TAPROOT | SCRIPT_VERIFY_XCOIN_V3};
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!Consensus::CheckTxInputs(tx, tx_state, view, pindex->nHeight, m_chainman.GetConsensus().coinbaseMaturity, txfee)) {
            ...
            // Settlement levy (REGENESIS.md section 6): the fee must reach
            // levy_bp of the value moved; paid to this block's miner below.
            if (!Consensus::CheckSettlementLevy(tx, txfee, params.GetConsensus().SettlementLevyAt(pindex->nHeight), tx_state)) {
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                              tx_state.GetRejectReason(),
                              tx_state.GetDebugMessage() + " in transaction " + tx.GetHash().ToString());
                break;
    ...
    // Every height, block 1 included: subsidy plus fees (there is no block-1 distribution).
...
CBlockIndex* Chainstate::FindMostWorkChain()
{
    ...
    do {
        ...
        {
            std::set<CBlockIndex*, node::CBlockIndexWorkComparator>::reverse_iterator it = setBlockIndexCandidates.rbegin();
...
bool Chainstate::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, std::vector<ConnectedBlock>& connected_blocks)
{
    ...
    if (m_mempool) m_mempool->check(this->CoinsTip(), this->m_chain.Height() + 1, m_chainman.GetConsensus().coinbaseMaturity);
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // The xCoin output rule (REGENESIS.md sections 3 and 4) takes the chain's
    // setting, except for the genesis block: its zero-value witness v2 marker
    // output is part of the chain's permanent identity and predates the rule.
    // The output-value floor (Consensus::MIN_OUTPUT_VALUE_SAT) exempts the genesis
    // block for the same reason.
    const bool is_genesis{block.GetHash() == consensusParams.hashGenesisBlock};
    const bool permit_v2_outputs{consensusParams.permitV2Outputs || is_genesis};
    const int64_t min_output_value{is_genesis ? int64_t{0} : consensusParams.minOutputValueSat};
    for (const auto& tx : block.vtx) {
        TxValidationState tx_state;
        if (!CheckTransaction(*tx, tx_state, permit_v2_outputs, min_output_value)) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // BIP94 chains only: Check timestamp against prev for difficulty-adjustment
    ...
    // Multi-algorithm proof of work, reserved (REGENESIS.md section 7): the
    // PoW-algorithm id in nVersion bits 28..29 must be 0 (MetalDAG) until
    // multi_algo_activation_height, which is 0 (inactive) on every chain today.
    if (const uint32_t algo{Consensus::GetPowAlgoId(block.nVersion)}; algo != Consensus::POW_ALGO_METALDAG && !consensusParams.MultiAlgoActiveAt(nHeight)) {
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-pow-algo",
                             strprintf("PoW algorithm id %u in nVersion=0x%08x before multi-algorithm activation", algo, block.nVersion));
    }

    // Release checkpoints (REGENESIS.md section 6): block hashes compiled into
    // this build. No key, no signer, nothing transmissible — the binary states
    // which block it saw at a height and refuses a header that disagrees. This
    // is what protects a node syncing FROM SCRATCH, which nothing else here can:
    // it holds no tip of its own to defend, and nMinimumChainWork cannot bind
    // until the chain has real depth. A map lookup that almost always misses.
    if (!consensusParams.ReleaseCheckpointAllows(nHeight, block.GetHash())) {
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-checkpoint-release",
                             strprintf("header %s at height %d is not the block this release checkpoints (%s)",
                                       block.GetHash().ToString(), nHeight,
                                       consensusParams.release_checkpoints.at(nHeight).ToString()));
    }
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    // Enforce BIP113 (Median Time Past). Guard on pindexPrev: Xcoin activates CSV
    // at genesis (CSVHeight=0), so DeploymentActiveAfter() is true even for the
    // genesis block (pindexPrev == nullptr), which is re-validated during -reindex.
    // Genesis has no previous block and thus no median-time-past to enforce.
    ...
    // Enforce rule that the coinbase starts with serialized block height. Never on
    // the genesis block (pindexPrev == nullptr): its coinbase is the charter message,
    // and a chain whose BIP34 height were 0 would reject its own genesis on -reindex
    // and then assert on a null tip (audit finding 1).
    if (pindexPrev != nullptr && DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_HEIGHTINCB))
    {
    ...
    // xCoin data-carrier cap (contrib/regenesis/REGENESIS.md section 5): the
    // scriptPubKey bytes of every OP_RETURN output, summed over all outputs of
    // all transactions in the block, may not exceed MAX_BLOCK_DATACARRIER_BYTES.
    // Every height, block 1 included: nothing is inscribed on this chain.
    if (const uint64_t datacarrier_bytes{GetBlockDataCarrierBytes(block)}; datacarrier_bytes > MAX_BLOCK_DATACARRIER_BYTES) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-datacarrier",
                             strprintf("%s : data carrier limit failed (%d bytes of OP_RETURN outputs, limit %d)", __func__, datacarrier_bytes, MAX_BLOCK_DATACARRIER_BYTES));
    }
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        // The proof of work is memory-hard and its cost follows the epoch the header names,
        // so the clock-relative time-too-new rule runs BEFORE it here (audit finding 6): a
        // header more than MAX_FUTURE_BLOCK_TIME ahead can never be accepted, is not hashed,
        // and fails with the non-punishable BLOCK_TIME_FUTURE rather than "high-hash".
        // Headers and blocks read back from disk never pass through here.
        if (block.Time() > NodeClock::now() + std::chrono::seconds{MAX_FUTURE_BLOCK_TIME}) {
            return state.Invalid(BlockValidationResult::BLOCK_TIME_FUTURE, "time-too-new", "block timestamp too far in the future");
        }
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        bool ret = block->Time() <= NodeClock::now() + std::chrono::seconds{MAX_FUTURE_BLOCK_TIME};
        if (!ret) {
            // Audit finding 6: the time-too-new rule before the memory-hard proof of work.
...
MempoolAcceptResult ChainstateManager::ProcessTransaction(const CTransactionRef& tx, bool test_accept)
{
    ...
    active_chainstate.GetMempool()->check(active_chainstate.CoinsTip(), active_chainstate.m_chain.Height() + 1, active_chainstate.m_chainman.GetConsensus().coinbaseMaturity);
    return result;
}
    ...
    const bool check_merkle_root)
{
    ...
    if (check_pow && block.Time() > NodeClock::now() + std::chrono::seconds{MAX_FUTURE_BLOCK_TIME}) {
        state.Invalid(BlockValidationResult::BLOCK_TIME_FUTURE, "time-too-new", "block timestamp too far in the future");
        return state;
    }
