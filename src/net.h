 ...
 *  Sized to the consensus block ceiling (MAX_BLOCK_SERIALIZED_SIZE, 64 MB), but note
 *  what the transports actually carry: v1 refuses anything over MAX_SIZE (32 MiB) and
 *  BIP324 v2 cannot frame more than 16,777,215 bytes. That is why the mining policy
 *  stops at MAX_RELAYABLE_BLOCK_WEIGHT (policy.h, audit finding 8): a block between
 *  16 MB and 64 MB would be consensus-valid and undeliverable. This is also the buffer
 *  a single peer can make this node allocate. */
