...
public:
    ...
    std::string operator()(const WitnessV2PQ& id) const
    {
    ...
    std::string operator()(const WitnessV3PQ& id) const
    {
        std::vector<unsigned char> data = {3}; // witness version 3 (bech32 'r'): xpa1r...
        data.reserve(53);
        ConvertBits<8, 5, true>([&](unsigned char c) { data.push_back(c); }, id.begin(), id.end());
        return bech32::Encode(bech32::Encoding::BECH32M, m_params.Bech32HRP(), data);
    }
...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    if (dec.encoding == bech32::Encoding::BECH32 || dec.encoding == bech32::Encoding::BECH32M) {
        ...
        if (ConvertBits<5, 8, false>([&](unsigned char c) { data.push_back(c); }, dec.data.begin() + 1, dec.data.end())) {
            ...
            // Witness v2: PQ address (32-byte SHA-256 hash of ML-DSA-65 pubkey)
            if (version == 2 && data.size() == 32) {
                WitnessV2PQ pq;
                std::copy(data.begin(), data.end(), pq.begin());
                // On a chain that admits only witness v3 outputs (mainnet, testnet A), a v2
                // address names an output nobody can create, so it is refused wherever an
                // address is parsed.
                if (!params.GetConsensus().permitV2Outputs) {
                    error_str = "Legacy witness v2 address: this chain accepts only witness v3 (post-quantum script tree) addresses";
                    return CNoDestination();
                }
                return pq;
            }

            // Witness v3: PQ script-tree address (32-byte Merkle root)
            if (version == 3 && data.size() == WITNESS_V3_SIZE) {
                static_assert(WITNESS_V3_SIZE == uint256::size());
                WitnessV3PQ v3;
                std::copy(data.begin(), data.end(), v3.begin());
                return v3;
            }
