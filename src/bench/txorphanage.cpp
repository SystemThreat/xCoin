...
static void OrphanageEraseAll(benchmark::Bench& bench, bool block_or_disconnect)
{
    ...
    // All the block inputs are spent by the orphanage transactions: each peer's
    // NUM_TXNS_PER_PEER transactions of INPUTS_PER_TX inputs each step through
    // its slice by INPUT_STRIDE and between them cover it. The stride is derived
    // rather than hard-coded because xCoin's MAX_BLOCK_WEIGHT is 16,000,000
    // (REGENESIS.md section 0), four times Bitcoin's, so a peer's slice is four
    // times as long; the formula reproduces upstream's 7 at a 4,000,000-weight
    // block and gives 28 here.
    constexpr unsigned int INPUT_STRIDE{(INPUTS_PER_PEER - INPUTS_PER_TX + NUM_TXNS_PER_PEER - 1) / (NUM_TXNS_PER_PEER - 1)};
    static_assert(INPUTS_PER_PEER > INPUTS_PER_TX && NUM_TXNS_PER_PEER > 1);
    static_assert(INPUT_STRIDE * (NUM_TXNS_PER_PEER - 1) + INPUTS_PER_TX >= INPUTS_PER_PEER);
