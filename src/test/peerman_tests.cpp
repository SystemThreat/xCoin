...
/** Window, in blocks, for connecting to NODE_NETWORK_LIMITED peers: mirrors
 *  net_processing.cpp, 24 hours of this chain's 300 s blocks (audit finding T4). */
static constexpr int64_t NODE_NETWORK_LIMITED_ALLOW_CONN_BLOCKS = 288;
...
static void mineBlock(const node::NodeContext& node, std::chrono::seconds block_time)
{
    ...
    while (!CheckProofOfWork(block, block.nBits, node.chainman->GetConsensus())) ++block.nNonce;
