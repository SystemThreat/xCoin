...
BOOST_AUTO_TEST_CASE(tx_valid)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        if (test[0].isArray())
        {
            ...
            if (!CheckTransaction(tx, state, /*permit_v2_outputs=*/true)) {
                // The upstream vectors carry legacy outputs, which NEX's
                // PQ-only output rule rejects; that must be the only reason
                // this sanity check fails.
                BOOST_CHECK_MESSAGE(state.GetRejectReason() == "bad-txout-not-pq", strTest);
            }
...
BOOST_AUTO_TEST_CASE(tx_invalid)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        if (test[0].isArray())
        {
            ...
            TxValidationState state;
            if (!CheckTransaction(tx, state, /*permit_v2_outputs=*/true) || state.IsInvalid()) {
                // Legacy outputs in the upstream vectors fail NEX's PQ-only
                // output rule before any other CheckTransaction check can run,
                // so a bad-txout-not-pq rejection on a vector NOT marked BADTX
                // just means "legacy outputs": fall through to the script-level
                // invalidity checks those vectors are actually about. BADTX
                // vectors stay structurally invalid either way.
                if (state.GetRejectReason() != "bad-txout-not-pq" || test[2].get_str() == "BADTX") {
...
BOOST_AUTO_TEST_CASE(tx_no_inputs)
{
    ...
    TxValidationState state;
    BOOST_CHECK_MESSAGE(!CheckTransaction(CTransaction(empty), state, /*permit_v2_outputs=*/true), "Transaction with no inputs should be invalid.");
...
BOOST_AUTO_TEST_CASE(tx_oversized)
{
    ...
    {
        TxValidationState state;
        CheckTransaction(createTransaction(maxPayloadSize), state, /*permit_v2_outputs=*/true);
    ...
    {
        TxValidationState state;
        BOOST_CHECK_MESSAGE(!CheckTransaction(createTransaction(maxPayloadSize), state, /*permit_v2_outputs=*/true), "Oversized transaction should be invalid");
        BOOST_CHECK(state.GetRejectReason() == "bad-txns-oversize");
    }
}
...
BOOST_AUTO_TEST_CASE(basic_transaction_tests)
{
    ...
    TxValidationState state;
    // The transaction pays legacy P2PKH outputs, so the PQ-only output rule is
    // the one check it may fail on NEX.
    BOOST_CHECK_MESSAGE((CheckTransaction(CTransaction(tx), state, /*permit_v2_outputs=*/true) && state.IsValid()) ||
                            state.GetRejectReason() == "bad-txout-not-pq",
                        "Simple deserialized transaction should be valid.");

    // Check that duplicate txins fail
    tx.vin.push_back(tx.vin[0]);
    BOOST_CHECK_MESSAGE(!CheckTransaction(CTransaction(tx), state, /*permit_v2_outputs=*/true) || !state.IsValid(), "Transaction with duplicate txins should be invalid.");
}
...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    constexpr auto CheckIsStandard = [](const auto& t, const unsigned int max_op_return_relay = MAX_OP_RETURN_RELAY) {
        std::string reason;
        BOOST_CHECK(IsStandardTx(CTransaction{t}, max_op_return_relay, g_bare_multi, /*permit_v2_outputs=*/true, g_dust, reason));
    ...
    // xCoin default (REGENESIS.md section 5): an 80-byte TxoutType::NULL_DATA
    // scriptPubKey is standard, 81 is not. The limit is on the raw script, so
    // 77 bytes of data (OP_RETURN + OP_PUSHDATA1 + length + data = 80) fit and
    // 78 do not.
    BOOST_CHECK_EQUAL(MAX_OP_RETURN_RELAY, 80U);
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(77, 0x58);
    BOOST_CHECK_EQUAL(80, t.vout[0].scriptPubKey.size());
    CheckIsStandard(t);
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(78, 0x58);
    BOOST_CHECK_EQUAL(81, t.vout[0].scriptPubKey.size());
    CheckIsNotStandard(t, "datacarrier");
    // A direct push (up to 75 bytes, one opcode byte) is counted the same way:
    // 75 bytes of data is a 77-byte script, standard.
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(75, 0x58);
    BOOST_CHECK_EQUAL(77, t.vout[0].scriptPubKey.size());
    CheckIsStandard(t);
    ...
    // Multiple TxoutType::NULL_DATA are permitted, as long as their aggregate
    // size is within the limit: two 42-byte outputs are 84 bytes, over the
    // 80-byte xCoin default and standard only with a limit of at least 84.
    t.vout.resize(2);
    ...
    t.vout[1].nValue = 0;
    BOOST_CHECK_EQUAL(84, t.vout[0].scriptPubKey.size() + t.vout[1].scriptPubKey.size());
    CheckIsNotStandard(t, "datacarrier");
    CheckIsStandard(t, /*max_op_return_relay=*/84);
    CheckIsNotStandard(t, "datacarrier", /*max_op_return_relay=*/83);
    // Two 40-byte outputs are exactly 80 bytes: standard by default.
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(38, 0x58);
    t.vout[1].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(38, 0x58);
    BOOST_CHECK_EQUAL(80, t.vout[0].scriptPubKey.size() + t.vout[1].scriptPubKey.size());
    CheckIsStandard(t);
    t.vout[1].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(39, 0x58);
    CheckIsNotStandard(t, "datacarrier");
    ...
    CheckIsNotStandard(t, "datacarrier"); // 166 bytes, over the 80-byte default
...
BOOST_AUTO_TEST_CASE(checktxinputs_invalid_transactions_test)
{
    auto check_invalid{[](CAmount input_value, CAmount output_value, bool coinbase, int spend_height, TxValidationResult expected_result, std::string_view expected_reason) {
        ...
        TxValidationState state;
        CAmount txfee{0};
        BOOST_CHECK(!Consensus::CheckTxInputs(CTransaction{mtx}, state, inputs, spend_height, COINBASE_MATURITY, txfee));
...
BOOST_AUTO_TEST_CASE(spends_witness_prog)
{
    ...
    // WitnessV1Taproot, WitnessV2PQ, WitnessV3PQ, PayToAnchor, WitnessUnknown.
    ...
    BOOST_CHECK(GetScriptForDestination(WitnessV3PQ{uint256{program}}) == GetScriptForDestination(WitnessUnknown{3, program}));
    for (int i{2}; i <= 16; ++i) {
        tx_create.vout[0].scriptPubKey = GetScriptForDestination(WitnessUnknown{i, program});
        // Witness v2 (single ML-DSA key) and v3 (PQ script tree) with a 32-byte
        // program are xCoin's defined PQ output types, not unknown witness versions.
        BOOST_CHECK_EQUAL(Solver(tx_create.vout[0].scriptPubKey, sol_dummy),
                          i == 2 ? TxoutType::WITNESS_V2_PQ : i == 3 ? TxoutType::WITNESS_V3_PQ : TxoutType::WITNESS_UNKNOWN);
        tx_spend.vin[0].prevout.hash = tx_create.GetHash();
        AddCoins(coins, CTransaction{tx_create}, 0, false);
        BOOST_CHECK(::SpendsNonAnchorWitnessProg(CTransaction{tx_spend}, coins));
