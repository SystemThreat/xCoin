...
BOOST_AUTO_TEST_CASE(MempoolSizeLimitTest)
{
    ...
    TryAddToMempool(pool, entry.Fee(5000LL).FromTx(tx2));
    ...
    TryAddToMempool(pool, entry.Fee(20000LL).FromTx(tx3));
    ...
    TryAddToMempool(pool, entry.Fee(7000LL).FromTx(tx4));
    auto usage_with_tx4_only = pool.DynamicMemoryUsage();
    TryAddToMempool(pool, entry.Fee(1000LL).FromTx(tx5));
    TryAddToMempool(pool, entry.Fee(1100LL).FromTx(tx6));
    TryAddToMempool(pool, entry.Fee(9000LL).FromTx(tx7));
