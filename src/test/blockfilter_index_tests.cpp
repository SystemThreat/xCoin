...
BOOST_FIXTURE_TEST_CASE(blockfilter_index_initial_sync, BuildChainTestingSetup)
{
    ...
    // Only WITNESS_V2_PQ outputs are consensus-valid on NEX; the fork coinbases
    // are never spent, so any two distinct witness programs do.
    CScript coinbase_script_pub_key_A = GetScriptForDestination(WitnessV2PQ(m_rng.rand256()));
    CScript coinbase_script_pub_key_B = GetScriptForDestination(WitnessV2PQ(m_rng.rand256()));
...
BOOST_FIXTURE_TEST_CASE(index_reorg_crash, BuildChainTestingSetup)
{
    ...
    BOOST_REQUIRE(BuildChain(prev_tip, GetScriptForDestination(WitnessV2PQ(m_rng.rand256())), 3, fork));
