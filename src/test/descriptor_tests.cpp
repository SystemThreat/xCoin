...
//! Rewrite upstream Bitcoin base58check strings embedded in a descriptor (or
//! expected error message) for NEX's version bytes: xprv/xpub, WIF secrets,
//! P2PKH and P2SH addresses. If the string carried a descriptor checksum that
//! was valid for the original body, refresh it for the rewritten body.
//! Everything that does not base58check-decode is left untouched, so the
//! intentionally-invalid vectors stay invalid.
std::string AdaptForNEX(const std::string& in)
{
    static const std::vector<unsigned char> BTC_XPUB{0x04, 0x88, 0xB2, 0x1E};
    static const std::vector<unsigned char> BTC_XPRV{0x04, 0x88, 0xAD, 0xE4};
    const auto& params = Params();
    const auto adapt_token = [&](const std::string& tok) -> std::string {
        std::vector<unsigned char> data;
        if (tok.size() < 26 || tok.size() > 120 || !DecodeBase58Check(tok, data, 100) || data.empty()) return tok;
        std::vector<unsigned char> out;
        if (data.size() >= 4 && std::equal(BTC_XPUB.begin(), BTC_XPUB.end(), data.begin())) {
            out = params.Base58Prefix(CChainParams::EXT_PUBLIC_KEY);
            out.insert(out.end(), data.begin() + 4, data.end());
        } else if (data.size() >= 4 && std::equal(BTC_XPRV.begin(), BTC_XPRV.end(), data.begin())) {
            out = params.Base58Prefix(CChainParams::EXT_SECRET_KEY);
            out.insert(out.end(), data.begin() + 4, data.end());
        } else if (data[0] == 128) {
            out = params.Base58Prefix(CChainParams::SECRET_KEY);
            out.insert(out.end(), data.begin() + 1, data.end());
        } else if (data[0] == 0) {
            out = params.Base58Prefix(CChainParams::PUBKEY_ADDRESS);
            out.insert(out.end(), data.begin() + 1, data.end());
        } else if (data[0] == 5) {
            out = params.Base58Prefix(CChainParams::SCRIPT_ADDRESS);
            out.insert(out.end(), data.begin() + 1, data.end());
        } else {
            return tok;
        }
    ...
    static const std::string b58_chars{"123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"};
    std::string body{in}, suffix;
    if (const auto pos = in.find('#'); pos != std::string::npos) {
        body = in.substr(0, pos);
        suffix = in.substr(pos);
    }
    bool changed{false};
    // Re-encode Bitcoin-mainnet bech32/bech32m addresses under the NEX HRP.
    {
        static const std::regex bech32_regex{"bc1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]{6,}"};
        std::string rewritten;
        auto begin = std::sregex_iterator(body.begin(), body.end(), bech32_regex);
        size_t last = 0;
        for (auto it = begin; it != std::sregex_iterator(); ++it) {
            const auto dec = bech32::Decode(it->str());
            if (dec.hrp != "bc" || dec.data.empty()) continue;
            rewritten += body.substr(last, it->position() - last);
            rewritten += bech32::Encode(dec.encoding, Params().Bech32HRP(), dec.data);
    ...
    const auto flush = [&] {
        ...
        std::string adapted{adapt_token(token)};
        changed |= adapted != token;
        out += adapted;
        token.clear();
    };
    for (const char c : body) {
        if (b58_chars.find(c) != std::string::npos) token += c; else { flush(); out += c; }
    }
    flush();
    if (!suffix.empty()) {
        if (changed && suffix.size() == 9 && GetDescriptorChecksum(body) == suffix.substr(1)) {
            suffix = "#" + GetDescriptorChecksum(out);
        }
        out += suffix;
    }
    return out;
...
void CheckUnparsable(const std::string& prv_in, const std::string& pub_in, const std::string& expected_error_in)
{
    const std::string prv{AdaptForNEX(prv_in)}, pub{AdaptForNEX(pub_in)}, expected_error{AdaptForNEX(expected_error_in)};
...
// Count the number of extended pubkeys in a descriptor string. NEX's
// EXT_PUBLIC_KEY version bytes make every serialized xpub start with "v28X"
// (upstream counted the literal "xpub").
static size_t CountXpubs(const std::string& desc)
{
    size_t count = 0;
    size_t p = desc.find("v28X", 0);
    while (p != std::string::npos) {
        count++;
        p = desc.find("v28X", p + 1);
    }
    return count;
}
//
...
{
    // NEX-serialized xpubs all start with "v28X" (see CountXpubs above)
    std::regex xpub_regex("v28X\\w+?(?=/)");
...
void DoCheck(std::string prv, std::string pub, const std::string& norm_pub_in, int flags,
             ...
             std::optional<std::string> expected_prv = std::nullopt, std::optional<std::string> expected_pub = std::nullopt, int desc_index = 0)
{
    prv = AdaptForNEX(prv);
    pub = AdaptForNEX(pub);
    const std::string norm_pub_adapted{AdaptForNEX(norm_pub_in)};
    const std::string& norm_pub{norm_pub_adapted};
    if (expected_prv) expected_prv = AdaptForNEX(*expected_prv);
    if (expected_pub) expected_pub = AdaptForNEX(*expected_pub);
    FlatSigningProvider keys_priv, keys_pub;
...
{
    const std::string pub{AdaptForNEX(pub_in)};
...
void CheckInferDescriptor(const std::string& script_hex, const std::string& expected_desc_in, const std::vector<std::string>& hex_scripts, const std::vector<std::pair<std::string, std::string>>& origin_pubkeys)
{
    const std::string expected_desc{AdaptForNEX(expected_desc_in)};
...
BOOST_AUTO_TEST_CASE(descriptor_test)
{
    ...
    // Checksums
    Check("sh(multi(2,[00000000/111'/222]xprvA1RpRA33e1JQ7ifknakTFpgNXPmW2YvmhqLQYMmrj4xJXXWYpDPS3xz7iAxn8L39njGVyuoseXzU6rcxFLJ8HFsTjSyQbLYnMpCqE2VbFWc,xprv9uPDJpEQgRQfDcW7BkF7eTya6RPxXeJCqCJGHuCJ4GiRVLzkTXBAJMu2qaMWPrS7AANYqdq6vcBcBUdJCVVFceUvJFjaPdGZ2y9WACViL4L/0))#ggrsrxfy", "sh(multi(2,[00000000/111'/222]xpub6ERApfZwUNrhLCkDtcHTcxd75RbzS1ed54G1LkBUHQVHQKqhMkhgbmJbZRkrgZw4koxb5JaHWkY4ALHY2grBGRjaDMzQLcgJvLJuZZvRcEL,xpub68NZiKmJWnxxS6aaHmn81bvJeTESw724CRDs6HbuccFQN9Ku14VQrADWgqbhhTHBaohPX4CjNLf9fq9MYo6oDaPPLPxSb7gwQN3ih19Zm4Y/0))#tjg09x5t", "sh(multi(2,[00000000/111h/222]xpub6ERApfZwUNrhLCkDtcHTcxd75RbzS1ed54G1LkBUHQVHQKqhMkhgbmJbZRkrgZw4koxb5JaHWkY4ALHY2grBGRjaDMzQLcgJvLJuZZvRcEL,xpub68NZiKmJWnxxS6aaHmn81bvJeTESw724CRDs6HbuccFQN9Ku14VQrADWgqbhhTHBaohPX4CjNLf9fq9MYo6oDaPPLPxSb7gwQN3ih19Zm4Y/0))#hgmsckna", DEFAULT, {{"a91445a9a622a8b0a1269944be477640eedc447bbd8487"}}, OutputType::LEGACY, /*op_desc_id=*/uint256{"c86a81f95cd390705366138f8010817a9f45acaf4cb2ef3f0761135a4aa7855d"}, {{0x8000006FUL,222},{0}});
    Check("sh(multi(2,[00000000/111'/222]xprvA1RpRA33e1JQ7ifknakTFpgNXPmW2YvmhqLQYMmrj4xJXXWYpDPS3xz7iAxn8L39njGVyuoseXzU6rcxFLJ8HFsTjSyQbLYnMpCqE2VbFWc,xprv9uPDJpEQgRQfDcW7BkF7eTya6RPxXeJCqCJGHuCJ4GiRVLzkTXBAJMu2qaMWPrS7AANYqdq6vcBcBUdJCVVFceUvJFjaPdGZ2y9WACViL4L/0))", "sh(multi(2,[00000000/111'/222]xpub6ERApfZwUNrhLCkDtcHTcxd75RbzS1ed54G1LkBUHQVHQKqhMkhgbmJbZRkrgZw4koxb5JaHWkY4ALHY2grBGRjaDMzQLcgJvLJuZZvRcEL,xpub68NZiKmJWnxxS6aaHmn81bvJeTESw724CRDs6HbuccFQN9Ku14VQrADWgqbhhTHBaohPX4CjNLf9fq9MYo6oDaPPLPxSb7gwQN3ih19Zm4Y/0))", "sh(multi(2,[00000000/111h/222]xpub6ERApfZwUNrhLCkDtcHTcxd75RbzS1ed54G1LkBUHQVHQKqhMkhgbmJbZRkrgZw4koxb5JaHWkY4ALHY2grBGRjaDMzQLcgJvLJuZZvRcEL,xpub68NZiKmJWnxxS6aaHmn81bvJeTESw724CRDs6HbuccFQN9Ku14VQrADWgqbhhTHBaohPX4CjNLf9fq9MYo6oDaPPLPxSb7gwQN3ih19Zm4Y/0))", DEFAULT, {{"a91445a9a622a8b0a1269944be477640eedc447bbd8487"}}, OutputType::LEGACY, /*op_desc_id=*/uint256{"c86a81f95cd390705366138f8010817a9f45acaf4cb2ef3f0761135a4aa7855d"}, {{0x8000006FUL,222},{0}});
    ...
    CheckUnparsable("sh(multi(3,[00000000/111'/222]xprvA1RpRA33e1JQ7ifknakTFpgNXPmW2YvmhqLQYMmrj4xJXXWYpDPS3xz7iAxn8L39njGVyuoseXzU6rcxFLJ8HFsTjSyQbLYnMpCqE2VbFWc,xprv9uPDJpEQgRQfDcW7BkF7eTya6RPxXeJCqCJGHuCJ4GiRVLzkTXBAJMu2qaMWPrS7AANYqdq6vcBcBUdJCVVFceUvJFjaPdGZ2y9WACViL4L/0))#ggrsrxfy", "sh(multi(3,[00000000/111'/222]xpub6ERApfZwUNrhLCkDtcHTcxd75RbzS1ed54G1LkBUHQVHQKqhMkhgbmJbZRkrgZw4koxb5JaHWkY4ALHY2grBGRjaDMzQLcgJvLJuZZvRcEL,xpub68NZiKmJWnxxS6aaHmn81bvJeTESw724CRDs6HbuccFQN9Ku14VQrADWgqbhhTHBaohPX4CjNLf9fq9MYo6oDaPPLPxSb7gwQN3ih19Zm4Y/0))#tjg09x5t", "Provided checksum 'tjg09x5t' does not match computed checksum 'u07q3c75'"); // Error in payload
    CheckUnparsable("sh(multi(2,[00000000/111'/222]xprvA1RpRA33e1JQ7ifknakTFpgNXPmW2YvmhqLQYMmrj4xJXXWYpDPS3xz7iAxn8L39njGVyuoseXzU6rcxFLJ8HFsTjSyQbLYnMpCqE2VbFWc,xprv9uPDJpEQgRQfDcW7BkF7eTya6RPxXeJCqCJGHuCJ4GiRVLzkTXBAJMu2qaMWPrS7AANYqdq6vcBcBUdJCVVFceUvJFjaPdGZ2y9WACViL4L/0))#ggssrxfy", "sh(multi(2,[00000000/111'/222]xpub6ERApfZwUNrhLCkDtcHTcxd75RbzS1ed54G1LkBUHQVHQKqhMkhgbmJbZRkrgZw4koxb5JaHWkY4ALHY2grBGRjaDMzQLcgJvLJuZZvRcEL,xpub68NZiKmJWnxxS6aaHmn81bvJeTESw724CRDs6HbuccFQN9Ku14VQrADWgqbhhTHBaohPX4CjNLf9fq9MYo6oDaPPLPxSb7gwQN3ih19Zm4Y/0))#tjq09x4t", "Provided checksum 'tjq09x4t' does not match computed checksum '6gsqgcwn'"); // Error in checksum
    ...
    Check(
        ...
        OutputType::BECH32M,
        /*op_desc_id=*/uint256{"b7152f3dde4c264df23c12731152d106735885fade91effa3f2681cd3d3d1fa1"},
