...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, Dersig100Setup)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        spends[i].vout[0].nValue = (11 + i) * CENT; // differentiate the txids via the value
        spends[i].vout[0].scriptPubKey = scriptPubKey;

        // Sign (the coinbase pays the shared test WITNESS_V2_PQ script):
        SignTestPQInput(spends[i], 0, m_coinbase_txns[0]->vout[0]);
    }
...
// DISABLED on NEX: this test builds its entire flag matrix from legacy script
// types (P2PK/P2SH/P2WPKH/CLTV/CSV outputs) mined into blocks, which the
// PQ-only output rule (bad-txout-not-pq) makes consensus-invalid — the
// pre-DERSIG/pre-P2SH activation scenarios it exercises cannot be staged from
// valid NEX transactions. Script-flag caching is still exercised via the PQ
// spends in the other block-producing suites.
