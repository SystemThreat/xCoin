...
BOOST_FIXTURE_TEST_CASE(connect_tip_does_not_cache_inputs_on_failed_connect, TestChain100Setup)
{
    ...
    // A PQ output so the block passes context-free checks and fails only at
    // connect time (spending more than the input's value), as upstream intended.
    tx.vout.emplace_back(MAX_MONEY, GetTestPQScript());

    const auto tip{WITH_LOCK(cs_main, return chainstate.m_chain.Tip()->GetBlockHash())};
    const CBlock block{CreateBlock({tx}, GetTestPQScript(), chainstate)};
