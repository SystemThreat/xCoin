    ...
    : m_args{}
{
    ...
    std::vector<const char*> arguments = Cat(
        ...
        opts.extra_args);
    // The shared fixtures (TestChain100Setup coinbases, CreateValidMempoolTransaction callers)
    // still pay to the legacy witness v2 test script; relaying those is non-standard by default
    // after the re-genesis, so permit it for the harness. The option is regtest only (mainnet and
    // the rehearsal chain have no witness v2 outputs at all and refuse it), and so is the need.
    if (chainType == ChainType::REGTEST) {
        arguments.insert(arguments.begin() + 1, "-permitv2outputs=1");
    }
    // The settlement levy is off on every chain at genesis (REGENESIS.md section 6);
    // a test that exercises it enables it itself with -levybp (xcoin_levy_tests,
    // spend_tests). Nothing to inject here.
    // Coinbase maturity is 1,000 on every chain, regtest included; the inherited
    // fixtures mine 100 blocks to a spendable coinbase, so the harness passes 100
    // unless the test itself sets -coinbasematurity.
    if (std::none_of(opts.extra_args.begin(), opts.extra_args.end(), [](const char* a) { return std::string_view{a}.starts_with("-coinbasematurity"); })) {
        arguments.insert(arguments.begin() + 1, "-coinbasematurity=100");
    }
    // A mainnet fixture (ChainType::MAIN) runs AppInitParameterInteraction below,
    // which refuses to start a mainnet node while GENESIS_IS_FINAL is false
    // (kernel/chainparams.h, CheckGenesisFinalityForStartup). The harness never
    // peers with anyone, so it takes the cutover dry-run override; the refusal
    // itself is unit-tested directly (regenesis_tests/mainnet_v2_identity_and_startup_gate).
    if (std::none_of(opts.extra_args.begin(), opts.extra_args.end(), [](const char* a) { return std::string_view{a}.starts_with("-allowunfinalgenesis"); })) {
        arguments.insert(arguments.begin() + 1, "-allowunfinalgenesis=1");
    }
    ...
    : TestingSetup{ChainType::REGTEST, opts}
{
...

    {
        ...
        // Re-pinned 2026-09-14: the previous pin (5780b38e..., fc15bd4 of 2026-09-05)
        // predated the removal of the block-1 distribution (35c4fda, 506d9ad,
        // 8e440be, 7263c5d). Deterministic across runs: this is what the fixture mines today.
        static constexpr std::string_view EXPECTED_TIP{"b5dfd2da7d35368bb64b90ed11cab2729ea0ace537ef31a1932411f64287dbcf"};
        const std::string tip{m_node.chainman->ActiveChain().Tip()->GetBlockHash().ToString()};
        if (tip != EXPECTED_TIP) {
            std::fprintf(stderr, "TestChain100Setup: deterministic tip is %s, expected %s\n", tip.c_str(), std::string{EXPECTED_TIP}.c_str());
        }
...
{
    // NEX consensus only allows WITNESS_V2_PQ (and NULL_DATA) outputs, so the
    // coinbase pays the shared deterministic test ML-DSA-65 key instead of the
    // upstream P2PK-to-coinbaseKey script.
    ...
    Chainstate& chainstate)
{
    ...
    // Grind the header-based consensus PoW (MetalDAG), not the SHA-256d block
    // hash — they differ, and only the former is what validation checks.
                                                                                  ...
                                                                                  const std::optional<uint32_t>& fee_output)
{
    ...
    // The built-in signer refuses WITNESS_V2_PQ on purpose, so sign PQ inputs
    // (e.g. the fixture's coinbase outputs) with the shared test ML-DSA-65 key
    // after letting SignTransaction handle any legacy inputs.
    auto sign_all_inputs = [&] {
        std::map<int, bilingual_str> input_errors;
        SignTransaction(mempool_txn, &keystore, input_coins, nHashType, input_errors);
        for (unsigned int i = 0; i < mempool_txn.vin.size(); ++i) {
            const CTxOut& spent = input_coins.at(mempool_txn.vin[i].prevout).out;
            if (IsTestPQScript(spent.scriptPubKey)) {
                SignTestPQInput(mempool_txn, i, spent, nHashType);
                input_errors.erase(i);
            }
        }
        assert(input_errors.empty());
    };
    sign_all_inputs();
