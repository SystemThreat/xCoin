...
void BulkTransaction(CMutableTransaction& tx, int32_t target_weight)
{
    ...
    // pad transaction by repeatedly appending a dummy opcode to the OP_RETURN
    // output added above (OP_1 counts as a push, so the script stays NULL_DATA,
    // which is the only non-PQ output type NEX consensus accepts)
    tx.vout.back().scriptPubKey.insert(tx.vout.back().scriptPubKey.end(), dummy_vbytes, OP_1);
