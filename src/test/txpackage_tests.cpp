...
// unit tests. Scaled 10x from upstream: an ML-DSA spend is ~1,455 vB, so 2,000 sat sits above
// the 1 sat/vB relay floor and below the 5 sat/vB mempool minimum the tests mock.
static const CAmount low_fee_amt{2000};
...
inline CTransactionRef create_placeholder_tx(size_t num_inputs, size_t num_outputs)
{
    ...
    for (size_t o{0}; o < num_outputs; ++o) {
        ...
        // Outputs must be PQ or CheckTransaction fails before the policy
        // checks these placeholders are used to exercise.
        mtx.vout[o].scriptPubKey = GetTestPQScript();
    }
...
BOOST_AUTO_TEST_CASE(package_validation_tests)
{
    ...
    CScript parent_locking_script = GetTestPQScript();
    ...
    CScript child_locking_script = GetTestPQScript();
...
BOOST_AUTO_TEST_CASE(noncontextual_package_tests)
{
    ...
    CScript spk = GetTestPQScript();
    CKey placeholder_key_2 = GenerateRandomKey();
    CScript spk2 = GetTestPQScript();
...
BOOST_AUTO_TEST_CASE(package_submission_tests)
{
    ...
    CScript grandchild_locking_script = GetTestPQScript();
    ...
    // missing inputs, so the package validation isn't expected to happen.
    {
        ...
        if (auto err_parent_invalid{CheckPackageMempoolAcceptResult(package_invalid_parent, result_quit_early, /*expect_valid=*/false, m_node.mempool.get())}) {
            ...
            // Upstream classified this as TX_WITNESS_MUTATED via the
            // IsWitnessStandard pre-check (a witness on a non-witness input).
            // On NEX the input IS a witness (v2 PQ) program, so the garbage
            // witness instead fails script validation.
            BOOST_CHECK_EQUAL(it_parent->second.m_state.GetResult(), TxValidationResult::TX_NOT_STANDARD);
            BOOST_CHECK_EQUAL(it_parent->second.m_state.GetRejectReason(), "mempool-script-verify-flag-failed (Witness program hash mismatch)");
    ...
    // In-mempool parent and child with missing parent.
    {
        ...
        auto tx_parent_2 = MakeTransactionRef(CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[2], /*input_vout=*/0,
                                                                            /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                                            /*output_destination=*/parent_locking_script,
                                                                            /*output_amount=*/CAmount(50 * COIN - 8000), /*submit=*/false)); // 10x upstream's 800: PQ txs are ~10x the vsize and must beat the mocked 5sat/vB min fee
...
BOOST_AUTO_TEST_CASE(package_single_tx)
{
    ...
    const CAmount high_fee{10000}; // ~7 sat/vB on a ~1,455-vB ML-DSA spend (upstream: 1000)

    // No unconfirmed parents
    CKey single_key = GenerateRandomKey();
    CScript single_locking_script = GetTestPQScript();
...
BOOST_AUTO_TEST_CASE(package_witness_swap_tests)
{
    ...
    // On NEX the parent must pay a PQ output. Two distinct valid witnesses for
    // the same txid are produced below by signing with different sighash types
    // (the ML-DSA signature lives only in the witness).
    ...
    SignTestPQInput(mtx_child1, 0, CTxOut(49 * COIN, GetTestPQScript()), SIGHASH_ALL);

    CMutableTransaction mtx_child2{mtx_child1};
    SignTestPQInput(mtx_child2, 0, CTxOut(49 * COIN, GetTestPQScript()), SIGHASH_ALL | SIGHASH_ANYONECANPAY);
    ...
    // All parents pay the shared test PQ key; the child's inputs are signed
    // explicitly below (there is no anyone-can-spend output type on NEX).
    CScript acs_spk = GetTestPQScript();
    ...
    CScript grandparent2_spk = GetTestPQScript();
    ...
    SignTestPQInput(mtx_parent2_v1, 0, CTxOut(49 * COIN, GetTestPQScript()), SIGHASH_ALL);

    CMutableTransaction mtx_parent2_v2{mtx_parent2_v1};
    SignTestPQInput(mtx_parent2_v2, 0, CTxOut(49 * COIN, GetTestPQScript()), SIGHASH_ALL | SIGHASH_ANYONECANPAY);
    ...
    CScript mixed_child_spk = GetTestPQScript();
    ...
    SignTestPQInput(mtx_mixed_child, 0, CTxOut(49 * COIN, GetTestPQScript()));
    SignTestPQInput(mtx_mixed_child, 1, CTxOut(48 * COIN, GetTestPQScript()));
    SignTestPQInput(mtx_mixed_child, 2, CTxOut(50 * COIN - low_fee_amt, GetTestPQScript()));
...
BOOST_AUTO_TEST_CASE(package_cpfp_tests)
{
    ...
    CKey child_key = GenerateRandomKey();
    CScript parent_spk = GetTestPQScript();
    CKey grandchild_key = GenerateRandomKey();
    CScript child_spk = GetTestPQScript();
    ...
    // Scaled from upstream's 200/600: PQ transactions are ~1455 vB, and the
    // child alone must beat the mocked 5 sat/vB mempool min fee while the
    // package as a whole stays below it.
    const CAmount parent_fee{2000};
    const CAmount child_fee{9000};
...
BOOST_AUTO_TEST_CASE(package_rbf_tests)
{
    ...
    {
        // 1 parent paying 2000sat, 1 child paying 3000sat
        Package package1;
        // 1 parent paying 2000sat, 1 child paying 5000sat (2000 more: the incremental
        // relay fee on a ~1,455-vB replacement at 1 sat/vB)
        Package package2;
        ...
        CTransactionRef tx_child_1 = MakeTransactionRef(CreateValidMempoolTransaction(tx_parent, 0, 101, child_key, child_spk, coinbase_value - low_fee_amt - 3000, false));
        package1.push_back(tx_child_1);
        CTransactionRef tx_child_2 = MakeTransactionRef(CreateValidMempoolTransaction(tx_parent, 0, 101, child_key, child_spk, coinbase_value - low_fee_amt - 5000, false));
    ...
    // Test package rbf.
    // (Fee amounts are scaled 10x from upstream: PQ transactions are ~10x the
    // vsize of the upstream legacy ones, and the RBF feerate-diagram rules
    // compare feerates, not absolute fees.)
    {
        ...
        // 1 parent paying 2000sat, 1 child paying 2000sat.
        Package package1{tx_parent_1, tx_child_1};
        // 1 parent paying 8000sat, 1 child paying 2000sat.
        Package package2{tx_parent_2, tx_child_2};
        // 1 parent paying 1990sat, 1 child paying 17000sat.
        ...
        BOOST_CHECK_EQUAL(it_parent_3->second.m_effective_feerate.value().GetFee(package3_total_vsize), 1990 + 17000);
        BOOST_CHECK_EQUAL(it_child_3->second.m_effective_feerate.value().GetFee(package3_total_vsize), 1990 + 17000);
        ...
        m_node.mempool->PrioritiseTransaction(tx_child_1->GetHash(), 20000); // package1 then pays 24,000: 5,010 more than package3, over the ~2,824 vB it replaces at 1 sat/vB
