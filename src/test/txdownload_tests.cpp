...
BOOST_FIXTURE_TEST_CASE(handle_missing_inputs, TestChain100Setup)
{
    ...
    CScript destination = GetTestPQScript(); // only PQ outputs are consensus-valid on NEX
    // Amount for spending coinbase in a 1-in-1-out tx, at depth n, each time deducting 10000 from the amount as fees
    // (~7 sat/vB on a ~1,440-vB ML-DSA spend, above the 1 sat/vB relay floor; upstream deducts 1000).
    CAmount amount_depth_1{50 * COIN - 10000};
    CAmount amount_depth_2{amount_depth_1 - 10000};
