...
BOOST_AUTO_TEST_CASE(disconnectpool_memory_limits)
{
    ...
    // The block-1 coinbase is skipped: it once paid a chain-defined genesis
    // distribution instead of the miner's script. Block 1 is an ordinary block
    // now (7263c5d); 99 coinbases are still plenty, so the slice is kept.
    std::vector<CTransactionRef> block_vtx(m_coinbase_txns.begin() + 1, m_coinbase_txns.end());
