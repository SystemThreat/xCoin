...
FUZZ_TARGET(transaction, .init = initialize_transaction)
{
...

    {
        ...
        const bool res{CheckTransaction(tx, state_with_dupe_check, /*permit_v2_outputs=*/true)};
        Assert(res == state_with_dupe_check.IsValid());
    }

    const CFeeRate dust_relay_fee{DUST_RELAY_TX_FEE};
    std::string reason;
    const bool is_standard_with_permit_bare_multisig = IsStandardTx(tx, std::nullopt, /* permit_bare_multisig= */ true, /*permit_v2_outputs=*/false, dust_relay_fee, reason);
    const bool is_standard_without_permit_bare_multisig = IsStandardTx(tx, std::nullopt, /* permit_bare_multisig= */ false, /*permit_v2_outputs=*/false, dust_relay_fee, reason);
