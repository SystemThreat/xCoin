...
BOOST_FIXTURE_TEST_CASE(txospenderindex_initial_sync, TestChain100Setup)
{
    ...
    for (size_t i = 0; i < spent.size(); i++) {
        ...
        // Only vout 0 is being spent; GetValueOut() would count every output.
        spender[i].vout[0].nValue = coinbase_tx->vout[0].nValue;
        spender[i].vout[0].scriptPubKey = coinbase_script;

        // Sign (the coinbases pay the shared test WITNESS_V2_PQ script)
        SignTestPQInput(spender[i], 0, coinbase_tx->vout[0]);
    }
