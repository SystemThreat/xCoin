...
std::shared_ptr<CBlock> MinerTestingSetup::Block(const uint256& prev_hash)
{
    ...
    // One zero-value OP_RETURN with a unique payload to make sure that blocks
    // at the same height can have a different hash, and one that pays the
    // reward to the shared test PQ key (the only spendable output type on
    // NEX; upstream used P2WSH-OP_TRUE).
    CMutableTransaction txCoinbase(*pblock->vtx[0]);
    const int prev_height{WITH_LOCK(::cs_main, return m_node.chainman->m_blockman.LookupBlockIndex(prev_hash)->nHeight)};
    // The assembler built this template on the active tip, but the test grafts
    // it onto arbitrary parents, so pay the era subsidy of the real height (a
    // template built for a lower height could otherwise overpay the coinbase).
    txCoinbase.vout.clear();
    txCoinbase.vout.emplace_back(0, CScript{} << OP_RETURN << i++);
    txCoinbase.vout.emplace_back(GetBlockSubsidy(prev_height + 1, Params().GetConsensus()), GetTestPQScript());
...
std::shared_ptr<CBlock> MinerTestingSetup::FinalizeBlock(std::shared_ptr<CBlock> pblock)
{
    ...
    while (!CheckProofOfWork(*pblock, pblock->nBits, Params().GetConsensus())) {
        ++(pblock->nNonce);
    }
...
BOOST_AUTO_TEST_CASE(mempool_locks_reorg)
{
    ...
    for (int test_runs = 3; test_runs > 0; --test_runs) {
        ...
        for (int num_txs = 22; num_txs > 0; --num_txs) {
            ...
            mtx.vout[0].nValue -= 10000; // ~7 sat/vB on a ~1,440-vB ML-DSA spend, above the 1 sat/vB relay floor
            SignTestPQInput(mtx, 0, last_mined->vtx[0]->vout[1]);
