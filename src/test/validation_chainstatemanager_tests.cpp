        ...
        for (const bool loading_blocks : {false, true}) {
            for (const bool tip_exists : {false, true}) {
                for (const bool enough_work : {false, true}) {
                    // NEX ships a provisional nMinimumChainWork of zero until
                    // the chain has depth, so the insufficient-work scenario
                    // cannot be constructed (MinimumChainWork() - 1 wraps to
                    // the maximum work value).
                    if (!enough_work && chainman.MinimumChainWork() == arith_uint256{0}) continue;
...
struct SnapshotTestSetup : TestChain100Setup {
    ...
    std::tuple<Chainstate*, Chainstate*> SetupSnapshot()
    {
        ...
        // Make some initial assertions about the contents of the chainstate.
        {
            ...
            // Every coinbase on the test chain has a single output (block 1 is
            // an ordinary block, 7263c5d), so every block adds exactly one coin.
