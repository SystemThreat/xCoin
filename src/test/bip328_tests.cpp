...
//! The BIP328 vectors serialize xpubs with Bitcoin's version bytes; swap in
//! NEX's EXT_PUBLIC_KEY prefix before comparing.
std::string AdaptXpub(const std::string& in)
{
    std::vector<unsigned char> data;
    if (!DecodeBase58Check(in, data, 100) || data.size() < 4) return in;
    std::vector<unsigned char> out{Params().Base58Prefix(CChainParams::EXT_PUBLIC_KEY)};
...
BOOST_AUTO_TEST_CASE(valid_keys)
{
    ...
    for (int i = 0; i < (int) test_vectors.size(); ++i) {
        ...
        BOOST_CHECK_MESSAGE(xpub == AdaptXpub(test.expected_aggregate_xpub), "Test vector " << i << ": Synthetic xpub mismatch");
    }
}
