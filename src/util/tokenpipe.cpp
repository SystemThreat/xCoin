...
{
    int fds[2] = {-1, -1};
// Not on Apple even when the SDK declares pipe2: from the macOS 27.0 SDK on it is
// declared with a future availability, so the configure check passes and the call
// links as a weak import that resolves to NULL on every macOS that ships today —
// the first SignalInterrupt then jumps to address 0 (seen in test_bitcoin).
#if HAVE_O_CLOEXEC && HAVE_DECL_PIPE2 && !defined(__APPLE__)
