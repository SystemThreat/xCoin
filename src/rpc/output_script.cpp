...
static RPCHelpMan getmetaldagpowhash()
{
    return RPCHelpMan{
        "getmetaldagpowhash",
        "Compute the MetalDAG proof-of-work hash of an 80-byte block header (light verify).\n"
        "Used by the mining pool to validate MetalDAG shares without reimplementing the PoW.\n"
        "Returns the 256-bit PoW hash as big-endian hex (compare as an integer against the target).\n",
        {
            {"headerhex", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The 80-byte block header, hex-encoded (version|prevhash|merkle|ntime|nbits|nonce)."},
        },
        RPCResult{RPCResult::Type::STR_HEX, "", "The MetalDAG proof-of-work hash (32 bytes, big-endian hex)"},
        RPCExamples{HelpExampleCli("getmetaldagpowhash", "\"<160 hex chars>\"")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            if (!IsHex(request.params[0].get_str())) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "headerhex must be hex");
            }
            std::vector<unsigned char> data = ParseHex(request.params[0].get_str());
            if (data.size() != 80) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "header must be exactly 80 bytes (160 hex chars)");
            }
            DataStream ss(data);
            CBlockHeader header;
            ss >> header;
            // Match the cheap pre-PoW gate in CheckProofOfWork(header, ...).
            // This RPC is normally called by a public Stratum bridge, so its
            // authenticated node connection does not make header.nTime trusted.
            // Without this check a remote miner can select a far-future epoch
            // and force construction of a very large verification cache.
            if ((int64_t)header.nTime > GetTime() + 2 * MAX_FUTURE_BLOCK_TIME) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "header time is too far in the future");
            }
            uint256 h = metaldag::PoWHash(header, Params().GetConsensus());
            return h.GetHex();  // big-endian display; the pool compares it as an integer
        },
    };
}
...
static RPCHelpMan createmultisig()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            } else if (output_type.value() == OutputType::XCOIN_V2 || output_type.value() == OutputType::XCOIN_V3) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "createmultisig cannot create post-quantum multisig addresses; use a pqtr() descriptor");
            }
...
static RPCHelpMan getdescriptorinfo()
{
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            result.pushKV("hasprivatekeys", (provider.keys.size() > 0 || !provider.pq_keys.empty() || !provider.slh_keys.empty()));
            return result;
        },
    };
}
