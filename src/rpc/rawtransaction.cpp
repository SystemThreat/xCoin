    ...
    return RPCHelpMan{
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::ARR, "xcoin_v3_leaf_warnings", /*optional=*/true,
                 "Present only when this script would be ANYONE-CAN-SPEND if it were used as an xCoin witness v3 leaf script:\n"
                 "it carries an OP_SUCCESSx byte, or its key push is not 32 bytes (an upgradable key type, which passes the\n"
                 "signature check without verifying anything). Not a statement about this script's own validity.",
                 {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // xCoin witness v3 (REGENESIS.md section 4): say so when this script would
    // be anyone-can-spend as a leaf script, so a wallet can warn before paying
    ...
    if (const std::vector<std::string> leaf_warnings{XcoinV3LeafWarnings(script)}; !leaf_warnings.empty()) {
        ...
        r.pushKV("xcoin_v3_leaf_warnings", std::move(arr));
    }
