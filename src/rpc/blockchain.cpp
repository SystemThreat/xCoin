...
static RPCHelpMan getcharter()
{
    return RPCHelpMan{
        "getcharter",
        "Returns the xCoin charter commitment of this chain (contrib/regenesis/REGENESIS.md section 1): the\n"
        "hash of the charter compiled into this node, the currency id derived from the genesis header and the\n"
        "charter text, the genesis it was derived from, the first chain's genesis this chain descends from, and\n"
        "whether that genesis is final. Digests are plain SHA-256 in natural byte order (as shasum prints them);\n"
        "block hashes use the usual display order.\n",
        {},
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::STR_HEX, "charter_hash", "SHA-256 of contrib/regenesis/CHARTER.md"},
                {RPCResult::Type::STR_HEX, "currency_id", "SHA-256(genesis header (80 bytes) || charter text)"},
                {RPCResult::Type::STR_HEX, "genesis_hash", "hash of the genesis block compiled into this node"},
                {RPCResult::Type::STR_HEX, "v1_genesis_hash", "hash of the first chain's genesis block (the lineage)"},
                {RPCResult::Type::BOOL, "genesis_is_final", "true once the final v2 genesis (with the charter output) is compiled in; false while a placeholder stands in, in which case currency_id must not be attested or timestamped"},
            }},
        RPCExamples{
            HelpExampleCli("getcharter", "")
            + HelpExampleRpc("getcharter", "")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    obj.pushKV("charter_hash", charter::DigestHex(Consensus::CHARTER_HASH));
    obj.pushKV("currency_id", charter::DigestHex(charter::CurrencyId(genesis)));
    obj.pushKV("genesis_hash", genesis.GetHash().GetHex());
    obj.pushKV("v1_genesis_hash", Consensus::V1_GENESIS_HASH.GetHex());
    obj.pushKV("genesis_is_final", params.GenesisIsFinal());
    return obj;
},
    };
}
