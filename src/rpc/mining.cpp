...
static bool GenerateBlock(ChainstateManager& chainman, CBlock&& block, uint64_t& max_tries, std::shared_ptr<const CBlock>& block_out, bool process_new_block)
{
    ...
    while (max_tries > 0 && block.nNonce < std::numeric_limits<uint32_t>::max() && !CheckProofOfWork(block, block.nBits, chainman.GetConsensus()) && !chainman.m_interrupt) {
        ++block.nNonce;
        --max_tries;
    }
...
static RPCHelpMan getblocktemplate()
{
    return RPCHelpMan{
        ...
        "On mainnet a template is only served while the node has at least one peer and is not in\n"
        "initial block download (errors -9 and -10 otherwise); the node option -allowsolomining lifts\n"
        "that gate for a single-node launch or an isolated rehearsal. Test chains are exempt.\n",
        {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Upstream's gate: a template is only served while the node has peers and
    // is not in initial block download, so a miner never works on a stale tip.
    // Test chains (isTestChain) are exempt as upstream. -allowsolomining lifts
    // the gate for a single-node launch or an isolated rehearsal (the node
    // warns at startup when it is set on mainnet or testnet A).
    if (!miner.isTestChain() && !EnsureArgsman(node).GetBoolArg("-allowsolomining", node::DEFAULT_ALLOW_SOLO_MINING)) {
    ...
    // What the miner may pay itself: subsidy plus fees at every height, block 1
    // included (there is no block-1 distribution).
    const auto coinbase{block_template->getCoinbaseTx()};
    result.pushKV("coinbasevalue", coinbase.block_reward_remaining);
    ...
    if (coinbase.required_outputs.size() > 0) {
        // The witness commitment is the only required output.
        result.pushKV("default_witness_commitment", HexStr(coinbase.required_outputs.back().scriptPubKey));
    }
