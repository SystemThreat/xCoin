...
public:
    ...
    /** Whether this chain's genesis is the final v2 genesis that commits to
     *  the charter (REGENESIS.md section 1). False while the mainnet
     *  FINAL_GENESIS_* constants are unset (the v1 genesis stands in as a
     *  placeholder) and on chains whose genesis is a placeholder; reported by
     *  `getcharter` so nobody attests or timestamps a provisional genesis. */
    bool GenesisIsFinal() const { return m_genesis_is_final; }
    ...
    struct RegTestOptions {
        ...
        /** Settlement levy override (-levybp, 0..10,000); unset = the mainnet value. */
        std::optional<int64_t> levy_bp{};
        /** Coinbase maturity override (-coinbasematurity, >= 1); unset = the mainnet value, 1,000. */
        std::optional<int> coinbase_maturity{};
    };
